...
# Note: We don't use 'set -e' here because cleanup should continue even if some commands fail
...
stop_localstack() {
    ...
    if [ -f "docker-compose-localstack.yml" ]; then
        ...
        print_warning "docker-compose-localstack.yml not found in setup directory, stopping container directly..."
        # Stop any container with "localstack" in the name
        LOCALSTACK_CONTAINERS=$(docker ps -a --format "{{.Names}}" | grep localstack || true)
        if [ -n "$LOCALSTACK_CONTAINERS" ]; then
            echo "$LOCALSTACK_CONTAINERS" | while read container; do
                print_step "Stopping container: $container"
                docker stop "$container" 2>/dev/null || true
                docker rm "$container" 2>/dev/null || true
            done
            print_success "LocalStack container(s) stopped and removed"
        else
            print_info "No LocalStack containers found"
        fi
    fi
}
...
main() {
    ...
    # Clean up AWS resources (unless skipped)
    if [ "$SKIP_AWS" != "true" ]; then
        cleanup_aws_resources
    else
        print_info "Skipping AWS resource cleanup"
    fi
    ...
    echo ""
    print_info "To restart the environment, run: cd $SCRIPT_DIR && ./setup.sh"
}
