...
stop_services() {
    ...
    if [ -f "docker-compose.yml" ]; then
        ...
        print_warning "docker-compose.yml not found in setup directory. Stopping containers manually..."
...
main() {
    ...
    if check_running_services; then
        ...
        echo ""
        echo "To start again, run: cd $SCRIPT_DIR && ./start-local-kafka.sh"
        echo "For force cleanup: cd $SCRIPT_DIR && ./stop-local-kafka.sh --force"
        echo ""
    else
