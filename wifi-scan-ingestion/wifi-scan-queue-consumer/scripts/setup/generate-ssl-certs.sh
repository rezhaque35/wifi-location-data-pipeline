...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "Options:"
            echo "  --force    Force regenerate certificates even if they already exist"
...
# Directory paths - Fixed to create kafka/secrets in parent scripts directory
SECRETS_DIR="${SCRIPTS_DIR}/kafka/secrets"
# Get project root directory (2 levels up from scripts dir)
PROJECT_ROOT="$(dirname "$SCRIPTS_DIR")"
TEST_RESOURCES_SECRETS_DIR="${PROJECT_ROOT}/src/test/resources/secrets"

# Function to check if certificates already exist
check_existing_certificates() {
    if [ -f "${SECRETS_DIR}/${KEYSTORE_FILE}" ] && \
       [ -f "${SECRETS_DIR}/${TRUSTSTORE_FILE}" ] && \
       [ -f "${TEST_RESOURCES_SECRETS_DIR}/${KEYSTORE_FILE}" ] && \
       [ -f "${TEST_RESOURCES_SECRETS_DIR}/${TRUSTSTORE_FILE}" ]; then
...
main() {
    ...
    print_status "Test resources directory: $TEST_RESOURCES_SECRETS_DIR"
    echo ""
    ...
    if check_existing_certificates; then
        if [ "$FORCE_REGENERATE" = false ]; then
            ...
            print_status "Keystore: ${SECRETS_DIR}/${KEYSTORE_FILE}"
            print_status "Truststore: ${SECRETS_DIR}/${TRUSTSTORE_FILE}"
            echo ""
            print_warning "Certificates are already configured. Skipping generation to avoid service disruption."
            print_status "To regenerate certificates (requires service restart), run: $0 --force"
            echo ""
            return 0
        else
            print_warning "Force regeneration requested. Existing certificates will be replaced."
            print_warning "⚠️  You will need to restart your Spring Boot application after this!"
            echo ""
        fi
    else
        print_status "No existing certificates found. Generating new certificates..."
        echo ""
    fi
