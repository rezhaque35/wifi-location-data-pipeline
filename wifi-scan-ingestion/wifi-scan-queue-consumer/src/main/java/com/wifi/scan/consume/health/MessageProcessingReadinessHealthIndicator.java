 ...
 * 
 * Readiness focuses on whether the service is ready to handle requests/traffic.
 * This indicator checks:
 * - Consumer connectivity and group membership
 * - Topic accessibility
 * - Recent processing activity (with tolerance for idle periods)
 * - Processing performance and error rates
 * 
 * Unlike liveness, readiness can be more strict about operational health
 * but should still be tolerant of normal idle periods.
 */
@Slf4j
...
public class MessageProcessingReadinessHealthIndicator implements HealthIndicator {
    ...
    @Autowired
    public MessageProcessingReadinessHealthIndicator(KafkaMonitoringService kafkaMonitoringService,
    ...
    public Health health() {
        try {
            ...
            boolean isReady = isConsumerConnected && 
                             isConsumerGroupActive && 
                             areTopicsAccessible &&
                             (isConsumptionHealthy || totalConsumed == 0); // Allow healthy status if no messages consumed yet
