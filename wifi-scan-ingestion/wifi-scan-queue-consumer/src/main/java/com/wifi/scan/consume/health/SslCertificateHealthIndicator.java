...
/**
 * Health indicator for SSL certificate validation.
 * Checks SSL/TLS configuration, certificate accessibility, and expiration dates.
 */
@Slf4j
...
public class SslCertificateHealthIndicator implements HealthIndicator {
    ...
    @Autowired
    public SslCertificateHealthIndicator(KafkaMonitoringService kafkaMonitoringService,
    ...
    public Health health() {
        ...
        try {
            ...
            CertificateInfo keystoreInfo = checkCertificateStore(
            ...
            CertificateInfo truststoreInfo = checkCertificateStore(
            ...
            // Determine the minimum days until expiry across all stores for CloudWatch metric & warning level
            long minDaysUntilExpiry = Math.min(
                    keystoreInfo.getDaysUntilExpiry() == -1 ? Long.MAX_VALUE : keystoreInfo.getDaysUntilExpiry(),
                    truststoreInfo.getDaysUntilExpiry() == -1 ? Long.MAX_VALUE : truststoreInfo.getDaysUntilExpiry());
            ...
            if (!isSslHealthy) {
                ...
                if (warningLevel != null) {
                    healthBuilder.withDetail("warning", "SSL certificates are " + warningLevel.toLowerCase() + " (" + minDaysUntilExpiry + " days left)");
                }
            }
    ...
    private CertificateInfo checkCertificateStore(String location, String password, String type) {
        try {
            ...
            return new CertificateInfo(true, hasExpired, expiringSoon, 
     ...
     * <p>
     * WARNING  –&gt; &gt;= urgent threshold (default 30 days) &amp;&amp; &lt;= warning threshold
     * URGENT   –&gt; &gt;= critical threshold (default 15 days) &amp;&amp; &lt; warning threshold
     * CRITICAL –&gt; &lt;= critical threshold (default 7 days)
     * </p>
     * The method returns <code>null</code> when the certificate is considered healthy
     * (i.e., days until expiry is above the warning threshold).
     *
    ...
    private static class CertificateInfo {
        ...
        public CertificateInfo(boolean accessible, boolean expired, boolean expiringSoon, long daysUntilExpiry) {
