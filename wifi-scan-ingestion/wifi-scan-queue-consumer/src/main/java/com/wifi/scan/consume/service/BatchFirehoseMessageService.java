 ...
 * 
 * This service implements efficient batch delivery to AWS Kinesis Data Firehose with
 * intelligent sub-batching, rate limiting integration, and comprehensive metrics tracking.
 * It handles the complex requirements of Firehose delivery including size limits,
 * batch constraints, and enhanced error handling with intelligent retry strategies.
 * 
 * Key Functionality:
 * - Intelligent sub-batch sizing based on message size and rate limits
 * - Firehose-compliant batch delivery with size and count limits
 * - Enhanced exception classification and retry strategies
 * - Buffer full scenario handling with graduated backoff
 * - Comprehensive metrics collection and reporting
 * - Connectivity testing and health monitoring
 * - Error handling and failed record logging
 * 
 * High-Level Processing Steps:
 * 1. Receive pre-compressed messages from upstream services
 * 2. Calculate optimal sub-batch sizes based on constraints
 * 3. Create sub-batches respecting Firehose limits
 * 4. Deliver sub-batches to Firehose with intelligent error handling
 * 5. Apply exception-specific retry strategies (Buffer Full, Rate Limit, Network, Generic)
 * 6. Track metrics and handle failed deliveries
 * 7. Provide connectivity testing and health status
 * 
 * Firehose Constraints Handled:
 * - Maximum batch size: 500 records per batch
 * - Maximum batch size: 4MB per batch
 * - Maximum record size: 1MB per record
 * - Rate limiting integration for downstream protection
 * - Buffer full scenarios with intelligent backoff
 * 
 * NOTE: Messages are expected to be pre-compressed and encoded by MessageCompressionService.
...
public class BatchFirehoseMessageService {
     ...
     * 
     * This method creates optimally-sized sub-batches from a list of messages,
     * respecting ALL Firehose limits including record count, batch size in bytes,
     * and individual message sizes. It uses intelligent partitioning to maximize
     * throughput while staying within AWS Kinesis Data Firehose constraints.
     * 
     * Processing Steps:
     * 1. Validate input messages for null or empty conditions
     * 2. Initialize sub-batch tracking with size and count limits
     * 3. For each message, calculate its byte size
     * 4. Check if adding the message would exceed either limit:
     *    - Record count limit (maxBatchSize parameter)
     *    - Byte size limit (maxBatchSizeBytes parameter)
     * 5. If either limit would be exceeded, start a new sub-batch
     * 6. Add message to current sub-batch and update counters
     * 7. Finalize and return all sub-batches
     * 
     * Sub-batch Creation Logic:
     * - Each sub-batch respects both record count AND byte size limits
     * - Messages are processed sequentially to maintain order
     * - Empty sub-batches are avoided through proper validation
     * - Single large messages get their own sub-batch if needed
     * 
     * Constraint Handling:
     * - Record Count: Maximum specified by maxBatchSize parameter
     * - Byte Size: Maximum specified by maxBatchSizeBytes parameter
     * - Individual Size: Should be pre-validated by caller
     * 
     * Performance Characteristics:
     * - O(n) time complexity where n is number of messages
     * - Minimal memory overhead with incremental size tracking
     * - Optimal sub-batch utilization maximizing Firehose throughput
     * 
     ...
     */
    public List<List<String>> createSubBatches(List<String> messages, int maxBatchSize, long maxBatchSizeBytes) {
        ...
        log.debug("Creating sub-batches from {} messages with dual constraints: {} records and {} bytes", 
                 messages.size(), maxBatchSize, maxBatchSizeBytes);
        ...
        for (String message : messages) {
            ...
            if ((exceedsRecordLimit || exceedsSizeLimit) && !currentSubBatch.isEmpty()) {
                ...
                log.trace("Created sub-batch: {} records, {} bytes (triggered by {} limit)", 
                         currentSubBatch.size(), currentSubBatchSizeBytes,
        ...
        if (!currentSubBatch.isEmpty()) {
            ...
            log.trace("Final sub-batch: {} records, {} bytes", 
                     currentSubBatch.size(), currentSubBatchSizeBytes);
        }
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug("Sub-batch creation complete: {} sub-batches from {} messages, max {} records per sub-batch", 
                     subBatches.size(), totalMessages, maxRecordsInSubBatch);
        }
     ...
     * 
     * This method orchestrates the complete batch delivery process including
     * sub-batch creation, delivery execution, and metrics tracking. It handles
     * the complex workflow of delivering large message batches while respecting
     * Firehose constraints.
     * 
     * Processing Steps:
     * 1. Validate input messages and handle empty batches
     * 2. Create optimally-sized sub-batches using Firehose constraints
     * 3. Deliver each sub-batch to Firehose with error handling
     * 4. Track delivery success/failure for all sub-batches
     * 5. Update metrics based on overall batch success
     * 6. Log comprehensive delivery results
     * 
     * Error Handling:
     * - Empty or null batches are considered successful
     * - Individual sub-batch failures are logged and tracked
     * - Overall batch success requires all sub-batches to succeed
     * - Exceptions are caught and logged with appropriate metrics updates
     * 
     * Metrics Tracking:
     * - Successful batches counter
    ...
    public boolean deliverBatch(List<String> messages) {
        ...
        log.info("Processing batch of {} pre-compressed messages for Firehose delivery", messages.size());

        try {
            // Step 1: Create sub-batches with simple, efficient algorithm
            List<List<String>> subBatches = createSubBatches(messages, 
            ...
            if (allSuccessful) {
                ...
                log.info("Successfully delivered entire batch: {} messages in {} sub-batches", 
                        messages.size(), subBatches.size());
            } else {
     ...
     * 
     * This method handles the delivery of a single sub-batch to AWS Kinesis Data Firehose,
     * including record creation, batch request building, response processing, and intelligent
     * retry strategies based on exception classification. It implements buffer full handling,
     * rate limiting awareness, and network issue recovery.
     * 
     * Processing Steps:
     * 1. Convert pre-compressed messages to Firehose Record objects
     * 2. Filter out invalid or null records
     * 3. Build PutRecordBatchRequest with delivery stream configuration
     * 4. Attempt delivery with intelligent retry strategy
     * 5. Classify exceptions and apply appropriate retry logic
     * 6. Handle buffer full scenarios with graduated backoff
     * 7. Track retry metrics and recovery patterns
     * 8. Log detailed results for monitoring and debugging
     * 
     * Enhanced Error Handling:
     * - Exception classification (Buffer Full, Rate Limit, Network Issue, Generic)
     * - Graduated backoff for buffer full (5s → 15s → 45s → 2min → 5min)
     * - Standard retry for rate limiting and network issues
     * - Comprehensive retry metrics and tracking
     * 
     * Pre-compressed Message Handling:
     * - Messages are expected to be already compressed and encoded
     * - No additional compression is performed
     * - Direct conversion to Firehose Record format
     * 
     ...
     */
    private boolean deliverSubBatch(List<String> compressedSubBatch, int subBatchIndex, int totalSubBatches) {
        if (compressedSubBatch == null || compressedSubBatch.isEmpty()) {
            log.warn("Null or empty sub-batch {}/{}", subBatchIndex, totalSubBatches);
            return false;
        }

        log.debug("Processing pre-compressed sub-batch {}/{}: {} messages", 
                 subBatchIndex, totalSubBatches, compressedSubBatch.size());

        // Step 1: Convert pre-compressed messages directly to Firehose records
        List<Record> records = compressedSubBatch.stream()
                .map(this::createFirehoseRecordFromCompressed)
                .filter(record -> record != null) // Remove failed transformations
                .toList();

        if (records.isEmpty()) {
            log.warn("No valid records in pre-compressed sub-batch {}/{}", subBatchIndex, totalSubBatches);
            return false;
        }

        // Step 2: Attempt delivery with intelligent retry strategy
        return deliverSubBatchWithRetry(records, compressedSubBatch.size(), subBatchIndex, totalSubBatches);
    }

    /**
     * Deliver sub-batch with intelligent retry strategy based on exception classification.
     */
    private boolean deliverSubBatchWithRetry(List<Record> records, int originalMessageCount, 
                                           int subBatchIndex, int totalSubBatches) {
        int maxRetries = 0;
        ...
        for (int attempt = 0; attempt < 8; attempt++) {
            try {
                ...
                if (handleSuccessfulResponse(response, subBatchIndex, totalSubBatches, records.size(), lastExceptionType, attempt)) {
                    return true;
                }
                
                log.warn("Sub-batch {}/{} delivery failed without exception - not retrying", subBatchIndex, totalSubBatches);
                return false;

            } catch (Exception e) {
                FirehoseExceptionClassifier.ExceptionType exceptionType = exceptionClassifier.classifyException(e);
                ...
                if (shouldStopRetrying(exceptionType, attempt, maxRetries, subBatchIndex, totalSubBatches, originalMessageCount)) {
                    return false;
                }
    ...
    private boolean handleSuccessfulResponse(PutRecordBatchResponse response, int subBatchIndex, int totalSubBatches, 
                                           int recordCount, FirehoseExceptionClassifier.ExceptionType lastExceptionType, int attempt) {
        boolean success = handleFirehoseResponse(response, subBatchIndex, totalSubBatches, recordCount);
        if (success && lastExceptionType == FirehoseExceptionClassifier.ExceptionType.BUFFER_FULL && attempt > 0) {
            bufferFullRecoveries.incrementAndGet();
            log.info("Buffer full recovery successful for sub-batch {}/{} after {} attempts", 
                    subBatchIndex, totalSubBatches, attempt + 1);
        }
        return success;
    }

    private boolean shouldStopRetrying(FirehoseExceptionClassifier.ExceptionType exceptionType, int attempt, int maxRetries,
                                     int subBatchIndex, int totalSubBatches, int originalMessageCount) {
        log.warn("Sub-batch {}/{} delivery attempt {} failed with {}", 
                subBatchIndex, totalSubBatches, attempt + 1, exceptionType);

        if (!retryStrategy.shouldRetry(exceptionType, attempt) || attempt >= maxRetries - 1) {
            log.error("Sub-batch {}/{} delivery failed after {} attempts with {}", 
                     subBatchIndex, totalSubBatches, attempt + 1, exceptionType);
    ...
    private void trackRetryMetrics(FirehoseExceptionClassifier.ExceptionType exceptionType, int originalMessageCount) {
        if (exceptionType == FirehoseExceptionClassifier.ExceptionType.BUFFER_FULL) {
    ...
    private boolean waitForRetry(FirehoseExceptionClassifier.ExceptionType exceptionType, int attempt, int maxRetries,
                               int subBatchIndex, int totalSubBatches) {
        try {
            java.time.Duration retryDelay = retryStrategy.calculateRetryDelay(exceptionType, attempt);
            log.info("Retrying sub-batch {}/{} in {} for {} (attempt {} of {})", 
                    subBatchIndex, totalSubBatches, retryDelay, exceptionType, attempt + 2, maxRetries);
     ...
     */
    private int getMaxRetriesForExceptionType(FirehoseExceptionClassifier.ExceptionType exceptionType) {
     ...
     * 
     * This method converts pre-compressed and encoded message strings into
     * AWS Kinesis Data Firehose Record objects. It performs validation,
     * size checking, and proper byte conversion for Firehose delivery.
     * 
     * Processing Steps:
     * 1. Validate input message for null or empty content
     * 2. Convert compressed string to UTF-8 bytes
     * 3. Validate record size against Firehose limits (1MB)
     * 4. Update total bytes processed metrics
     * 5. Create Firehose Record with SdkBytes wrapper
     * 6. Log detailed information for monitoring
     * 
     * Validation Logic:
     * - Null or empty messages are rejected
     * - Size validation against MAX_RECORD_SIZE_BYTES (1MB)
     * - UTF-8 encoding validation
     * 
     * Metrics Integration:
     * - Updates totalBytesProcessed counter
     * - Tracks individual record sizes
     * - Provides trace-level logging for debugging
     * 
     * Pre-compressed Message Assumptions:
     * - Message is already compressed (GZIP)
     * - Message is already Base64 encoded
     * - No additional compression or encoding needed
     * 
    ...
    private Record createFirehoseRecordFromCompressed(String compressedMessage) {
        try {
            ...
            if (finalData.length > MAX_RECORD_SIZE_BYTES) {
                log.error("Pre-compressed message too large: {} bytes (limit: {} bytes)", 
                         finalData.length, MAX_RECORD_SIZE_BYTES);
            ...
            return Record.builder()
                    .data(SdkBytes.fromByteArray(finalData))
                    .build();
     ...
     * 
     * This method processes the response from AWS Kinesis Data Firehose
     * after a batch delivery attempt. It analyzes the response for failures,
     * logs detailed error information, and determines overall success status.
     * 
     * Processing Steps:
     * 1. Validate response object for null values
     * 2. Check failedPutCount for any delivery failures
     * 3. Log detailed failure information if any records failed
     * 4. Log success information for monitoring
     * 5. Return boolean success status
     * 
     * Failure Analysis:
     * - Checks failedPutCount from Firehose response
     * - Logs detailed error information for failed records
     * - Provides sub-batch context for debugging
     * - Handles null responses gracefully
     * 
     * Success Criteria:
     * - All records in the sub-batch must be delivered successfully
     * - failedPutCount must be 0 for success
     * - Response object must be valid
     * 
     * Logging Strategy:
     * - Error level for failures with detailed context
     * - Debug level for successful deliveries
     ...
     */
    private boolean handleFirehoseResponse(PutRecordBatchResponse response, int subBatchIndex, 
                                         int totalSubBatches, int recordCount) {
        if (response == null) {
        ...
        if (failedRecordCount > 0) {
            log.error("Sub-batch {}/{} had {} failed records out of {}", 
                     subBatchIndex, totalSubBatches, failedRecordCount, recordCount);
        ...
        log.debug("Sub-batch {}/{} delivered successfully: {} records", 
                 subBatchIndex, totalSubBatches, recordCount);
        return true;
    }
        ...
        if (log.isDebugEnabled()) {
            for (int i = 0; i < responses.size(); i++) {
                ...
                if (entry.errorCode() != null) {
                    log.debug("Failed record in sub-batch {}: index={}, error={}, message={}", 
                             subBatchIndex, i, entry.errorCode(), entry.errorMessage());
                }
            }
        }
    }
    ...
    /**
     * Get comprehensive metrics for monitoring and health checks.
     * Includes enhanced buffer management metrics for operational insights.
     */
    ...
    public boolean testConnectivity() {
        try {
            ...
            log.warn("Batch Firehose connectivity test failed for stream {}: {}", deliveryStreamName, e.getMessage());
            return false;
        }
    }

    /**
     * Enhanced metrics data class for batch Firehose operations.
     * Includes comprehensive buffer management and retry tracking.
     */
    @lombok.Builder
    @lombok.Data
