 ...
 *
 * <p>This class provides comprehensive SSL/TLS configuration capabilities for secure Kafka
 * communication. It handles certificate loading from multiple sources including classpath resources
 * and external file paths, with support for various keystore formats and automatic resource
 * resolution.
 *
 * <p>Key Functionality: - Keystore and truststore loading from classpath and external locations -
 * SSL context creation with proper certificate validation - Certificate validation and health
 * checking - SSL properties building for Kafka configuration - Resource location resolution and
 * normalization - Temporary file extraction for classpath resources
 *
 * <p>High-Level Processing Steps: 1. Load keystore/truststore from configured locations 2. Validate
 * certificate integrity and expiration 3. Create SSL context with proper key and trust managers 4.
 * Build SSL properties for Kafka client configuration 5. Handle resource resolution for different
 * location types 6. Extract classpath resources to temporary files when needed
 *
 * <p>Certificate Management: - Supports JKS, PKCS12, and other keystore formats - Handles both
 * client certificates (keystore) and server certificates (truststore) - Provides certificate
 * validation and health monitoring - Supports classpath and external file system locations
 *
 * <p>Security Features: - Secure certificate loading with proper exception handling - Certificate
 * validation and integrity checking - Temporary file cleanup for extracted resources -
...
public class SslConfiguration {
  ...
  /**
   * Creates a new SslConfiguration without ResourceLoader (for testing). Limited functionality for
   * resource loading.
   */
   ...
   *
   * <p>This method intelligently determines the source of the keystore (classpath or external file
   * system) and loads it accordingly. It provides comprehensive error handling and logging for
   * security auditing.
   *
   * <p>Processing Steps: 1. Extract keystore configuration parameters (location, password, type) 2.
   * Determine source type (classpath vs external file system) 3. Load keystore from appropriate
   * source 4. Validate keystore integrity and accessibility 5. Log loading results for security
   * auditing
   *
   * <p>Source Detection Logic: - Classpath resources: locations starting with "classpath:" -
   * External files: absolute or relative file system paths - Automatic fallback handling for
   * different location formats
   *
   * <p>Error Handling: - Comprehensive exception catching and logging - Detailed error messages for
   * troubleshooting - Runtime exception wrapping for consistent error handling
   *
   ...
   *
   * <p>This method intelligently determines the source of the truststore (classpath or external
   * file system) and loads it accordingly. It provides comprehensive error handling and logging for
   * security auditing.
   *
   * <p>Processing Steps: 1. Extract truststore configuration parameters (location, password, type)
   * 2. Determine source type (classpath vs external file system) 3. Load truststore from
   * appropriate source 4. Validate truststore integrity and accessibility 5. Log loading results
   ...
   *
   * <p>This method loads a keystore from an external file system location, providing direct file
   * access for production deployments where certificates are stored outside the application
   * classpath.
   *
   * <p>Processing Steps: 1. Validate resource loader availability 2. Resolve keystore location to
   * absolute path 3. Load keystore from file system 4. Validate keystore format and password 5.
   * Return loaded keystore for SSL context creation
   *
   * <p>File System Handling: - Supports absolute and relative file paths - Handles different
   * keystore formats (JKS, PKCS12) - Provides detailed logging for security auditing - Validates
   * file existence and accessibility
   *
   * <p>Security Considerations: - Password validation during keystore loading - File system
   * permission checking - Comprehensive error handling for security issues
   *
   ...
   *
   * <p>This method loads a truststore from an external file system location, providing direct file
   * access for production deployments where certificates are stored outside the application
   * classpath.
   *
   * <p>Processing Steps: 1. Validate resource loader availability 2. Resolve truststore location to
   * absolute path 3. Load truststore from file system 4. Validate truststore format and password 5.
   * Return loaded truststore for SSL context creation
   *
   * <p>File System Handling: - Supports absolute and relative file paths - Handles different
   * truststore formats (JKS, PKCS12) - Provides detailed logging for security auditing - Validates
   * file existence and accessibility
   *
   * <p>Security Considerations: - Password validation during truststore loading - File system
   * permission checking - Comprehensive error handling for security issues
   *
   ...
   *
   * <p>This method initializes an SSLContext with the necessary key and trust managers based on the
   * provided KafkaProperties. It ensures that both keystore and truststore are loaded and validated
   * before creating the SSL context.
   *
   * <p>Processing Steps: 1. Load keystore and truststore from configuration 2. Initialize
   * KeyManagerFactory and TrustManagerFactory 3. Create SSLContext with proper key and trust
   * managers 4. Return initialized SSLContext
   *
   * <p>Security Features: - Secure keystore and truststore loading - Proper key and trust manager
   * initialization - Nullable trust manager for client-only authentication - Comprehensive error
   * handling for SSL context creation
   *
   ...
   *
   * <p>This method performs a basic validation of the keystore and truststore to ensure they are
   * loaded and accessible. It checks for certificate integrity, expiration, and proper format.
   *
   * <p>Processing Steps: 1. Load keystore and truststore from configuration 2. Validate keystore
   * integrity and accessibility 3. Validate truststore integrity and accessibility 4. Log
   * validation results
   *
   * <p>Security Features: - Secure keystore and truststore loading - Certificate integrity checking
   * - Expiration date validation - Comprehensive error handling for validation failures
   *
   * @param kafkaProperties Kafka configuration properties containing SSL settings
  ...
  public void validateCertificates(KafkaProperties kafkaProperties) {
    ...
    if (!sslConfig.isEnabled()) {
      throw new IllegalStateException(
          "SSL is not enabled but certificate validation was requested");
    }
   ...
   *
   * <p>This method constructs a map of SSL properties that can be passed to a Kafka consumer's
   * configuration. It includes all necessary properties for SSL/TLS connectivity, including
   * keystore and truststore locations, passwords, and types.
   *
   * <p>Processing Steps: 1. Extract SSL configuration from Kafka properties 2. Determine if SSL is
   * enabled 3. Build SSL properties map 4. Resolve resource locations for keystore and truststore
  ...
  public Map<String, Object> buildSslProperties(KafkaProperties kafkaProperties) {
    ...
    sslProps.put(
        "ssl.keystore.location", resolveKeystoreLocation(sslConfig.getKeystore().getLocation()));
    ...
    sslProps.put(
        ...
        resolveTruststoreLocation(sslConfig.getTruststore().getLocation()));
    ...
    log.debug(
        "SSL properties configured: security.protocol={}, keystore.type={}, truststore.type={}",
        sslConfig.getProtocol(),
        sslConfig.getKeystore().getType(),
        sslConfig.getTruststore().getType());
   ...
   *
   * <p>This method attempts to find a valid certificate location by checking external file system
   * paths first, then classpath resources. It provides detailed logging to help in debugging
   * resource resolution issues.
   *
   * <p>Processing Steps: 1. Check if external location is provided and accessible 2. If not, check
   * if classpath location is provided and accessible 3. If neither, throw an exception 4. Log the
   * chosen location
   *
   * @param classpathLocation Location of the certificate in classpath (e.g.,
   *     "classpath:com/example/cert.p12")
  ...
  /**
   * Resolves a resource location using Spring's ResourceLoader abstraction. This method is agnostic
   * to whether the location is a classpath resource, file system path, or URL.
   *
   * <p>Supported prefixes: - classpath: for classpath resources - file: for file system resources -
   * http:/https: for URL resources - No prefix: treated as file system path (relative to working
   * directory)
   *
   * <p>Processing Steps: 1. Normalize the input location to ensure it has a proper Spring Resource
   * prefix 2. Attempt to load the resource using the ResourceLoader 3. If resource not found, throw
   * an exception 4. If resource is found, try to get the file directly 5. If file access fails,
   * extract the resource to a temporary file 6. Return the resolved absolute path
   *
   * @param location The original location of the resource (e.g., "classpath:com/example/cert.p12",
   *     "file:/path/to/cert.p12", "https://example.com/cert.p12")
  ...
  private String resolveResourceLocation(String location, String resourceType) {
    try {
        ...
        if (location.startsWith("classpath:")) {
          ...
          resource =
              new org.springframework.core.io.DefaultResourceLoader()
                  .getResource(normalizedLocation);
        }
      }

      if (!resource.exists()) {
        throw new RuntimeException(
            String.format("%s resource not found: %s", resourceType, location));
      }
      ...
      try {
        ...
        log.debug(
            "Resource {} is not accessible as file, extracting to temporary location", location);
      ...
      throw new RuntimeException(
          String.format("Failed to resolve %s location: %s", resourceType, location), e);
    }
  }

  /**
   * Normalizes a resource location to ensure proper Spring Resource prefixes. This ensures that
   * file system paths have the 'file:' prefix for proper resolution.
   *
   * <p>Processing Steps: 1. Check if location already has a Spring Resource prefix 2. If not, add
   * 'file:' prefix for file system paths 3. Return the normalized location
   *
   * @param location The original location (e.g., "com/example/cert.p12", "/path/to/cert.p12",
   *     "classpath:com/example/cert.p12")
   * @return Location with a proper Spring Resource prefix (e.g., "file:/path/to/cert.p12",
   *     "classpath:com/example/cert.p12")
   */
  ...
  /**
   * Extracts a Spring Resource to a temporary file. This handles resources that cannot be accessed
   * as direct files (e.g., inside JARs).
   *
   * <p>Processing Steps: 1. Create a temporary file 2. Copy the resource content to the temporary
   * file 3. Set up deletion on JVM exit 4. Log the extraction 5. Return the path to the temporary
   * file
   *
  ...
  private Path extractResourceToTempFile(ClassPathResource resource, String prefix, String suffix) {
    try {
      ...
      log.debug(
          "Extracted classpath resource {} to temporary file: {}", resource.getPath(), tempFile);
      return tempFile;

    } catch (IOException e) {
      log.error(
          "Failed to extract classpath resource to temporary file: {}", resource.getPath(), e);
