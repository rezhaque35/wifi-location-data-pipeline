 ...
 *
 * <p>This configuration class sets up the Kafka consumer infrastructure with advanced features for
 * production-ready message consumption. It provides both standard and batch-optimized consumer
 * configurations with comprehensive SSL/TLS support.
 *
 * <p>Key Functionality: - Configures consumer factory with SSL/TLS support - Sets up optimized
 * listener container factories - Implements batch processing capabilities - Provides manual
 * acknowledgment for message processing control - Configures optimized polling parameters for high
 * throughput - Handles SSL certificate validation and trust store management
 *
 * <p>High-Level Steps: 1. Build base consumer properties from configuration 2. Merge SSL/TLS
 * properties when SSL is enabled 3. Create consumer factory with merged properties 4. Configure
 * listener container factory with optimized settings 5. Set up batch processing container for
 * high-volume consumption 6. Configure error handling and recovery mechanisms
 *
 * <p>The configuration supports both standard and batch message processing modes, with the batch
 * mode optimized for processing large volumes of WiFi scan data efficiently while maintaining
...
public class KafkaConsumerConfiguration {
   ...
   *
   * <p>This bean method creates the primary consumer factory used by the application. It
   * automatically integrates SSL/TLS configuration when enabled and provides the foundation for all
   ...
   *
   * <p>This method builds a complete consumer factory by combining base Kafka properties with
   * SSL/TLS configuration when enabled. It ensures proper serialization and security settings for
   * production message consumption.
   *
   * <p>High-Level Steps: 1. Build base consumer properties from Kafka configuration 2. Check if SSL
   * is enabled in configuration 3. Merge SSL properties with base properties if SSL is enabled 4.
   * Create and return DefaultKafkaConsumerFactory with merged properties 5. Log configuration
  ...
  public ConsumerFactory<String, String> consumerFactory(
    ...
    if (kafkaProperties.getSsl().isEnabled()) {
      ...
      Map<String, Object> mergedProps =
          mergeConsumerProperties(consumerProps, kafkaProperties, sslConfiguration);
   ...
   *
   * <p>This bean method creates the primary listener container factory used for standard message
   * processing. It configures the container with optimized settings for reliable message
   * consumption.
   *
   ...
   *
   * <p>This method configures a listener container factory with production-optimized settings for
   * high-performance message consumption. It sets up manual acknowledgment, optimized polling
   * parameters, and error handling mechanisms.
   *
   * <p>High-Level Steps: 1. Create ConcurrentKafkaListenerContainerFactory instance 2. Set consumer
   * factory for message deserialization 3. Configure container properties for manual acknowledgment
   * 4. Set optimized polling timeout and idle intervals 5. Configure error handling and recovery
   * settings 6. Enable synchronous commits for reliability
   *
   ...
   *
   * <p>This method creates the foundational properties map for Kafka consumer configuration. It
   * validates required properties and sets up essential consumer settings for reliable message
   * consumption.
   *
   * <p>High-Level Steps: 1. Validate bootstrap servers configuration 2. Create properties map with
   * required consumer settings 3. Configure deserializers for String key-value pairs 4. Set
   * consumer group ID and auto-offset reset behavior 5. Configure session timeout and heartbeat
   * intervals 6. Set fetch and max poll settings for optimal performance
   *
  ...
  public Map<String, Object> buildConsumerProperties(KafkaProperties kafkaProperties) {
    ...
    if (kafkaProperties.getConsumer().getGroupId() == null
        || kafkaProperties.getConsumer().getGroupId().isEmpty()) {
    ...
    props.put(
        ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false); // Disabled for manual acknowledgment
    ...
    if (kafkaProperties.getConsumer().getSessionTimeout() != null) {
      props.put(
          ...
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getSessionTimeout()));
    }

    if (kafkaProperties.getConsumer().getHeartbeatInterval() != null) {
      props.put(
          ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getHeartbeatInterval()));
    }

    if (kafkaProperties.getConsumer().getAutoCommitInterval() != null) {
      props.put(
          ConsumerConfig.AUTO_COMMIT_INTERVAL_MS_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getAutoCommitInterval()));
    }
    ...
    if (kafkaProperties.getConsumer().getMaxPollInterval() != null) {
      props.put(
          ...
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getMaxPollInterval()));
    }

    props.put(
        ConsumerConfig.FETCH_MIN_BYTES_CONFIG, kafkaProperties.getConsumer().getFetchMinBytes());

    if (kafkaProperties.getConsumer().getFetchMaxWait() != null) {
      props.put(
          ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getFetchMaxWait()));
    }

    if (kafkaProperties.getConsumer().getRequestTimeout() != null) {
      props.put(
          ConsumerConfig.REQUEST_TIMEOUT_MS_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getRequestTimeout()));
    }

    if (kafkaProperties.getConsumer().getRetryBackoff() != null) {
      props.put(
          ConsumerConfig.RETRY_BACKOFF_MS_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getRetryBackoff()));
    }

    if (kafkaProperties.getConsumer().getConnectionsMaxIdle() != null) {
      props.put(
          ConsumerConfig.CONNECTIONS_MAX_IDLE_MS_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getConnectionsMaxIdle()));
    }

    if (kafkaProperties.getConsumer().getMetadataMaxAge() != null) {
      props.put(
          ConsumerConfig.METADATA_MAX_AGE_CONFIG,
          parseTimeoutMilliseconds(kafkaProperties.getConsumer().getMetadataMaxAge()));
    }
    ...
    log.debug(
        "Consumer properties built: bootstrap.servers={}, group.id={}, auto.offset.reset={}, max.poll.interval.ms={}",
  ...
  public Map<String, Object> mergeConsumerProperties(
  ...
  public int parseTimeoutMilliseconds(String timeoutString) {
    ...
    try {
      ...
      throw new NumberFormatException(
          "Invalid timeout format: " + timeoutString + ". Expected format: '30000ms' or '30s'");
    }
  }

  /**
   * Creates batch Kafka listener container factory for processing message batches. Configured for
   * 150-message batches with manual acknowledgment.
   */
  @Bean
  public ConcurrentKafkaListenerContainerFactory<String, String>
    ...
    log.info(
        "Batch Kafka listener container factory configured: batchListener=true, ackMode=MANUAL, pollTimeout=1500ms");
