...
/**
 * Health indicator for Kafka consumer group status and partition assignments. Checks if the
 * consumer is properly registered with the consumer group and has partition assignments.
 */
@Slf4j
  ...
  public Health health() {
    try {
      ...
      if (!isConsumerConnected) {
        healthBuilder =
            Health.down().withDetail("reason", "Consumer is not connected to Kafka cluster");
      } else if (!isConsumerGroupActive) {
        healthBuilder =
            Health.down().withDetail("reason", "Consumer group is not active in Kafka cluster");
      }
