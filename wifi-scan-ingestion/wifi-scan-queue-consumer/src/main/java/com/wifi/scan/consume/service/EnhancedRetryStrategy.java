 ...
 *
 * <p>This service provides exception-specific retry logic with graduated backoff strategies
 * optimized for different failure scenarios. It implements sophisticated retry timing that aligns
 * with Firehose behavior patterns and system recovery characteristics.
 *
 * <p>Retry Strategies by Exception Type: - BUFFER_FULL: Graduated backoff (5s → 15s → 45s → 2min →
 * 5min) over ~15 minutes - RATE_LIMIT: Standard exponential backoff (1s → 2s → 4s → 8s → 16s → 30s)
 * - NETWORK_ISSUE: Quick retries (1s → 2s → 4s) with limited attempts (3-5) - GENERIC_FAILURE:
 * Standard backoff with moderate attempts (5 max)
 *
 * <p>Key Features: - Exception-specific retry logic and timing - Jitter to prevent thundering herd
 * problems - Comprehensive retry attempt tracking and metrics - Configurable retry limits and
 * timing parameters - Integration with Firehose buffer flush intervals (5-minute alignment)
 *
...
public class EnhancedRetryStrategy {
  ...
  public EnhancedRetryStrategy(
      ...
      @Value("${app.firehose.retry.generic-failure.base-delay:PT2S}")
          Duration genericFailureBaseDelay) {
    ...
    log.info(
        "EnhancedRetryStrategy initialized: bufferFull={} retries, rateLimit={} retries, "
            + "network={} retries, generic={} retries",
        bufferFullMaxRetries,
        rateLimitMaxRetries,
        networkIssueMaxRetries,
        genericFailureMaxRetries);
  }
  ...
  public boolean shouldRetry(
      FirehoseExceptionClassifier.ExceptionType exceptionType, int currentAttempt) {
    ...
    log.debug(
        "Retry decision for {}: attempt {} of {}, shouldRetry={}",
        exceptionType,
        currentAttempt + 1,
        maxRetries,
        shouldRetry);
   ...
   *
   * <p>This method implements exception-specific retry timing strategies: - Buffer Full: Graduated
   * backoff aligned with Firehose 5-minute buffer flush - Rate Limit: Standard exponential backoff
   * - Network Issue: Quick retries for transient issues - Generic Failure: Standard backoff with
   * jitter
   *
   * @param exceptionType The type of exception that occurred
  ...
  public Duration calculateRetryDelay(
      FirehoseExceptionClassifier.ExceptionType exceptionType, int attemptNumber) {
    ...
    log.debug(
        "Calculated retry delay for {} attempt {}: {} (with jitter: {})",
        exceptionType,
        attemptNumber + 1,
        calculatedDelay,
        jitteredDelay);
   ...
   *
   * <p>This strategy implements increasing delays that align with Firehose's 5-minute buffer flush
   * interval, providing optimal recovery timing for buffer capacity issues.
   *
   * <p>Delay Schedule: 5s → 15s → 45s → 2min → 5min → 5min → 5min
   *
   * @param attemptNumber Current retry attempt number (0-based)
  ...
  private Duration calculateBufferFullDelay(int attemptNumber) {
    switch (attemptNumber) {
      ...
      case 1:
        return Duration.ofSeconds(15); // 15 seconds
      case 2:
        return Duration.ofSeconds(45); // 45 seconds
      case 3:
        return Duration.ofMinutes(2); // 2 minutes
      default:
        return Duration.ofMinutes(5); // 5 minutes (aligned with buffer flush)
    }
  }
   ...
   *
   * <p>Network issues are often transient, so this strategy uses shorter delays to quickly recover
   * from temporary connectivity problems.
   *
   * <p>Delay Schedule: 1s → 2s → 4s
   *
   * @param attemptNumber Current retry attempt number (0-based)
  ...
  private Duration calculateNetworkIssueDelay(int attemptNumber) {
    switch (attemptNumber) {
      case 0:
        return Duration.ofSeconds(1); // 1 second
      case 1:
        return Duration.ofSeconds(2); // 2 seconds
      default:
        return Duration.ofSeconds(4); // 4 seconds
    }
  }
  ...
  private int getMaxRetries(FirehoseExceptionClassifier.ExceptionType exceptionType) {
    switch (exceptionType) {
      case BUFFER_FULL:
        return bufferFullMaxRetries;
      case RATE_LIMIT:
        return rateLimitMaxRetries;
      case NETWORK_ISSUE:
        return networkIssueMaxRetries;
      case GENERIC_FAILURE:
        return genericFailureMaxRetries;
      default:
        return genericFailureMaxRetries;
    }
  }
  ...
  private Duration getBaseDelay(FirehoseExceptionClassifier.ExceptionType exceptionType) {
    switch (exceptionType) {
      case BUFFER_FULL:
        return bufferFullBaseDelay;
      case RATE_LIMIT:
        return rateLimitBaseDelay;
      case NETWORK_ISSUE:
        return networkIssueBaseDelay;
      case GENERIC_FAILURE:
        return genericFailureBaseDelay;
      default:
        return genericFailureBaseDelay;
    }
  }
  ...
  private void incrementRetryCounter(FirehoseExceptionClassifier.ExceptionType exceptionType) {
    switch (exceptionType) {
      case BUFFER_FULL:
        ...
        break;
      case RATE_LIMIT:
        rateLimitRetries.incrementAndGet();
        break;
      case NETWORK_ISSUE:
        networkIssueRetries.incrementAndGet();
        break;
      case GENERIC_FAILURE:
        genericFailureRetries.incrementAndGet();
        break;
    }
  }
  ...
  public static class RetryMetrics {
    ...
    public RetryMetrics(
        ...
        long failedRetries,
        long bufferFullRetries,
        long rateLimitRetries,
        long networkIssueRetries,
    ...
    public long getBufferFullRetries() {
      return bufferFullRetries;
    }

    public long getRateLimitRetries() {
      return rateLimitRetries;
    }

    public long getNetworkIssueRetries() {
      return networkIssueRetries;
    }

    public long getGenericFailureRetries() {
      return genericFailureRetries;
    }
    ...
    public String toString() {
      return String.format(
          "RetryMetrics{total=%d, successful=%d, failed=%d, "
