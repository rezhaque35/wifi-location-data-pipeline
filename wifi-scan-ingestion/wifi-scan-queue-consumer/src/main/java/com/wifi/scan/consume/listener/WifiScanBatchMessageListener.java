...
public class WifiScanBatchMessageListener {
  ...
  @Autowired private BatchFirehoseMessageService batchFirehoseService;

  @Autowired private KafkaConsumerMetrics metrics;

  @Autowired private KafkaMonitoringService monitoringService;

  @Autowired private MessageCompressionService compressionService;

  // Performance metrics
  @Getter private final AtomicLong processedBatchCount = new AtomicLong(0);

  @Getter private final AtomicLong processedMessageCount = new AtomicLong(0);

  @Getter private volatile long lastBatchProcessingTimeMs = 0;
  ...
  public void listenBatch(ConsumerRecords<String, String> records, Acknowledgment acknowledgment) {
    ...
    try {
      ...
      List<String> compressedMessages =
          compressionService.compressAndEncodeBatch(validOriginalMessages);
      ...
      if (deliverySuccess) {
        ...
        log.info(
            "Successfully processed batch: {} compressed messages delivered to Firehose",
            compressedMessages.size());
      } else {
        throw new RuntimeException(
            "Failed to deliver compressed batch to Firehose after processing");
      }
   ...
   */
  private void handleBatchProcessingException(
