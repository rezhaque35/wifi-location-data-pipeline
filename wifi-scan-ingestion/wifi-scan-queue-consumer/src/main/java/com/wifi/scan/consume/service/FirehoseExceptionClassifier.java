 ...
 *
 * <p>This service implements intelligent exception classification to enable appropriate retry
 * strategies and error handling for different types of Firehose failures. It distinguishes between
 * buffer full scenarios, rate limiting, network issues, and generic failures to enable targeted
 * response strategies.
 *
 * <p>Exception Classifications: - BUFFER_FULL: Firehose internal buffer is at capacity (123MB+) -
 * RATE_LIMIT: Firehose rate limiting exceeded (5MB/sec or other limits) - NETWORK_ISSUE: Network
 * connectivity or timeout problems - GENERIC_FAILURE: Other failures requiring standard error
 * handling
 *
 * <p>Key Features: - AWS SDK exception analysis with error code parsing - Exception pattern
 * recognition and classification - Metrics tracking for different exception types - Comprehensive
...
public class FirehoseExceptionClassifier {
   ...
   *
   * <p>This method analyzes AWS SDK exceptions to determine the most appropriate classification
   * based on error codes, exception types, and message content. The classification enables targeted
   * retry strategies and error handling.
   *
   * <p>Classification Logic: 1. Buffer Full: ServiceUnavailableException with buffer-related
   * indicators 2. Rate Limit: Throttling exceptions and rate limit error codes 3. Network Issue:
   * Connection timeouts, DNS issues, network failures 4. Generic Failure: All other exceptions
   * requiring standard handling
   *
  ...
  public ExceptionType classifyException(Exception exception) {
    ...
    try {
      ...
      log.warn(
          "Error during exception classification, defaulting to GENERIC_FAILURE: {}",
          e.getMessage());
      genericFailureExceptions.incrementAndGet();
      return ExceptionType.GENERIC_FAILURE;
    }
   ...
   *
   * <p>Buffer full scenarios occur when the Firehose internal buffer (typically 128MB) is at
   * capacity and cannot accept additional data. This is indicated by specific
   * ServiceUnavailableException patterns and error messages.
   *
   * <p>Detection Patterns: - ServiceUnavailableException from Firehose - Error messages containing
   * "buffer", "capacity", "full" - Specific AWS error codes indicating buffer issues
   *
  ...
  private boolean isBufferFullException(Exception exception) {
    ...
    if (exception instanceof FirehoseException) {
      ...
      String errorCode =
          firehoseEx.awsErrorDetails() != null ? firehoseEx.awsErrorDetails().errorCode() : "";
   ...
   *
   * <p>Rate limiting occurs when throughput exceeds Firehose limits (5MB/sec, etc.). This is
   * typically indicated by throttling exceptions and specific error codes.
   *
   * <p>Detection Patterns: - Throttling-related error codes - Rate limit error messages - HTTP 429
   * status codes - Specific AWS throttling exceptions
   *
   * @param exception The exception to analyze
   ...
   *
   * <p>Network issues include connection timeouts, DNS resolution failures, and other network-level
   * problems that may be temporary.
   *
   * <p>Detection Patterns: - Connection timeout exceptions - DNS resolution failures - Network
   * unreachable errors - Socket exceptions
   *
   * @param exception The exception to analyze
  ...
  /** Resets all exception metrics counters. Used for testing and periodic metrics reset. */
  ...
  public static class ExceptionMetrics {
    ...
    public ExceptionMetrics(
        ...
        long rateLimitCount,
        long networkIssueCount,
        long genericFailureCount) {
    ...
    public long getNetworkIssueCount() {
      return networkIssueCount;
    }

    public long getGenericFailureCount() {
    ...
    public String toString() {
      return String.format(
          "ExceptionMetrics{bufferFull=%d, rateLimit=%d, network=%d, generic=%d, total=%d}",
