 ...
 * 
 * This controller provides comprehensive REST endpoints for monitoring the Kafka
 * consumer application's performance, health, and operational status. It exposes
 * detailed metrics suitable for integration with monitoring systems like Prometheus
 * and Grafana, as well as operational dashboards.
 * 
 * Key Functionality:
 * - Exposes detailed Kafka consumer metrics via REST API
 * - Provides performance monitoring data (processing times, success rates)
 * - Reports consumer state and connectivity information
 * - Offers memory usage and system resource metrics
 * - Supports metrics reset for testing and monitoring periods
 * - Delivers operational status and health information
 * - Provides both JSON and plain text metric formats
 * 
 * High-Level Steps:
 * 1. Collect metrics from Kafka monitoring service
 * 2. Aggregate performance and operational data
 * 3. Format metrics for different consumption patterns (JSON, text)
 * 4. Provide operational status and health indicators
 * 5. Support metrics reset for testing scenarios
 * 6. Include metadata and version information
 * 
 * The controller is separate from Spring Boot Actuator health endpoints and
 * focuses on operational metrics rather than basic health status. It provides
 * the detailed information needed for production monitoring and alerting.
 * 
...
public class MetricsController {
     ...
     * 
     * This endpoint provides comprehensive metrics data in JSON format suitable
     * for monitoring systems like Prometheus and Grafana. It includes performance
     * metrics, operational status, and system resource utilization.
     * 
     * High-Level Steps:
     * 1. Collect message processing metrics (consumed, processed, failed)
     * 2. Gather performance metrics (processing times, success rates)
     * 3. Retrieve activity metrics (timestamps, polling status)
     * 4. Collect consumer state information (connectivity, group status)
     * 5. Gather system resource metrics (memory usage)
     * 6. Calculate consumption rate and health indicators
     * 7. Add metadata and version information
     * 8. Return comprehensive metrics in JSON format
     * 
     * The metrics include both real-time operational data and historical
     * performance indicators for comprehensive system monitoring.
     * 
     ...
     * 
     * This endpoint provides a human-readable summary of key metrics in plain
     * text format, useful for quick debugging, log analysis, and simple
     * monitoring dashboards that don't require JSON parsing.
     * 
     * High-Level Steps:
     * 1. Request metrics summary from monitoring service
     * 2. Format response as plain text with appropriate headers
     * 3. Return human-readable metrics summary
     * 
     * The summary includes key performance indicators and operational status
     * in a format that's easy to read and parse for quick operational checks.
     * 
     ...
     * 
     * This endpoint allows resetting all collected metrics, useful for testing
     * scenarios, starting fresh monitoring periods, or clearing historical
     * data when needed for operational purposes.
     * 
     * High-Level Steps:
     * 1. Reset all metrics in the monitoring service
     * 2. Log the reset operation for audit purposes
     * 3. Return confirmation of successful reset
     * 
     * Use this endpoint carefully as it will clear all historical metrics
     * data and start fresh monitoring from the reset point.
     * 
     ...
     * 
     * This endpoint provides a focused view of the operational status,
     * including connectivity, health indicators, and key performance
     * metrics for quick operational assessment.
     * 
     * High-Level Steps:
     * 1. Collect operational status from monitoring service
     * 2. Gather key health and performance indicators
     * 3. Include connectivity and consumer state information
     * 4. Add operational metadata and timestamps
     * 5. Return focused operational status data
     * 
     * This endpoint is designed for operational dashboards and quick
     * status checks without the full metrics detail.
     * 
     ...
     * 
     * This endpoint provides a REST API for accepting individual WiFi scan messages
     * and processing them through the compression and Firehose delivery pipeline.
     * It converts the single message to a list format expected by the batch services
     * and orchestrates the complete delivery process.
     * 
     * High-Level Steps:
     * 1. Accept WiFi scan message as JSON in request body
     * 2. Convert message to JSON string format
     * 3. Create single-element list for batch processing
     * 4. Compress and encode the message using MessageCompressionService
     * 5. Deliver compressed message to Firehose using BatchFirehoseMessageService
     * 6. Return delivery status and metrics
     * 
    ...
    public ResponseEntity<Map<String, Object>> processWifiScanMessage(@RequestBody Object wifiScanMessage) {
        ...
        try {
            ...
            response.put("message", deliverySuccess ? 
            ...
            response.put("compressionRatio", calculateCompressionRatio(messageJson.length(), compressedMessages.get(0).length()));
            response.put("timestamp", System.currentTimeMillis());
