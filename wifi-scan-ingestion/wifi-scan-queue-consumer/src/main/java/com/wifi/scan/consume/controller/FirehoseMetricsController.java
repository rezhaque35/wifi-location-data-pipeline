 ...
 *
 * <p>This controller provides comprehensive REST endpoints for monitoring AWS Kinesis Data Firehose
 * batch delivery performance, connectivity status, and operational health. It serves as the primary
 * interface for external monitoring systems to track Firehose delivery metrics.
 *
 * <p>Key Functionality: - Real-time batch delivery metrics retrieval - Connectivity testing and
 * status reporting - Metrics summary for quick health assessment - Metrics reset for testing and
 * monitoring purposes - Health status determination based on success rates
 *
 * <p>High-Level Processing Steps: 1. Receive HTTP requests for metrics or connectivity testing 2.
 * Retrieve current metrics from BatchFirehoseMessageService 3. Transform raw metrics into
 * appropriate response formats 4. Apply business logic for health determination 5. Return
 * structured JSON responses with appropriate HTTP status codes
 *
 * <p>Monitoring Integration: - Compatible with Prometheus, Grafana, and other monitoring tools -
 * Provides both detailed and summary metrics endpoints - Supports automated health checks and
 * alerting
 *
...
public class FirehoseMetricsController {
   ...
   *
   * <p>This endpoint provides complete metrics including batch delivery counts, success rates,
   * bytes processed, and detailed performance statistics. Suitable for detailed monitoring
   * dashboards and performance analysis.
   *
   * <p>Processing Steps: 1. Retrieve current metrics from BatchFirehoseMessageService 2. Log debug
   * information for monitoring purposes 3. Return complete metrics object with HTTP 200 status 4.
  ...
  public ResponseEntity<BatchFirehoseMetrics> getFirehoseMetrics() {
    try {
      ...
      log.debug(
          "Batch Firehose metrics requested: {} successful, {} failed batches",
          metrics.getSuccessfulBatches(),
          metrics.getFailedBatches());
   ...
   *
   * <p>This endpoint provides a simplified metrics summary including key performance indicators and
   * health status determination. Suitable for quick health checks and automated monitoring systems.
   *
   * <p>Processing Steps: 1. Retrieve detailed metrics from the service 2. Calculate summary
   * statistics and health indicators 3. Determine health status based on success rate threshold
   * (95%) 4. Build and return summary response 5. Handle exceptions with appropriate error
   * responses
   *
   * <p>Health Determination Logic: - Healthy: Batch success rate > 95% - Unhealthy: Batch success
   * rate ≤ 95%
   *
  ...
  public ResponseEntity<FirehoseMetricsSummary> getFirehoseMetricsSummary() {
    try {
      ...
      FirehoseMetricsSummary summary =
          FirehoseMetricsSummary.builder()
              ...
              .isHealthy(
                  metrics.getBatchSuccessRate()
                      > 0.95) // Consider healthy if >95% batch success rate
              .build();
   ...
   *
   * <p>This endpoint allows external systems to reset accumulated metrics, useful for testing
   * scenarios, monitoring resets, and performance baselines.
   *
   * <p>Processing Steps: 1. Call service method to reset all accumulated metrics 2. Log the reset
   * operation for audit purposes 3. Return confirmation message with HTTP 200 status 4. Handle
   * exceptions with appropriate error responses
   *
   * <p>Use Cases: - Testing and development environments - Monitoring system resets - Performance
   * baseline establishment - Troubleshooting scenarios
   *
   ...
   *
   * <p>This endpoint performs an active connectivity test to AWS Kinesis Data Firehose and returns
   * detailed status information including connection status, delivery stream information, and test
   * timestamp.
   *
   * <p>Processing Steps: 1. Perform connectivity test through the service 2. Capture current
   * timestamp for status tracking 3. Build connectivity status response with test results 4. Return
   * structured status information 5. Handle exceptions with appropriate error responses
   *
   * <p>Connectivity Test Information: - Connection status (connected/disconnected) - Delivery
   * stream name and configuration - Last test timestamp for monitoring - Status message with
   * additional details
   *
  ...
  public ResponseEntity<FirehoseConnectivityStatus> testFirehoseConnectivity() {
    try {
      ...
      FirehoseConnectivityStatus status =
          FirehoseConnectivityStatus.builder()
              .connected(isConnected)
              .deliveryStreamName(metrics.getDeliveryStreamName())
              .lastTestTime(System.currentTimeMillis())
              .message(
                  isConnected
                      ? "Successfully connected to Firehose"
      ...
      FirehoseConnectivityStatus errorStatus =
          FirehoseConnectivityStatus.builder()
              .connected(false)
   ...
   *
   * <p>This class provides a simplified view of Firehose metrics including key performance
   * indicators and health status determination.
   *
   * <p>Key Features: - Essential metrics for health monitoring - Calculated success rates and
   * averages - Boolean health status for automated systems - Delivery stream identification
   *
   * <p>High-Level Data Structure: 1. Basic identification (delivery stream name) 2. Batch delivery
   * statistics (total, successful, failed) 3. Performance metrics (success rate, messages, bytes)
   ...
   *
   * <p>This class provides detailed connectivity information including connection status, delivery
   * stream details, and test metadata.
   *
   * <p>Key Features: - Boolean connection status - Delivery stream identification - Test timestamp
   * for monitoring - Descriptive status message
   *
   * <p>High-Level Data Structure: 1. Connection status (connected/disconnected) 2. Delivery stream
   * information 3. Test metadata (timestamp, message)
   */
  @lombok.Builder
  @lombok.Data
