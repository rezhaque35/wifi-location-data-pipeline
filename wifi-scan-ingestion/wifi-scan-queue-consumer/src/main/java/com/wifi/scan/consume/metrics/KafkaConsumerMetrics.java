 ...
 *
 * <p>This component provides comprehensive metrics tracking for Kafka consumer operations,
 * including both individual message processing and batch processing statistics. It uses thread-safe
 * atomic counters and volatile variables to ensure accurate metrics collection in multi-threaded
 * environments.
 *
 * <p>Key Functionality: - Tracks individual message consumption, processing, and failure rates -
 * Monitors batch processing performance and success rates - Records processing time statistics
 * (min, max, average) - Maintains timestamp tracking for first and last message processing -
 * Provides rate limiting and partial processing metrics - Offers comprehensive metrics summary
 * generation - Supports metrics reset for testing and monitoring periods
 *
 * <p>High-Level Steps: 1. Record message consumption events with timestamps 2. Track message
 * processing success/failure with timing 3. Monitor batch processing performance and outcomes 4.
 * Calculate success rates and performance statistics 5. Maintain processing time min/max/average
 * calculations 6. Generate comprehensive metrics summaries 7. Support metrics reset for operational
 * purposes
 *
 * <p>The component is designed for production monitoring and provides the detailed metrics needed
 * for performance analysis, alerting, and operational dashboards.
 *
...
public class KafkaConsumerMetrics {
   ...
   *
   * <p>This method increments the total messages consumed counter and updates the timestamp
   * tracking for operational monitoring. It's called when a message is successfully received from
   * Kafka.
   *
   * <p>High-Level Steps: 1. Increment total messages consumed counter atomically 2. Update
   * timestamp tracking for operational monitoring 3. Log debug information for monitoring and
   * debugging
   */
   ...
   *
   * <p>This method tracks successful message processing with timing information for performance
   * monitoring. It updates processing statistics and maintains min/max processing time tracking.
   *
   * <p>High-Level Steps: 1. Increment total messages processed counter atomically 2. Add processing
   * time to total processing time accumulator 3. Update min/max processing time statistics 4. Log
   * debug information for monitoring and debugging
   *
  ...
  public void recordMessageProcessed(long processingTimeMs) {
    ...
    log.debug(
        "Message processed in {} ms. Total processed: {}",
        processingTimeMs,
   ...
   *
   * <p>This method tracks message processing failures for error rate calculation and operational
   * monitoring. It's called when message processing fails due to errors or exceptions.
   *
   * <p>High-Level Steps: 1. Increment total messages failed counter atomically 2. Log debug
   ...
   *
   * <p>This method calculates the success rate based on consumed vs processed messages. It returns
   * 100% if no messages have been consumed yet.
   *
   * <p>High-Level Steps: 1. Get total messages consumed count 2. Return 100% if no messages
   * consumed (avoid division by zero) 3. Calculate success rate as (processed / consumed) * 100 4.
   ...
   *
   * <p>This method calculates the error rate based on failed vs consumed messages. It returns 0% if
   * no messages have been consumed yet.
   *
   * <p>High-Level Steps: 1. Get total messages consumed count 2. Return 0% if no messages consumed
   * (avoid division by zero) 3. Calculate error rate as (failed / consumed) * 100 4. Return error
   * rate percentage
   *
   ...
   *
   * <p>This method calculates the average processing time across all successfully processed
   * messages. It returns 0 if no messages have been processed yet.
   *
   * <p>High-Level Steps: 1. Get total messages processed count 2. Return 0 if no messages processed
   * (avoid division by zero) 3. Calculate average as total processing time / total processed 4.
  ...
  /** Resets all metrics to their initial state. Useful for testing or metrics refresh. */
  ...
  public void recordBatchConsumed(int batchSize) {
    ...
    updateTimestamp();
    log.debug(
        "Batch consumed with {} messages. Total batches: {}",
        batchSize,
  ...
  public void recordBatchProcessed(int batchSize, long processingTimeMs) {
    ...
    log.debug(
        "Batch processed in {} ms with {} messages. Total batches: {}",
        processingTimeMs,
        batchSize,
  ...
  public void recordBatchRateLimited(int batchSize) {
    ...
    log.debug(
        "Batch rate limited with {} messages. Total rate limited batches: {}",
        batchSize,
        totalBatchesRateLimited.get());
  }
  ...
  public void recordBatchFailed(int batchSize) {
    ...
    log.debug(
        "Batch processing failed with {} messages. Total batch failures: {}",
        batchSize,
  ...
  public void recordBatchPartiallyProcessed(int totalBatchSize, int processedCount) {
    ...
    log.debug(
        "Batch partially processed: {}/{} messages. Total partial batches: {}",
        processedCount,
        totalBatchSize,
        totalPartiallyProcessedBatches.get());
  }
