...
/**
 * Health indicator for JVM memory usage.
 * Monitors heap memory usage and triggers alerts when usage exceeds thresholds.
 */
@Slf4j
@Component("jvmMemory")
public class MemoryHealthIndicator implements HealthIndicator {
    ...
    @Autowired
    public MemoryHealthIndicator(KafkaMonitoringService kafkaMonitoringService,
    ...
    public Health health() {
        try {
            ...
            boolean isMemoryHealthy = kafkaMonitoringService.isMemoryHealthy(config.getMemoryThresholdPercentage());
            ...
            if (!isMemoryHealthy) {
                healthBuilder.withDetail("reason", 
                        String.format("Memory usage (%.1f%%) exceeds threshold (%d%%)", 
