 ...
 *
 * <p>This service provides comprehensive monitoring capabilities for Kafka consumer operations,
 * including connectivity checks, message consumption tracking, and performance metrics. It
 * implements caching mechanisms for expensive health checks and provides real-time monitoring of
 * consumer health, performance, and operational status.
 *
 * <p>Key Functionality: - Consumer connectivity and group membership monitoring - Message
 * consumption rate calculation using total messages processed over actual duration - SSL/TLS
 * connection health verification - Memory usage tracking and health assessment - Caching mechanism
 * for expensive health checks to optimize performance - Consumer activity tracking and stuck
 * detection - Cluster health and topic accessibility monitoring - Performance metrics collection
 * and reporting
 *
 * <p>High-Level Processing Steps: 1. Monitor consumer connectivity and group membership 2. Track
 * message consumption rates and activity 3. Verify SSL/TLS connection health 4. Monitor memory
 * usage and system health 5. Cache expensive health check results 6. Provide comprehensive metrics
 * and health reporting
 *
 * <p>Mathematical Formulas: - Message Consumption Rate: rate = totalMessagesProcessed /
 * actualDurationInMinutes where actualDuration = Duration.between(firstMessageTimestamp, now) -
 * Memory Usage Percentage: percentage = (usedMemory / totalMemory) * 100 - Success Rate:
 * successRate = (totalMessagesProcessed - totalMessagesFailed) / totalMessagesProcessed
 *
 * <p>Thread Safety: This service is thread-safe through the use of: - ConcurrentHashMap for health
 * check caching - Volatile fields for thread-visible state updates - Atomic operations in
 * KafkaConsumerMetrics - Synchronized access to shared resources
 *
 * <p>Caching Strategy: - 30-second TTL for health check results - Prevents resource leaks from
 * frequent expensive operations - Balances performance optimization with timely state detection -
 * Automatic cache expiration and cleanup
 *
 * <p>Health Check Categories: - Connectivity checks (consumer, cluster, SSL) - Activity monitoring
 * (polling, consumption) - Resource monitoring (memory usage) - Performance metrics (processing
 * times, rates)
 *
...
public class KafkaMonitoringService {
  ...
  private final ConcurrentHashMap<String, CachedResult> healthCheckCache =
  ...
  /**
   * Cache Time-To-Live for health check results in milliseconds. Rationale: 30 seconds provides a
   * good balance between performance optimization and timely detection of state changes. Frequent
   * health checks can be expensive, especially those involving network calls to Kafka brokers.
   */
  ...
  /**
   * Default timeout for consumer polling activity in minutes. Rationale: 5 minutes allows for
   * reasonable idle periods while detecting genuine connectivity issues. This accommodates normal
   * Kafka consumer behavior during low-traffic periods.
   */
  private static final int DEFAULT_POLLING_TIMEOUT_MINUTES = 5;

  /**
   * Default timeout threshold for message consumption health checks in minutes. Rationale: Used as
   * a baseline for determining if the consumer is active and processing messages within expected
   * timeframes.
   */
  private static final int DEFAULT_CONSUMPTION_TIMEOUT_MINUTES = 5;

  /**
   * Default minimum consumption rate threshold in messages per minute. Rationale: 0.1
   * messages/minute provides a very low threshold to avoid false negatives during legitimate idle
   * periods while still detecting complete consumption failures.
   */
  ...
  /**
   * Default memory usage threshold percentage for health checks. Rationale: 90% provides a
   * reasonable buffer before memory exhaustion while allowing for normal memory usage patterns and
   * garbage collection.
   */
  ...
  public KafkaMonitoringService(
      KafkaProperties kafkaProperties, KafkaConsumerMetrics metrics, AdminClient adminClient) {
   ...
   *
   * <p>This class provides a simple wrapper for caching health check results with automatic
   * expiration based on a configurable TTL. It ensures that expensive health checks are not
   * repeated unnecessarily while maintaining timely detection of state changes.
   *
   * <p>Key Features: - Result storage with timestamp - Automatic expiration checking - Thread-safe
   * access patterns - Memory-efficient storage
   *
   ...
   *
   * <p>This method implements a caching strategy for expensive health check operations. It first
   * checks if a valid cached result exists, and if not, computes a new result using the provided
   * supplier function.
   *
   * <p>Processing Steps: 1. Check if a cached result exists for the given key 2. If cached result
   * exists and is not expired, return it 3. If no cached result or expired, compute new result
   * using supplier 4. Cache the new result with current timestamp 5. Return the computed result
   *
   * <p>Caching Benefits: - Reduces expensive network calls to Kafka brokers - Improves health check
   * response times - Prevents resource exhaustion from frequent checks - Maintains timely state
   * detection with TTL
   *
   * <p>Thread Safety: - Uses ConcurrentHashMap for thread-safe caching - Atomic check-and-compute
   * operations - No race conditions in cache access
   *
   ...
   *
   * <p>This method updates the last polling activity timestamp to track when the consumer was last
   * active. It's called by the consumer to indicate ongoing polling activity for health monitoring.
   *
   * <p>Processing Steps: 1. Update the volatile lastPollActivity field with current timestamp 2.
   * Log the polling activity for monitoring purposes 3. Ensure thread-safe access to the timestamp
   * field
   *
   * <p>Thread Safety: - Uses volatile field for thread-visible updates - Atomic timestamp updates -
   * No synchronization required for simple assignment
   *
   * <p>Use Cases: - Called by consumer during normal polling operations - Used by health indicators
   * to detect stuck consumers - Provides activity tracking for monitoring dashboards - Enables
   * timeout-based health assessments
   */
   ...
   *
   * <p>FIXED: This method now tracks actual polling activity instead of message processing
   * activity. The consumer should call recordPollActivity() during each poll cycle to indicate it's
   * actively polling.
   *
   * <p>This properly distinguishes between: - "No messages available" (healthy - consumer is
   * polling but no messages) - "Consumer stopped polling" (unhealthy - consumer is not polling at
   * all)
   */
  public boolean isConsumerPollingActive(int timeoutMinutes) {
    try {
      ...
      if (!isActive) {
        log.warn(
            "Consumer polling appears inactive. Last poll activity: {}, timeout: {} minutes",
            lastPollActivity,
            timeoutMinutes);
      } else {
   ...
   *
   * <p>Mathematical Formula: rate = totalMessagesProcessed / actualDurationInMinutes
   *
   * <p>Where: - totalMessagesProcessed: Total number of messages successfully processed since first
   * message - actualDurationInMinutes: Time elapsed from first message timestamp to current time in
   * minutes
   *
   * <p>Special Cases: - Returns 0.0 if no messages have been processed or no first message
   * timestamp available - For durations < 1 minute, returns total message count to avoid division
   * by zero - Handles timestamp parsing errors gracefully by returning 0.0
   *
   ...
   *
   * <p>ENHANCED: This method now properly distinguishes between: - "No messages available" (healthy
   * - consumer can connect and poll) - "Consumer polling issues" (unhealthy - consumer connectivity
   * problems)
   *
   * <p>The key insight is that during idle periods (no messages), health should depend on
  ...
  public boolean isMessageConsumptionHealthy(int timeoutMinutes, double minimumRateThreshold) {
    ...
    try {
      ...
      if (totalConsumed == 0) {
        log.debug(
            "Message consumption healthy: Consumer ready, no messages consumed yet (idle state)");
        return true;
      }
      ...
      if (totalConsumed > 0 && totalProcessed < (totalConsumed * 0.8)) {
        log.warn(
            "Message consumption unhealthy: Processing lag detected - consumed={}, processed={}",
            totalConsumed,
            totalProcessed);
        return false;
      }
      ...
      if (lastMessageTime != null) {
        try {
          ...
          if (timeSinceLastMessage.toMinutes() > (timeoutMinutes * 2)) {
            log.warn(
                "Message consumption potentially degraded: No messages for {} minutes (2x timeout: {})",
                timeSinceLastMessage.toMinutes(),
                timeoutMinutes * 2);
  ...
  /**
   * Checks if consumer is actively polling. Uses the default polling timeout defined in
   ...
   *
   * <p>This method delegates to getMessageConsumptionRate() which calculates the rate based on
   * actual duration from first message to now.
   *
  ...
  /**
   * Checks if consumption is healthy. Uses default timeout and minimum rate thresholds defined in
   * constants.
   */
  public boolean isConsumptionHealthy() {
    return isMessageConsumptionHealthy(
        DEFAULT_CONSUMPTION_TIMEOUT_MINUTES, DEFAULT_MINIMUM_CONSUMPTION_RATE);
  }

  /**
   * Checks if memory is healthy. Uses the default memory threshold defined in
   ...
   *
   * <p>A consumer is considered stuck if: - It's actively polling (recent poll activity) - But
   * consumption rate is extremely low despite being connected - And there are no processing errors
   *
   * <p>This indicates the consumer might be stuck in a poll loop without advancing position.
   *
  ...
  public boolean isConsumerStuck() {
    try {
      ...
      boolean hasBeenRunning =
          getTotalMessagesConsumed() > 0
              || Duration.between(lastPollActivity, LocalDateTime.now()).toMinutes() > 2;

      // Consumer is potentially stuck if:
      // - Polling recently BUT consumption rate is very low AND connected AND low errors AND has
      // been running
      boolean potentiallyStuck =
          recentPollActivity && lowConsumptionRate && isConnected && lowErrorRate && hasBeenRunning;

      if (potentiallyStuck) {
        log.warn(
            "Consumer appears to be stuck - polling but not consuming. Rate: {}, Connected: {}, Errors: {}%",
