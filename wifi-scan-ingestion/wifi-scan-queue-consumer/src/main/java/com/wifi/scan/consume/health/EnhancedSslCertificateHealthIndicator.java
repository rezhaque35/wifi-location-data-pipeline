 ...
 * 
 * This health indicator provides comprehensive SSL/TLS certificate validation
 * with advanced features for production readiness monitoring, CloudWatch
 * integration, and operational guidance. It is specifically optimized for
 * Kubernetes readiness probes and automated certificate management workflows.
 * 
 * Key Functionality:
 * - Enhanced readiness-specific behavior for graceful service degradation
 * - CloudWatch integration preparation with detailed metrics
 * - Certificate chain validation for all certificates
 * - Multi-threshold warning system (30/15/7 days)
 * - Operational guidance for certificate management
 * - Health scoring and certificate expiry timeline analysis
 * - Event-driven certificate renewal workflow support
 * 
 * High-Level Processing Steps:
 * 1. Check if SSL is enabled in configuration
 * 2. Perform comprehensive certificate validation
 * 3. Determine health status based on validation results
 * 4. Add CloudWatch metrics for monitoring integration
 * 5. Include certificate chain validation details
 * 6. Provide certificate expiry timeline information
 * 7. Add operational guidance for detected issues
 * 8. Build comprehensive health response
 * 
 * Readiness Optimization Features:
 * - Graceful pod removal from service traffic for expired certificates
 * - CloudWatch metric export for automated alerting
 * - Event-driven certificate renewal workflows
 * - Zero infrastructure overhead monitoring
 * - Health scoring for automated decision making
 * 
 * Certificate Validation Features:
 * - Keystore and truststore accessibility validation
 ...
 * - Urgent: 15 days until expiration
 * - Warning: 30 days until expiration
 * 
 * CloudWatch Integration:
 * - Certificate expiry metrics for automated alerting
 * - Health score metrics for monitoring dashboards
 * - Event generation for certificate management workflows
 ...
 * - Service degradation for certificate issues
 * 
...
public class EnhancedSslCertificateHealthIndicator implements HealthIndicator {
     ...
     */
    @Autowired
    public EnhancedSslCertificateHealthIndicator(KafkaMonitoringService kafkaMonitoringService,
     ...
     * 
     * This method implements the core health check logic for SSL certificates,
     * providing comprehensive validation, monitoring integration, and operational
     * guidance. It is optimized for readiness probes and automated workflows.
     * 
     * Processing Steps:
     * 1. Check if SSL is enabled in configuration
     * 2. Perform comprehensive certificate validation
     ...
     * - SSL connectivity: Must be able to establish SSL connections
     * - Certificate accessibility: Keystore and truststore must be accessible
     * - Certificate expiration: No certificates should be expired
     * - Certificate chain: Certificate chains should be valid
     * - Warning thresholds: Certificates should not be in critical/urgent warning periods
     * 
     * Enhanced Features:
     * - Readiness optimization for graceful service degradation
     ...
     * - Operational guidance in health responses
     * 
    ...
    private EnhancedCertificateValidationResult performEnhancedCertificateValidation() {
        ...
        EnhancedCertificateInfo keystoreInfo = validateCertificateStore(
        ...
        EnhancedCertificateInfo truststoreInfo = validateCertificateStore(
    ...
    private Health.Builder determineHealthStatus(EnhancedCertificateValidationResult result, 
        ...
        if (result.hasExpiredCertificates()) {
            ...
            details.put("gracefulDegradation", 
    ...
    private void addCloudWatchMetrics(EnhancedCertificateValidationResult result, 
                                    Map<String, Object> details) {
        ...
        cloudWatchMetrics.put("alertTimelineStage", determineAlertStage(result.getMinDaysUntilExpiry()));
        cloudWatchMetrics.put("certificateHealthScore", result.calculateHealthScore());
        
        // Kubernetes event generation data
        cloudWatchMetrics.put("kubernetesEventData", Map.of(
                "eventType", result.hasIssues() ? "Warning" : "Normal",
                "reason", "SSLCertificateStatus",
                "message", generateEventMessage(result)
        ));
        
        details.put("cloudWatchMetrics", cloudWatchMetrics);
    }

    private void addCertificateChainValidation(EnhancedCertificateValidationResult result, 
                                             Map<String, Object> details) {
        ...
        chainValidation.put("totalCertificatesValidated", 
    ...
    private void addCertificateExpiryTimeline(EnhancedCertificateValidationResult result, 
                                            Map<String, Object> details) {
        Map<String, Object> timeline = new HashMap<>();
        
        timeline.put("warningThresholds", Arrays.asList(STANDARD_WARNING_DAYS, URGENT_WARNING_DAYS, CRITICAL_WARNING_DAYS));
    ...
    private void addOperationalGuidance(EnhancedCertificateValidationResult result, 
                                      Map<String, Object> details) {
        Map<String, Object> guidance = new HashMap<>();
        
        guidance.put("immediateAction", generateImmediateActionGuidance(result));
        guidance.put("renewalProcedure", "Follow certificate renewal procedures in operational runbook");
    ...
    private Health buildFailureResponse(Exception e, Map<String, Object> details) {
        ...
        details.put("gracefulDegradation", "Graceful failure - pod removed from service for manual investigation");
        return Health.down().withDetails(details).build();
    }

    private EnhancedCertificateInfo validateCertificateStore(String location, String password, 
                                                           String type, String storeType) {
        try {
    ...
    private EnhancedCertificateInfo validateCertificatesInStore(KeyStore keyStore, String storeType) 
        ...
        while (aliases.hasMoreElements()) {
            ...
            if (cert instanceof X509Certificate x509Cert) {
                ...
                certificates.add(new CertificateDetails(alias, x509Cert.getSubjectX500Principal().getName(), 
        ...
        return new EnhancedCertificateInfo(true, hasExpired, expiringSoon, 
                minDaysUntilExpiry == Long.MAX_VALUE ? -1 : minDaysUntilExpiry,
                storeType, certificates, chainValid);
    }
    ...
    private String generateEventMessage(EnhancedCertificateValidationResult result) {
        ...
        if (result.hasCriticalWarnings()) {
            return String.format("SSL certificates expire in %d days - urgent renewal required", 
                    result.getMinDaysUntilExpiry());
        }
        if (result.hasWarnings()) {
            return String.format("SSL certificates expire in %d days - renewal recommended", 
                    result.getMinDaysUntilExpiry());
        }
    ...
    private static class EnhancedCertificateValidationResult {
        ...
        public EnhancedCertificateValidationResult(boolean sslConnectionHealthy, 
        ...
        public boolean isSslConnectionHealthy() { return sslConnectionHealthy; }
        public EnhancedCertificateInfo getKeystoreInfo() { return keystoreInfo; }
        public EnhancedCertificateInfo getTruststoreInfo() { return truststoreInfo; }
        ...
        public boolean hasCriticalWarnings() {
            return (keystoreInfo.getDaysUntilExpiry() > 0 && keystoreInfo.getDaysUntilExpiry() <= CRITICAL_WARNING_DAYS) ||
                   (truststoreInfo.getDaysUntilExpiry() > 0 && truststoreInfo.getDaysUntilExpiry() <= CRITICAL_WARNING_DAYS);
        }
    ...
    private static class EnhancedCertificateInfo {
        ...
        public EnhancedCertificateInfo(boolean accessible, boolean expired, boolean expiringSoon, 
                                     long daysUntilExpiry, String storeType, 
                                     List<CertificateDetails> certificates, boolean chainValid) {
        ...
        public static EnhancedCertificateInfo notAccessible(String storeType) {
            return new EnhancedCertificateInfo(false, true, true, -1, storeType, 
        ...
        public List<CertificateDetails> getCertificates() { return certificates; }
        public boolean isChainValid() { return chainValid; }
        public int getCertificateCount() { return certificates.size(); }
    }

    private record CertificateDetails(String alias, String subject, LocalDateTime expiry, long daysUntilExpiry) {}
} 