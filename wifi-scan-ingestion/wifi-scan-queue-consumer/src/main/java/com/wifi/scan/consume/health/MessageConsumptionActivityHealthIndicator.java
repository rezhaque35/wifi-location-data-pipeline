 ...
 * 
 * This health indicator monitors the active consumption of messages from Kafka topics,
 ...
 * monitoring for consumer liveness and processing pipeline health.
 * 
 * Key Functionality:
 * - Track message consumption count and trends for processing pipeline health
 * - Monitor messages processed per time window with configurable thresholds
 ...
 * - Provide detailed health status with operational guidance
 * 
 * High-Level Processing Steps:
 * 1. Check basic consumer connectivity to Kafka cluster
 * 2. Verify consumer group membership and activity
 * 3. Monitor time since last poll activity
 * 4. Detect consumer stuck conditions (polling but not advancing)
 * 5. Analyze message consumption rates and success rates
 * 6. Compare consumption patterns against configured thresholds
 * 7. Provide detailed health status with operational context
 * 
 ...
 * - Connectivity: Consumer connection to Kafka cluster
 * - Group Activity: Consumer group membership and participation
 * - Poll Activity: Time since last poll and stuck detection
 * - Consumption Rate: Messages processed per time window
 * - Success Rate: Successful vs failed message processing
 * - Processing Pipeline: Overall message processing health
 * 
 * Configuration Properties:
 * - message-timeout-threshold: Timeout for message activity (default: 5 minutes)
 * - consumption-rate-threshold: Minimum consumption rate (default: 0.1 messages/minute)
 * 
 * Use Cases:
 * - Kubernetes liveness probe configuration
 * - Consumer health monitoring and alerting
 * - Processing pipeline health assessment
...
public class MessageConsumptionActivityHealthIndicator implements HealthIndicator {
    ...
    /**
     * Creates a new MessageConsumptionActivityHealthIndicator for testing with explicit configuration.
     * 
     * This constructor allows for testing with specific configuration values
     * without relying on Spring property injection.
     * 
     * @param kafkaMonitoringService Service for Kafka consumer monitoring and metrics
     ...
     * 
     * This method converts the configured timeout threshold from minutes
     * to milliseconds for internal time-based calculations.
     * 
     ...
     * 
     * This method implements the core health check logic, evaluating multiple
     * aspects of consumer health including connectivity, group activity,
     * poll activity, consumption rates, and processing success rates.
     * 
     * Processing Steps:
     * 1. Check basic consumer connectivity to Kafka cluster
     * 2. Verify consumer group membership and activity
     ...
     * 5. Analyze message consumption rates and success rates
     * 6. Compare consumption patterns against configured thresholds
     * 7. Build comprehensive health response with operational details
     * 
     * Health Evaluation Criteria:
     * - Consumer connectivity: Must be able to connect to Kafka cluster
     * - Group activity: Must be active member of consumer group
     * - Poll activity: Must have recent poll activity within timeout threshold
     * - Consumer stuck: Must not be stuck (polling but not advancing)
     * - Consumption rate: Must meet minimum consumption rate threshold
     * - Success rate: Must maintain acceptable processing success rate
     * 
     * Error Handling:
     * - Comprehensive exception catching and logging
     * - Detailed error messages for troubleshooting
     * - Graceful degradation with meaningful health status
