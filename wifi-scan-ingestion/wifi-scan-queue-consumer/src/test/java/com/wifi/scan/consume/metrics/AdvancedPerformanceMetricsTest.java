...
/**
 * Test class for AdvancedPerformanceMetrics following TDD principles.
 * Tests advanced performance monitoring capabilities for high-throughput batch processing.
 */
...
class AdvancedPerformanceMetricsTest {
    ...
    void shouldTrackPollProcessingTimeAndAlert() {
        ...
        // Then
        assertEquals(2, performanceMetrics.getTotalPollOperations(), 
                "Should track total poll operations");
        assertEquals(1050.0, performanceMetrics.getAveragePollProcessingTimeMs(), 0.1, 
                "Should calculate average poll processing time");
        assertEquals(1, performanceMetrics.getSlowPollCount(), 
                "Should count polls exceeding 1.2s threshold");
        assertTrue(performanceMetrics.isCurrentPollPerformanceAlert(), 
    ...
    void shouldMonitorThreadUtilizationAndAlert() {
        ...
        // Then
        assertEquals(75.0, performanceMetrics.getAverageThreadUtilization(), 0.1, 
                "Should calculate average thread utilization");
        assertEquals(85.0, performanceMetrics.getCurrentThreadUtilization(), 0.1, 
                "Should track current thread utilization");
        assertTrue(performanceMetrics.isHighThreadUtilizationAlert(), 
    ...
    void shouldTrackBufferFillTime() {
        ...
        // Then
        assertEquals(37_500.0, performanceMetrics.getAverageBufferFillTimeMs(), 0.1, 
                "Should calculate average buffer fill time");
        assertEquals(2, performanceMetrics.getTotalBufferFillOperations(), 
                "Should track total buffer fill operations");
        assertEquals(1, performanceMetrics.getBufferFillOutsideRangeCount(), 
                "Should count buffer fills outside 13-44s range");
        assertFalse(performanceMetrics.isOptimalBufferFillPerformance(), 
    ...
    void shouldMonitorFirehoseBatchComposition() {
        ...
        assertEquals(85.14, performanceMetrics.getAverageFirehoseBatchSize(), 0.1, 
                "Should calculate average batch size");
        assertEquals(7, performanceMetrics.getTotalFirehoseBatches(), 
                "Should track total Firehose batches");
    }
    ...
    void shouldTrackConsumerLagWithRateLimitingContext() {
        ...
        // Then
        assertEquals(825.0, performanceMetrics.getAverageConsumerLag(), 0.1, 
                "Should calculate average consumer lag");
        assertEquals(1500, performanceMetrics.getCurrentConsumerLag(), 
                "Should track current consumer lag");
        assertEquals(1, performanceMetrics.getHighLagDuringRateLimitingCount(), 
                "Should count high lag events during rate limiting");
        assertTrue(performanceMetrics.isConsumerLagAlert(), 
    ...
    void shouldTrackBatchRejectionRateAndAlert() {
        ...
        // Then
        assertEquals(7.0, performanceMetrics.getBatchRejectionRate(), 0.1, 
                "Should calculate batch rejection rate");
        assertEquals(100, performanceMetrics.getTotalBatchAttempts(), 
                "Should track total batch attempts");
        assertEquals(7, performanceMetrics.getTotalBatchRejections(), 
                "Should track total batch rejections");
        assertTrue(performanceMetrics.isBatchRejectionRateAlert(), 
        ...
        assertEquals(7, rejectionReasons.get("Rate limiting"), 
                "Should categorize rejection reasons");
    }
    ...
    void shouldMonitorDynamicBatchSizeEffectiveness() {
        ...
        assertTrue(effectiveness.containsKey("medium(61-100)"), "Should track medium batch effectiveness");
        assertTrue(effectiveness.containsKey("large(101-150)"), "Should track large batch effectiveness");
        
        assertEquals(100.0, effectiveness.get("small(<=60)"), 0.1, 
                "Small batches should have 100% success rate");
        assertEquals(100.0, effectiveness.get("medium(61-100)"), 0.1, 
                "Medium batches should have 100% success rate");
        assertEquals(0.0, effectiveness.get("large(101-150)"), 0.1, 
                "Large batches should have 0% success rate in this scenario");
        
        assertEquals(60.0, performanceMetrics.getOverallBatchSuccessRate(), 0.1, 
    ...
    void shouldDistinguishThrottlingTypes() {
        ...
        assertEquals(2, throttlingCounts.get(firehoseThrottling), 
                "Should count Firehose throttling events");
        assertEquals(1, throttlingCounts.get(appRateLimiting), 
                "Should count application rate limiting events");
        
        Map<String, Double> throttlingDurations = performanceMetrics.getAverageThrottlingDurations();
        assertEquals(1750.0, throttlingDurations.get(firehoseThrottling), 0.1, 
                "Should calculate average Firehose throttling duration");
        assertEquals(800.0, throttlingDurations.get(appRateLimiting), 0.1, 
                "Should calculate average app rate limiting duration");
        
        assertEquals(3, performanceMetrics.getTotalThrottlingEvents(), 
                "Should track total throttling events");
    }
    ...
    void shouldProvideComprehensiveMetricsSummary() {
        ...
        assertTrue(summary.contains("Poll Processing Performance"), 
                "Should include poll processing metrics");
        assertTrue(summary.contains("Thread Utilization"), 
                "Should include thread utilization metrics");
        assertTrue(summary.contains("Buffer Fill Performance"), 
                "Should include buffer fill metrics");
        assertTrue(summary.contains("Firehose Batch Composition"), 
                "Should include Firehose batch metrics");
        assertTrue(summary.contains("Consumer Lag Analysis"), 
                "Should include consumer lag metrics");
        assertTrue(summary.contains("Batch Processing Effectiveness"), 
    ...
    void shouldResetAllAdvancedMetrics() {
        ...
        // Then
        assertEquals(0, performanceMetrics.getTotalPollOperations(), 
                "Should reset poll operations count");
        assertEquals(0.0, performanceMetrics.getCurrentThreadUtilization(), 
                "Should reset thread utilization");
        assertEquals(0, performanceMetrics.getTotalBatchAttempts(), 
                "Should reset batch attempts count");
        assertFalse(performanceMetrics.isCurrentPollPerformanceAlert(), 
                "Should reset alert states");
    }
} 