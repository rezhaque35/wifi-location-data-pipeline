...
/**
 * Unit tests for Kafka consumer configuration functionality. Tests consumer factory and SSL
 * properties configuration using TDD approach.
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("Kafka Consumer Configuration Tests")
class KafkaConsumerConfigurationTest {

  @Mock private SslConfiguration sslConfiguration;
  ...
  void should_CreateConsumerFactory_When_SSLEnabled() {
    ...
    Map<String, Object> expectedSslProps =
        Map.of(
    ...
    ConsumerFactory<String, String> consumerFactory =
        kafkaConsumerConfiguration.consumerFactory(kafkaProperties, sslConfiguration);
    ...
    assertEquals(
        StringDeserializer.class, config.get(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG));
    assertEquals(
        StringDeserializer.class, config.get(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG));
  ...
  void should_ConfigureConsumerProperties_When_DefaultValuesUsed() {
    ...
    Map<String, Object> consumerProps =
        kafkaConsumerConfiguration.buildConsumerProperties(defaultProperties);
    ...
    assertEquals(
        ...
        consumerProps.get(
            ConsumerConfig
                .ENABLE_AUTO_COMMIT_CONFIG)); // Updated to false for manual acknowledgment
    assertEquals(
        StringDeserializer.class, consumerProps.get(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG));
    assertEquals(
  ...
  void should_MergeSSLProperties_When_SSLEnabledAndPropertiesProvided() {
    ...
    Map<String, Object> mergedProps =
        kafkaConsumerConfiguration.mergeConsumerProperties(
            baseProps, kafkaProperties, sslConfiguration);
  ...
  void should_ParseTimeoutValues_When_ValidTimeStringsProvided() {
    ...
    int heartbeatIntervalMs =
        kafkaConsumerConfiguration.parseTimeoutMilliseconds(heartbeatInterval);
    int autoCommitIntervalMs =
        kafkaConsumerConfiguration.parseTimeoutMilliseconds(autoCommitInterval);
