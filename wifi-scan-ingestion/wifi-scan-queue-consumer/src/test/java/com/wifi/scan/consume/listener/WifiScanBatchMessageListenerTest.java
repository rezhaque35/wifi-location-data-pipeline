...
/**
 * Test class for WifiScanBatchMessageListener following TDD principles.
 * Tests the simplified batch processing flow without batch-level rate limiting.
 * 
 * Updated Flow Being Tested:
 * 1. Receive and validate batch
 * 2. Compress all messages
 * 3. Send to Firehose (rate limiting handled at delivery level)
...
class WifiScanBatchMessageListenerTest {
    ...
    @Mock
    private BatchFirehoseMessageService mockBatchFirehoseService;
    ...
    @Mock
    private MessageCompressionService mockCompressionService;
    ...
    void setUp() {
        ...
        // Setup default mock behavior for successful processing (using lenient to avoid unnecessary stubbing warnings)
        lenient().when(mockCompressionService.compressAndEncodeBatch(anyList()))
                .thenReturn(List.of("compressed_test_content"));
        lenient().when(mockBatchFirehoseService.deliverBatch(anyList()))
                .thenReturn(true);
    }
    ...
    void shouldHandleFirehoseDeliveryFailureWithRetry() {
        ...
        // Then
        verify(mockBatchFirehoseService).deliverBatch(anyList());
        verify(mockAcknowledgment).acknowledge(); // Should acknowledge failed batches to prevent reprocessing
    ...
    void shouldHandleMultipleCompressedMessages() {
        ...
        List<String> compressedMessages = List.of(
            "compressed1",
            "compressed2",
            "compressed3"
        );
        when(mockCompressionService.compressAndEncodeBatch(anyList()))
            .thenReturn(compressedMessages);
        
        // When
        batchListener.listenBatch(records, mockAcknowledgment);
        
        // Then
        verify(mockBatchFirehoseService).deliverBatch(
            argThat(messages -> messages.size() == 3 && 
    ...
    private ConsumerRecords<String, String> createTestBatch(int messageCount) {
        ...
        for (int i = 0; i < messageCount; i++) {
            recordList.add(new ConsumerRecord<>("test-topic", 0, i, "key" + i, 
    ...
    private ConsumerRecords<String, String> createTestBatchWithInvalidMessages() {
        ...
        List<ConsumerRecord<String, String>> recordList = List.of(
