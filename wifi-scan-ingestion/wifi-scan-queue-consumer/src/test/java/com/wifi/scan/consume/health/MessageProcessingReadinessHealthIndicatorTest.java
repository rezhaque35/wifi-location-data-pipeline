...
/**
 * Unit tests for MessageProcessingReadinessHealthIndicator.
 * Tests the readiness-focused behavior that checks service readiness for traffic,
 * including comprehensive connectivity and processing health checks.
 */
...
class MessageProcessingReadinessHealthIndicatorTest {
    ...
    void should_ReturnDown_When_ExceptionOccurs() {
        // Given
        when(kafkaMonitoringService.isConsumerConnected()).thenThrow(new RuntimeException("Readiness check failed"));
        ...
        assertThat(health.getDetails().get("reason")).isEqualTo("Readiness check failed due to exception");
    }
