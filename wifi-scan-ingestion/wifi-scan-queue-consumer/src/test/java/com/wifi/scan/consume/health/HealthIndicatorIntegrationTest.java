...
/**
 * Integration test for health indicators. Tests that all custom health indicators are properly
 * configured and functioning. Uses embedded Kafka to avoid dependency on external Kafka instances.
...
class HealthIndicatorIntegrationTest {
  ...
  void should_ReturnLivenessStatus_When_CheckingLivenessEndpoint() {
    ...
    ResponseEntity<String> response =
        restTemplate.getForEntity(
            "http://localhost:" + port + "/frisco-location-wifi-scan-vmb-consumer/health/liveness",
            String.class);
