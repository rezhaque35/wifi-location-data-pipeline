...
# Create a large request with exactly 30 access points (maximum allowed)
LARGE_REQUEST='{
    "wifiScanResults": [
        {
    ...
    "requestId": "large-request-test-30-aps",
    "application": "wifi-positioning-large-test-suite",
    "calculationDetail": true
}'

echo -e "\n${YELLOW}Making request with ${CYAN}30 access points${YELLOW} (maximum allowed) and calculationDetail=true...${NC}"
...
if [ $? -eq 0 ]; then
    ...
    if [ "$result" = "SUCCESS" ]; then
        ...
        if [ "$calc_info_exists" = "true" ]; then
            ...
            # Show detailed access point information
            echo -e "\n${GREEN}Access Point Details (first 10):${NC}"
            echo "$response" | jq -r '.calculationInfo.accessPoints[0:10][] | "  " + .bssid + " - " + .ssid + " (usage: " + .usage + ", status: " + .status + ", signal: " + (.signalStrength | tostring) + " dBm)"'
            
            if [ "$total_aps" != "N/A" ] && [ "$total_aps" -gt 10 ]; then
                remaining=$((total_aps - 10))
                echo -e "  ${YELLOW}... and $remaining more access points${NC}"
            fi
        else
