 ...
 *
 * <p>The builder includes methods to determine: - Geometric Quality Factor: Based on AP
 * geometry/GDOP calculation - Signal Distribution Factor: Based on standard deviation of signal
 * strengths - Signal Quality Factor: Based on average signal strength
 */
@Component
public class SelectionContextBuilder {
   ...
   */
  public SelectionContext buildContext(
      List<WifiScanResult> validScans, Map<String, WifiAccessPoint> apMap) {
    // Calculate selection factors
    GeometricQualityFactor geometricQuality =
        GeometricQualityFactor.determineGeometricQuality(validScans, apMap);
    ...
    long apCount = validScans.stream().map(WifiScanResult::macAddress).distinct().count();
   ...
   *
   * <p>This method assesses how signal strengths vary across different APs by calculating the
   * standard deviation of signal strengths and categorizing: - UNIFORM_SIGNALS: standard deviation
   * < 3.0 dB (signals are consistent) - MIXED_SIGNALS: 3.0 ≤ standard deviation < 10.0 dB (moderate
   * variation) - SIGNAL_OUTLIERS: standard deviation ≥ 10.0 dB (significant variation/outliers)
   *
   ...
   *
   * <p>This method assesses the overall signal quality by averaging signal strengths and
   * categorizing: - STRONG_SIGNAL: average > -70 dBm (good signal quality) - MEDIUM_SIGNAL: -70 dBm
   * ≥ average ≥ -85 dBm (moderate signal quality) - WEAK_SIGNAL: average < -85 dBm (poor signal
   * quality)
   *
   * @param wifiScans List of WiFi scan results
  ...
  private double evaluateAPLocationCertainty(
      List<WifiScanResult> validScans, Map<String, WifiAccessPoint> apMap) {
