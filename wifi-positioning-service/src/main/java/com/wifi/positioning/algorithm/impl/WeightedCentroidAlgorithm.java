 ...
 *
 * <p>USE CASES: - Best suited for environments with many APs (4+ APs) - Effective when AP geometry
 * is poor for trilateration - Useful in areas with high AP density but variable signal quality -
 * Good fallback when more precise methods fail
 *
 * <p>STRENGTHS: - Simple and computationally efficient - Robust to individual AP failures or signal
 * anomalies - Works well with non-uniform AP distributions - Handles overlapping AP coverage
 * effectively
 *
 * <p>WEAKNESSES: - Less accurate than trilateration in ideal conditions - Sensitive to AP
 * distribution geometry - May be biased toward areas of high AP density - Accuracy depends heavily
 * on signal strength quality
 *
 * <p>MATHEMATICAL MODEL: The algorithm uses a weighted average of AP positions based on normalized
 * signal strengths:
 *
 * <p>1. Signal Strength Normalization: normalized = (RSSI - RSSI_MAX) / (RSSI_MIN - RSSI_MAX)
 * where: - RSSI is the received signal strength indicator - RSSI_MAX = -30 dBm (theoretical maximum
 * WiFi signal strength) - RSSI_MIN = -100 dBm (practical minimum usable WiFi signal strength) -
 * Normalized value ranges from 0 (weakest) to 1 (strongest)
 *
 * <p>2. Exponential Weight Calculation: weight = WEIGHT_BASE^normalized where: - WEIGHT_BASE = 10
 * (chosen to provide exponential weighting favoring stronger signals) - Stronger signals
 * (normalized closer to 1) get exponentially higher weights - This creates a preference for closer
 * APs with stronger signals
 *
 * <p>3. Weighted Position Calculation: P = Σ(Pi × wi) / Σ(wi) where: - P is the final calculated
 * position (latitude, longitude, altitude) - Pi is the position of access point i - wi is the
 * calculated weight for access point i - This formula ensures positions are biased toward stronger
 * signal sources
 *
 * <p>4. Confidence Calculation: confidence = min(MAX_CONFIDENCE, coverage × BASE_CONFIDENCE) where:
 * - coverage = number_of_scanned_APs / number_of_known_APs - MAX_CONFIDENCE = 0.8 (prevents
 * overconfidence even with perfect coverage) - BASE_CONFIDENCE = 0.7 (algorithm's inherent
 * confidence level)
 *
 * <p>5. Accuracy Estimation: Uses parallel stream to calculate average horizontal accuracy of
 * participating APs Falls back to DEFAULT_ACCURACY = 15.0 meters if no accuracy data available
 */
@Component
public class WeightedCentroidAlgorithm implements PositioningAlgorithm {
  ...
  /**
   * Maximum theoretical WiFi signal strength in dBm. Rationale: -30 dBm represents very close
   * proximity (< 1 meter) to an AP. This value is rarely achieved in practice but serves as the
   * upper bound for signal strength normalization calculations.
   */
  private static final double RSSI_MAX = -30.0;

  /**
   * Minimum practical WiFi signal strength in dBm for positioning. Rationale: -100 dBm represents
   * the lower threshold where WiFi signals become too weak for reliable positioning. Below this
   * threshold, signal strength variations are dominated by noise rather than distance.
   */
  private static final double RSSI_MIN = -100.0;

  // Weight calculation constants
  /**
   * Base value for exponential weight calculation. Rationale: Base 10 provides strong exponential
   * scaling that heavily favors stronger signals while still giving some weight to weaker ones.
  ...
  /**
   * Base confidence level for the weighted centroid algorithm. Rationale: 0.7 reflects the
   * algorithm's inherent reliability. It's lower than trilateration (0.8-0.9) but higher than
   * simple proximity (0.5) because it uses multiple APs but doesn't solve geometric constraints.
   */
  private static final double BASE_CONFIDENCE = 0.7;

  /**
   * Maximum achievable confidence level for the algorithm. Rationale: 0.8 prevents overconfidence
   * even with perfect AP coverage. Weighted centroid is inherently less precise than trilateration
   * due to geometric ambiguity, so confidence is capped below maximum levels.
   */
  private static final double MAX_CONFIDENCE = 0.8;

  /**
   * Default horizontal accuracy in meters when AP accuracy data is unavailable. Rationale: 15
   * meters represents typical indoor positioning accuracy for weighted centroid methods based on
   * empirical studies. This conservative estimate accounts for the algorithm's geometric
   * limitations.
   */
  private static final double DEFAULT_ACCURACY_METERS = 15.0;

  /**
   * Zero threshold for floating point comparisons. Rationale: Used to safely compare weighted sums
   * to zero, accounting for floating point precision limitations in calculations.
   */
   ...
   * weaknesses of the Weighted Centroid algorithm: - Works well with 2+ APs, optimal with 3-4+ APs
   * - Robust to signal quality variations - Improved performance with poor geometry (unlike
   * trilateration) - Very effective with mixed signals and outliers
   */
  // AP Count weights from framework document
  private static final double WEIGHTED_CENTROID_SINGLE_AP_WEIGHT =
  ...
  private static final double WEIGHTED_CENTROID_STRONG_SIGNAL_MULTIPLIER =
      1.0; // No change with strong signals
  private static final double WEIGHTED_CENTROID_MEDIUM_SIGNAL_MULTIPLIER =
      1.0; // No change with medium signals
  private static final double WEIGHTED_CENTROID_WEAK_SIGNAL_MULTIPLIER =
      0.8; // Moderate reduction with weak signals
  private static final double WEIGHTED_CENTROID_VERY_WEAK_SIGNAL_MULTIPLIER =
      0.0; // ×0.0 for very weak signals

  // Geometric quality multipliers from framework document
  private static final double WEIGHTED_CENTROID_EXCELLENT_GDOP_MULTIPLIER =
      1.0; // No change for excellent geometry
  private static final double WEIGHTED_CENTROID_GOOD_GDOP_MULTIPLIER =
      1.1; // Slight boost with good geometry
  private static final double WEIGHTED_CENTROID_FAIR_GDOP_MULTIPLIER =
      1.2; // Better with fair geometry
  private static final double WEIGHTED_CENTROID_POOR_GDOP_MULTIPLIER =
      1.3; // Best with poor geometry

  // Signal distribution multipliers from framework document
  private static final double WEIGHTED_CENTROID_UNIFORM_SIGNALS_MULTIPLIER =
      1.0; // No change for uniform signals
  private static final double WEIGHTED_CENTROID_MIXED_SIGNALS_MULTIPLIER =
      1.8; // Better with mixed signals
  private static final double WEIGHTED_CENTROID_SIGNAL_OUTLIERS_MULTIPLIER =
      1.4; // Best with signal outliers

  /**
   * Immutable data class to store weighted position calculation results.
   *
   * <p>This class encapsulates the results of signal strength to weight conversion and position
   * weighting calculations for a single access point.
   *
   * <p>Mathematical Representation: For each AP_i with position (lat_i, lon_i, alt_i) and weight
   * w_i: - weightedLat = lat_i × w_i - weightedLon = lon_i × w_i - weightedAlt = alt_i × w_i (if
   * altitude exists, 0.0 otherwise) - weight = w_i
   */
   ...
   *
   * <p>This class holds the final aggregated weighted sums and total weights before computing the
  ...
  private static final class AggregatedResults {
    ...
    AggregatedResults(
        double totalWeightedLat,
        double totalWeightedLon,
        double totalWeightedAlt,
        double totalWeight,
   ...
   *
   * <p>Mathematical Purpose: O(1) lookup optimization This creates an efficient lookup structure to
   * convert O(n×m) nested loop complexity to O(n+m) for matching scan results to known APs.
   *
   ...
   *
   * <p>Mathematical Process: For each scan result with signal strength RSSI_i: 1. normalized_i =
   * (RSSI_i - RSSI_MAX) / (RSSI_MIN - RSSI_MAX) 2. weight_i = WEIGHT_BASE^normalized_i 3.
   * weightedPosition_i = position_i × weight_i
   *
   * <p>The normalization ensures stronger signals (closer to RSSI_MAX) get higher normalized values
   * and thus exponentially higher weights.
   *
   ...
   *
   * <p>Signal Strength to Weight Conversion: The formula weight = WEIGHT_BASE^normalized creates
   * exponential weighting: - Signal at -30 dBm: normalized = 1.0, weight = 10^1.0 = 10.0 - Signal
   * at -65 dBm: normalized = 0.5, weight = 10^0.5 ≈ 3.16 - Signal at -100 dBm: normalized = 0.0,
   * weight = 10^0.0 = 1.0
   *
   * <p>This exponential relationship ensures that small improvements in signal strength (indicating
   * closer proximity) result in disproportionately higher influence on the final position
   * calculation.
   *
   ...
   */
  private WeightedPositionResult calculateSingleWeightedPosition(
      WifiScanResult scan, WifiAccessPoint ap) {
    if (ap == null) {
      return null;
    }
    ...
    return new WeightedPositionResult(
        weightedLatitude, weightedLongitude, weightedAltitude, weight);
  }

  /**
   * Normalizes signal strength to a 0.0-1.0 range for weight calculation.
   *
   * <p>Mathematical Formula: normalized = (RSSI - RSSI_MAX) / (RSSI_MIN - RSSI_MAX)
   *
   * <p>Boundary Behavior: - RSSI = -30 dBm → normalized = 1.0 (strongest possible signal) - RSSI =
   * -65 dBm → normalized = 0.5 (medium signal strength) - RSSI = -100 dBm → normalized = 0.0
   * (weakest usable signal)
   *
   * <p>Values outside the range are clamped to [0.0, 1.0] to prevent invalid weight calculations.
   *
   ...
   *
   * <p>Mathematical Process: Σ(weightedLat_i), Σ(weightedLon_i), Σ(weightedAlt_i), Σ(weight_i)
   *
   * <p>Uses thread-safe DoubleAdder for parallel aggregation to ensure accurate summation even with
   * concurrent calculations.
   *
   * <p>Special handling for altitude: Only APs with valid altitude data contribute to the altitude
   * calculation and altitudeWeightSum.
   *
   ...
   *
   * <p>Final Position Calculation: - latitude = Σ(weightedLat_i) / Σ(weight_i) - longitude =
   * Σ(weightedLon_i) / Σ(weight_i) - altitude = Σ(weightedAlt_i) / Σ(altitudeWeight_i) or 0.0 if no
   * altitude data
   *
   * <p>Confidence Calculation: confidence = min(MAX_CONFIDENCE, coverage × BASE_CONFIDENCE) where
   * coverage = scanned_APs / known_APs (measures AP visibility)
   *
   ...
   */
  private Position assemblePosition(
      AggregatedResults aggregated, List<WifiAccessPoint> knownAPs, List<WifiScanResult> wifiScan) {
    ...
    double finalAltitude =
        (aggregated.altitudeWeightSum > ZERO_THRESHOLD)
   ...
   *
   * <p>Uses parallel stream for efficiency with large AP datasets. Falls back to
   * DEFAULT_ACCURACY_METERS if no accuracy data is available.
   *
   * @param knownAPs List of known access points
   ...
   *
   * <p>Coverage Ratio Impact: - Higher coverage (more visible APs) increases confidence - Perfect
   * coverage (ratio = 1.0) yields maximum possible confidence - Confidence is capped at
   * MAX_CONFIDENCE to reflect algorithm limitations
   *
   * <p>Mathematical Formula: confidence = min(MAX_CONFIDENCE, (scannedCount / knownCount) ×
   * BASE_CONFIDENCE)
   *
