 ...
 * All calculations are based on established academic principles from satellite
 * navigation theory (GPS/GNSS) adapted for indoor WiFi positioning systems.
 * Key references include Kaplan & Hegarty "Understanding GPS" and IEEE positioning standards.
 */
public class GDOPCalculator {
    ...
    /**
     * Maximum allowed GDOP value for positioning calculations.
     * Rationale: Values above 30 indicate extremely poor geometry where 
     * positioning becomes unreliable. This threshold prevents numerical 
     * instability in downstream calculations.
     * Academic basis: Standard practice in GPS and indoor positioning systems.
     */
    public static final double MAX_ALLOWED_GDOP = 30.0;
    
    /**
     * Minimum number of access points required for GDOP calculation.
     * Mathematical requirement: Need at least 3 APs for 2D positioning geometry.
     * Rationale: GDOP calculation requires overdetermined system (n ≥ dimensions + 1).
     */
     ...
     * Value: 6,371,000 meters - standard value used in geodetic calculations.
     * Rationale: Required for converting latitude/longitude differences to metric distances.
     * Academic source: World Geodetic System 1984 (WGS84) definition.
     */
    private static final double EARTH_RADIUS_METERS = 6_371_000.0;
    
    /**
     * Conversion factor from degrees latitude to meters.
     * Formula: EARTH_RADIUS × π / 180°
     * Rationale: 1 degree of latitude ≈ 111,320 meters (constant globally).
     * Mathematical basis: Arc length = radius × angle_in_radians.
     */
    private static final double DEGREES_LAT_TO_METERS = EARTH_RADIUS_METERS * Math.PI / 180.0;
    
    /**
     * Minimum distance threshold in meters for valid unit vector calculation.
     * Rationale: Prevents division by zero and numerical instability when 
     * AP position is extremely close to estimated position.
     * Academic basis: Common practice in trilateration algorithms to avoid singularities.
     */
    private static final double MIN_DISTANCE_THRESHOLD_METERS = 1.0;
    
    /**
     * Default unit vector component for APs closer than minimum threshold.
     * Rationale: Provides stable fallback vector when distance calculation fails.
     * Mathematical basis: Unit vector [1,0,0] represents positive X-axis direction.
     */
    ...
    /**
     * Bias term value for positioning calculations.
     * Rationale: Standard practice in positioning systems to account for 
     * clock bias and systematic errors.
     * Mathematical basis: Augments geometry matrix for solving unknown bias parameter.
     */
    private static final double BIAS_TERM_VALUE = 1.0;
    
    /**
     * Zero value for safe initialization and fallback operations.
     * Rationale: Explicit constant for mathematical operations requiring zero,
    ...
    /**
     * Index for X-coordinate (latitude) in coordinate arrays.
     * Rationale: Explicit constant for array indexing improves code clarity
     * and reduces magic number usage.
     * Geodetic mapping: X-axis corresponds to latitude in geographic coordinates.
     */
    private static final int X_COORDINATE_INDEX = 0;
    
    /**
     * Index for Y-coordinate (longitude) in coordinate arrays.
     * Rationale: Explicit constant for array indexing improves code clarity.
     * Geodetic mapping: Y-axis corresponds to longitude in geographic coordinates.
     */
    private static final int Y_COORDINATE_INDEX = 1;
    
    /**
     * Index for Z-coordinate (altitude) in coordinate arrays.
     * Rationale: Explicit constant for 3D positioning support.
     * Geodetic mapping: Z-axis corresponds to altitude above reference ellipsoid.
     */
    private static final int Z_COORDINATE_INDEX = 2;
    
    /**
     * Minimum dimensions required for 2D positioning.
     * Rationale: 2D positioning requires latitude and longitude coordinates.
     * Mathematical basis: Planar geometry requires minimum 2 spatial dimensions.
     */
    private static final int MIN_2D_DIMENSIONS = 2;
    
    /**
     * Minimum dimensions required for 3D positioning.
     * Rationale: 3D positioning adds altitude to latitude and longitude.
     * Mathematical basis: Spatial geometry requires 3 dimensions for volume positioning.
     */
    ...
    /**
     * Base factor for excellent geometry (no adjustment needed).
     * Mathematical meaning: Multiplicative identity - no scaling applied.
     * Rationale: Excellent geometry requires no accuracy/confidence degradation.
     * Used in: calculateGDOPFactor for GDOP ≤ EXCELLENT_GDOP threshold.
     */
    private static final double EXCELLENT_GEOMETRY_BASE_FACTOR = 1.0;
    
    /**
     * Increment step for GDOP factor calculation in good geometry range.
     * Mathematical formula: Used in linear interpolation between geometry ranges.
     * Rationale: Provides smooth transition from excellent (1.0) to fair (1.5) range.
     * Academic basis: Linear scaling maintains proportional error relationship.
     */
    private static final double GOOD_GEOMETRY_INCREMENT_STEP = 0.5;
    
    /**
     * Threshold factor between good and fair geometry ranges.
     * Mathematical meaning: Factor = 1.5 represents 50% increase in uncertainty.
     * Rationale: Moderate geometry degradation requires proportional accuracy adjustment.
     * Used in: Transition point between good and fair GDOP ranges.
     */
    private static final double GOOD_TO_FAIR_THRESHOLD_FACTOR = 1.5;
    
    /**
     * Base factor for poor geometry conditions.
     * Mathematical meaning: Factor = 2.0 represents 100% increase in uncertainty.
     * Rationale: Poor geometry significantly impacts positioning reliability.
     * Academic basis: Doubling uncertainty reflects substantial geometric degradation.
     */
    private static final double POOR_GEOMETRY_BASE_FACTOR = 2.0;
    
    /**
     * Divisor for poor GDOP scaling calculation.
     * Mathematical formula: Used in (gdop - FAIR_GDOP) / POOR_GDOP_SCALING_DIVISOR.
     * Rationale: Controls rate of factor increase for very poor geometry.
     * Academic basis: Logarithmic-like scaling prevents excessive penalty for marginally poor geometry.
     */
    private static final double POOR_GDOP_SCALING_DIVISOR = 2.0;
    
    /**
     * Maximum cap for GDOP factor to prevent excessive penalties.
     * Mathematical meaning: Factor = 4.0 represents 300% increase in uncertainty.
     * Rationale: Prevents unlimited factor growth that could make positioning meaningless.
     * Academic basis: Practical limit based on positioning system utility thresholds.
     */
    ...
    /**
     * Coefficient for determinant term in eigenvalue discriminant calculation.
     * Mathematical formula: discriminant = trace² - EIGENVALUE_DETERMINANT_COEFFICIENT × determinant.
     * Rationale: Standard coefficient (4) from quadratic formula for 2×2 matrix eigenvalues.
     * Academic basis: Eigenvalue formula λ₁,λ₂ = (trace ± √(trace² - 4×det))/2.
     */
    private static final int EIGENVALUE_DETERMINANT_COEFFICIENT = 4;
    
    /**
     * Divisor for eigenvalue calculation from discriminant.
     * Mathematical formula: λ₁,λ₂ = (trace ± √discriminant) / EIGENVALUE_CALCULATION_DIVISOR.
     * Rationale: Standard divisor (2) from quadratic formula for characteristic polynomial.
     * Academic basis: Eigenvalues of 2×2 matrix A follow quadratic equation det(A - λI) = 0.
     */
    ...
    /**
     * Base factor for geometric quality when condition number indicates good geometry.
     * Mathematical meaning: Factor = 1.0 means no geometric penalty applied.
     * Rationale: Good geometry (low condition number) requires no accuracy adjustment.
     * Used in: calculateGeometricQualityFactor for excellent geometric conditions.
     */
    private static final double GOOD_GEOMETRY_BASE_FACTOR = 1.0;
    
    /**
     * Threshold factor for transitioning from moderate to poor geometric quality.
     * Mathematical meaning: Factor = 2.0 represents geometric quality degradation threshold.
     * Rationale: Marks point where geometric penalties become significant.
     * Academic basis: Condition number analysis suggests 2× factor for poor geometry transition.
     */
    private static final double GEOMETRIC_POOR_THRESHOLD_FACTOR = 2.0;
    
    /**
     * Minimum constraint value for various geometric calculations.
     * Mathematical meaning: Lower bound = 1.0 for normalized geometric factors.
     * Rationale: Prevents geometric quality factors from improving beyond baseline (1.0).
     * Academic basis: Geometric dilution can only worsen, never improve baseline precision.
     */
     ...
     * 
     * GDOP Algorithm Overview:
     * 1. Validate inputs and ensure sufficient APs
     * 2. Convert geographic coordinates to metric distances
     * 3. Calculate unit vectors from position to each AP
     * 4. Construct geometry matrix H with unit vectors
     * 5. Compute covariance matrix Q = (H^T × H)^(-1)
     * 6. Calculate GDOP = √(trace(Q))
     * 
     * Mathematical Foundation:
     * The geometry matrix H contains unit vectors representing the spatial 
     * relationship between the estimated position and each access point.
     * GDOP quantifies how small errors in distance measurements translate 
     * to errors in position estimation.
     * 
     ...
     * 
     * Validation Rules:
     * - Minimum number of APs required for geometry analysis
     * - Position array must be non-null with valid dimensions
     * - All coordinate arrays must match position dimensions
     * 
    ...
    private static boolean isValidGDOPInput(double[][] coordinates, double[] position) {
        if (coordinates.length < MIN_APS_FOR_GDOP || position == null) {
            logger.debug("Insufficient APs ({}) or null position for GDOP calculation", coordinates.length);
            return false;
        }
        ...
        for (double[] coordinate : coordinates) {
            if (coordinate.length < positionDimensions) {
                logger.warn("Coordinate dimensions ({}) do not match position dimensions ({})", 
                           coordinate.length, positionDimensions);
                return false;
            }
        }
     ...
     * 
     * Matrix Structure:
     * H = [u₁ᵀ; u₂ᵀ; ...; uₙᵀ] where uᵢ = (APᵢ - position) / ||APᵢ - position||
     * 
     * Optional bias term augments matrix: H = [u₁ᵀ 1; u₂ᵀ 1; ...; uₙᵀ 1]
     * 
     * @param coordinates Array of AP coordinates
     ...
     */
    private static double[][] createGeometryMatrix(double[][] coordinates, double[] position, boolean includeBiasTerm) {
     ...
     * 1. Convert lat/lon differences to metric distances using Earth's radius
     * 2. Calculate Euclidean distance in 3D space
     * 3. Normalize vector to unit length: û = Δr / ||Δr||
     * 
     * Coordinate Conversion:
     * - Latitude difference: Δlat × DEGREES_LAT_TO_METERS
     * - Longitude difference: Δlon × DEGREES_LAT_TO_METERS × cos(latitude)  
     * - Altitude difference: Δalt (already in meters)
     * 
     ...
     * 
     * Conversion Formulas:
     * - dx = (lat_AP - lat_pos) × DEGREES_LAT_TO_METERS
     * - dy = (lon_AP - lon_pos) × DEGREES_LAT_TO_METERS × cos(lat_pos)
     * - dz = alt_AP - alt_pos (if 3D positioning)
     * 
     * Mathematical Basis:
     * Uses spherical Earth model with WGS84 radius for accurate distance conversion.
     * Longitude scaling accounts for meridian convergence at different latitudes.
     * 
     * @param apCoordinate AP coordinates [lat, lon, alt]
     * @param position Estimated position [lat, lon, alt]
     * @return Metric distances as record
     */
    private static MetricDistances convertCoordinatesToMeters(double[] apCoordinate, double[] position) {
        double dx = (apCoordinate[X_COORDINATE_INDEX] - position[X_COORDINATE_INDEX]) * DEGREES_LAT_TO_METERS;
        
        // Longitude conversion factor varies with latitude (spherical Earth model)
        double lonToMeters = DEGREES_LAT_TO_METERS * Math.cos(Math.toRadians(position[X_COORDINATE_INDEX]));
        double dy = (apCoordinate[Y_COORDINATE_INDEX] - position[Y_COORDINATE_INDEX]) * lonToMeters;
        
        double dz = position.length > MIN_2D_DIMENSIONS ? 
                   apCoordinate[Z_COORDINATE_INDEX] - position[Z_COORDINATE_INDEX] : ZERO_VALUE;
        
        return new MetricDistances(dx, dy, dz);
    }
    
    /**
     * Creates default unit vector for APs too close to estimated position.
     * Prevents numerical instability in GDOP calculation.
     * 
     * Default Vector Structure:
     * 2D: [1, 0] - Points along positive X-axis (eastward)
     * 3D: [1, 0, 0] - Points along positive X-axis with zero altitude component
     * 
     * Mathematical Rationale:
     * Unit vector [1,0,0] provides stable geometric contribution without
     ...
     * 
     * Normalization Formula: û = Δr / ||Δr||
     * where ||Δr|| is the Euclidean norm of the distance vector.
     * 
     * Vector Components:
     * - X component: dx / euclideanDistance (latitude direction)
     * - Y component: dy / euclideanDistance (longitude direction)  
     * - Z component: dz / euclideanDistance (altitude direction, if 3D)
     * 
     ...
     */
    private static double[] normalizeVector(MetricDistances distances, double euclideanDistance, int dimensions) {
        double[] unitVector = new double[dimensions];
        unitVector[X_COORDINATE_INDEX] = distances.dx() / euclideanDistance;
        if (dimensions > MIN_2D_DIMENSIONS - 1) unitVector[Y_COORDINATE_INDEX] = distances.dy() / euclideanDistance;
        if (dimensions > MIN_3D_DIMENSIONS - 1) unitVector[Z_COORDINATE_INDEX] = distances.dz() / euclideanDistance;
        return unitVector;
    }
     ...
     * 
     * Matrix Operations:
     * 1. Create Apache Commons Math RealMatrix from geometry matrix
     * 2. Compute H^T (transpose of geometry matrix)
     * 3. Calculate H^T × H (normal equations matrix)
     * 4. Compute (H^T × H)^(-1) using QR decomposition
     * 
     * Error Handling:
     ...
     * 
     * GDOP Formula: GDOP = √(trace(Q))
     * where Q is the covariance matrix and trace(Q) = Σ(diagonal elements)
     * 
     * Mathematical Significance:
     * The trace represents the sum of variances along coordinate axes.
     * Square root converts variance measure to standard deviation scale.
     * 
     * Numerical Safety:
     * - Ensures trace is non-negative using max(ZERO_VALUE, trace)
     * - Caps result at MAX_ALLOWED_GDOP to prevent numerical overflow
     * 
     * Academic Reference:
     * This follows standard GDOP calculation methods from satellite navigation
     ...
     * For excellent geometry: factor = EXCELLENT_GEOMETRY_BASE_FACTOR
     * For good geometry: factor = EXCELLENT_GEOMETRY_BASE_FACTOR + GOOD_GEOMETRY_INCREMENT_STEP × 
     *                            ((gdop - EXCELLENT_GDOP) / (GOOD_GDOP - EXCELLENT_GDOP))
     * For fair geometry: factor = GOOD_TO_FAIR_THRESHOLD_FACTOR + GOOD_GEOMETRY_INCREMENT_STEP × 
     *                            ((gdop - GOOD_GDOP) / (FAIR_GDOP - GOOD_GDOP))
     * For poor geometry: factor = min(MAX_GDOP_FACTOR_CAP, POOR_GEOMETRY_BASE_FACTOR + 
    ...
    public static double calculateGDOPFactor(double gdop) {
        if (gdop <= EXCELLENT_GDOP) {
            ...
            double factor = EXCELLENT_GEOMETRY_BASE_FACTOR + GOOD_GEOMETRY_INCREMENT_STEP * 
                           ((gdop - EXCELLENT_GDOP) / (GOOD_GDOP - EXCELLENT_GDOP));
            return factor;
        } else if (gdop <= FAIR_GDOP) {
            // Fair geometry, moderate adjustment
            double factor = GOOD_TO_FAIR_THRESHOLD_FACTOR + GOOD_GEOMETRY_INCREMENT_STEP * 
                           ((gdop - GOOD_GDOP) / (FAIR_GDOP - GOOD_GDOP));
            return factor;
        } else {
     ...
     *    λ1,λ2 = (trace ± √[(trace)² - EIGENVALUE_DETERMINANT_COEFFICIENT×det]) / EIGENVALUE_CALCULATION_DIVISOR
     *    where:
     *    - trace = covLatLat + covLonLon
     *    - det = covLatLat×covLonLon - covLatLon²
     *    - EIGENVALUE_DETERMINANT_COEFFICIENT = 4 (from quadratic formula)
     *    - EIGENVALUE_CALCULATION_DIVISOR = 2 (from quadratic formula)
     * 
     ...
     * 
     * Academic Reference:
     * This follows the standard eigenvalue decomposition method for symmetric 2×2 matrices
     * as described in linear algebra texts (e.g., Strang's "Linear Algebra").
     * 
     ...
     * For collinear cases:
     *   factor = COLLINEAR_BASE_FACTOR + min(GEOMETRIC_MIN_CONSTRAINT, log10(conditionNumber)/COLLINEAR_LOG_SCALE)
     * 
     * For non-collinear cases, piecewise function:
     *   - Good geometry (κ < GOOD_GEOMETRY_THRESHOLD): factor = GOOD_GEOMETRY_BASE_FACTOR
     *   - Moderate geometry (GOOD_GEOMETRY_THRESHOLD ≤ κ < MODERATE_GEOMETRY_THRESHOLD): 
     *     factor = GOOD_GEOMETRY_BASE_FACTOR + (κ - GOOD_GEOMETRY_THRESHOLD)/CONDITION_NUMBER_SCALING_FACTOR
     *   - Poor geometry (κ ≥ MODERATE_GEOMETRY_THRESHOLD): 
     *     factor = GEOMETRIC_POOR_THRESHOLD_FACTOR + min(GEOMETRIC_MIN_CONSTRAINT, (κ - MODERATE_GEOMETRY_THRESHOLD)/POOR_GEOMETRY_SCALING_FACTOR)
     * 
     * Where:
     * - κ is the condition number
     * - The thresholds and scales are constants defining the geometry quality ranges
     * - GEOMETRIC_MIN_CONSTRAINT prevents factors from improving beyond baseline
     * 
     * Academic Basis:
     * The logarithmic scaling for collinear cases follows principles from numerical analysis
     * where condition numbers can grow exponentially with geometric degradation.
     * 
    ...
    public static double calculateGeometricQualityFactor(double conditionNumber, boolean isCollinear) {
        if (isCollinear) {
            ...
            return COLLINEAR_BASE_FACTOR + Math.min(GEOMETRIC_MIN_CONSTRAINT, Math.log10(conditionNumber) / COLLINEAR_LOG_SCALE);
        } else {
            ...
            if (conditionNumber < GOOD_GEOMETRY_THRESHOLD) {
                ...
                return GOOD_GEOMETRY_BASE_FACTOR + (conditionNumber - GOOD_GEOMETRY_THRESHOLD) / CONDITION_NUMBER_SCALING_FACTOR;
            } else {
                // Very poor geometry
                return GEOMETRIC_POOR_THRESHOLD_FACTOR + Math.min(GEOMETRIC_MIN_CONSTRAINT, (conditionNumber - MODERATE_GEOMETRY_THRESHOLD) / POOR_GEOMETRY_SCALING_FACTOR);
            }
        }
    }
