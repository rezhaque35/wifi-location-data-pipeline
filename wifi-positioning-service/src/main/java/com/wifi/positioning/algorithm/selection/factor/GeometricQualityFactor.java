...
/**
 * Enum representing different geometric quality scenarios based on GDOP (Geometric Dilution of
 * Precision) that affect algorithm weights.
 *
 * <p>GEOMETRIC DILUTION OF PRECISION (GDOP) METHODOLOGY:
 *
 * <p>This classification system is based on the industry-standard GDOP ratings used in satellite
 ...
 *
 * <p>INDUSTRY STANDARD REFERENCE: [1] Wikipedia: "Dilution of precision (navigation)"
 * https://en.wikipedia.org/wiki/Dilution_of_precision_(navigation) - Comprehensive overview of GDOP
 * theory and standard classification thresholds
 *
 * <p>STANDARD GDOP CLASSIFICATION (from satellite navigation):
 *
 * <p>| DOP Value | Rating | Description | |-----------|-----------|-------------| | < 1 | Ideal |
 * Highest possible confidence, most sensitive applications | | 1-2 | Excellent | Accurate enough
 * for all but most sensitive applications | | 2-5 | Good | Minimum appropriate for business
 * decisions, reliable navigation | | 5-10 | Moderate | Usable for calculations but fix quality
 * could be improved | | 10-20 | Fair | Low confidence level, very rough estimates only | | > 20 |
 * Poor | Inaccurate measurements, should be discarded |
 *
 * <p>WIFI POSITIONING ADAPTATION: Our implementation adapts these standards for indoor WiFi
 * positioning constraints:
 *
 * <p>EXCELLENT_GDOP (< 2.0): - Equivalent to "Ideal" to "Excellent" satellite navigation ratings -
 * Optimal access point geometric distribution - Enables all positioning algorithms with maximum
 * confidence - Typical scenario: APs well-distributed around user position - Algorithm Impact:
 * Enhances trilateration and maximum likelihood weights (×1.3, ×1.2)
 *
 * <p>GOOD_GDOP (2.0-4.0): - Equivalent to "Good" satellite navigation rating - Good geometric
 * distribution suitable for reliable positioning - Most algorithms perform well with standard
 * weights - Typical scenario: APs reasonably spaced with minor clustering - Algorithm Impact:
 * Standard weights with slight trilateration reduction (×0.9)
 *
 * <p>FAIR_GDOP (4.0-6.0): - Bridging "Good" to "Moderate" satellite navigation ratings - Acceptable
 * geometry but with reduced precision expectations - Favors robust algorithms over precision
 * methods - Typical scenario: APs somewhat clustered or irregularly distributed - Algorithm Impact:
 * Reduces trilateration weight (×0.6), increases centroid weight (×1.2)
 *
 * <p>POOR_GDOP (> 6.0): - Equivalent to "Moderate" and worse satellite navigation ratings - Poor
 * geometric distribution significantly affects accuracy - Only robust algorithms should be used -
 * Typical scenario: APs highly clustered or forming poor geometric patterns - Algorithm Impact:
 * Severely reduces trilateration (×0.3), emphasizes centroid methods (×1.3)
 *
 * <p>COLLINEAR (Special Case): - Not applicable to satellite navigation (satellites never collinear
 * from Earth) - Unique to terrestrial positioning where reference points can align linearly - Makes
 * trilateration mathematically impossible (×0.0 weight) - Requires alternative positioning
 * approaches - Algorithm Impact: Disables trilateration, maximizes weighted centroid (×1.4)
 *
 * <p>THRESHOLD ADAPTATION RATIONALE: - Indoor WiFi positioning has tighter geometric constraints
 * than satellite systems - Lower thresholds (2.0, 4.0, 6.0) vs. satellite standards (2.0, 5.0,
 * 10.0) reflect: * Limited 2D positioning (no altitude diversity like satellites) * Constrained
 * indoor environments with potential obstructions * Need for more conservative quality assessment
 * in confined spaces
 *
 * <p>MATHEMATICAL FOUNDATION: GDOP calculation follows standard navigation principles: - GDOP =
 * √(trace(Q)) where Q = (H^T × H)^(-1) - H is the geometry matrix with unit vectors from position
 * to each AP - Lower GDOP values indicate better geometric distribution - Values scale with
 * position uncertainty magnification factor
 *
 * <p>Based on the algorithm selection framework documentation and adapted from satellite navigation
 * standards for WiFi indoor positioning applications.
 */
public enum GeometricQualityFactor {
  ...
  private static final double COLLINEARITY_THRESHOLD =
      0.0002; // Maximum allowed deviation from line of best fit
  ...
  /**
   * Checks if a list of positions are collinear (lie on the same line). Uses the maximum deviation
   * from the line of best fit to determine collinearity. The method is designed to be robust to
   * small deviations from perfect collinearity.
   *
   ...
   */
  public static boolean checkCollinearity(
   ...
   *
   * <p>This method uses GDOP (Geometric Dilution of Precision) to assess how the geometric
   * configuration of access points affects position accuracy: - EXCELLENT_GDOP: GDOP < 2.0 (optimal
   * AP geometry) - GOOD_GDOP: 2.0 ≤ GDOP < 4.0 (good AP geometry) - FAIR_GDOP: 4.0 ≤ GDOP < 6.0
   * (acceptable AP geometry) - POOR_GDOP: GDOP ≥ 6.0 (poor AP geometry) - COLLINEAR: Special case
   * where APs lie approximately on a straight line
   *
   * <p>The calculation includes: 1. Checking if APs are collinear (arranged in a line) 2.
   * Estimating user position using weighted centroid (weights based on signal strength) 3. Creating
   * an array of AP coordinates 4. Computing GDOP using the GDOPCalculator 5. Mapping the GDOP value
   * to the appropriate GeometricQualityFactor
   *
   * @param wifiScans List of WiFi scan results
   * @param apMap Map of known access points by MAC address
   * @return The corresponding GeometricQualityFactor
   */
  public static GeometricQualityFactor determineGeometricQuality(
      List<WifiScanResult> wifiScans, Map<String, WifiAccessPoint> apMap) {
    ...
    if (totalWeight > 0) {
      ...
      estimatedPosition[0] =
          validAPs.stream().mapToDouble(WifiAccessPoint::getLatitude).average().orElse(0);
      estimatedPosition[1] =
          validAPs.stream().mapToDouble(WifiAccessPoint::getLongitude).average().orElse(0);
    }

    // Create array of AP coordinates for GDOP calculation
    double[][] apCoordinates =
        validAPs.stream()
