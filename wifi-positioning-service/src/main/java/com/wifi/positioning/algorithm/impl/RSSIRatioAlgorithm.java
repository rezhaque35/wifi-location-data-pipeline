 ...
 *
 * <p>This algorithm estimates position using relative signal strength ratios between access points,
 * eliminating the need for absolute signal strength calibration. It's particularly effective in
 * environments where hardware characteristics are similar across access points.
 *
 * <p>MATHEMATICAL FOUNDATION: ======================== The algorithm is based on the principle that
 * signal strength ratios correspond to distance ratios in free space propagation. For two access
 * points with signal strengths RSSI₁ and RSSI₂:
 *
 * <p>Distance Ratio = 10^((RSSI₂ - RSSI₁) / PATH_LOSS_COEFFICIENT)
 *
 * <p>Where PATH_LOSS_COEFFICIENT = 20.0 dB per decade for free space propagation.
 *
 * <p>Position Interpolation: For each AP pair (AP₁, AP₂), the position is interpolated as: P = (P₁
 * + ratio × P₂) / (1 + ratio)
 *
 * <p>Where: - P₁, P₂ are the geographic positions of AP₁ and AP₂ - ratio is the calculated distance
 * ratio from signal strengths - P is the interpolated position weighted by the ratio
 *
 * <p>ALGORITHM CHARACTERISTICS: ========================= USE CASES: - Ideal for scenarios with 2-3
 * access points - Effective when absolute signal calibration is difficult - Works well when APs
 * have similar hardware characteristics - Suitable for dynamic transmit power environments
 *
 * <p>STRENGTHS: - No need for absolute signal strength calibration - Resistant to environmental
 * changes affecting all signals equally - Computationally efficient for small numbers of APs -
 * Handles dynamic transmit power changes effectively
 *
 * <p>WEAKNESSES: - Accuracy decreases with dissimilar AP hardware - Performance degrades with more
 * than 4-5 APs - Sensitive to individual signal fluctuations - Less accurate than trilateration in
 * ideal conditions
 *
 * <p>THREAD SAFETY: ============== This implementation is thread-safe through the use of: -
 * Immutable constants and method parameters - ConcurrentHashMap for AP lookups - AtomicDouble
 * accumulators for parallel calculations - Stream-based parallel processing with proper isolation
 */
@Component
public class RSSIRatioAlgorithm implements PositioningAlgorithm {
  ...
  /**
   * Base confidence level returned by getConfidence() method. Rationale: 0.75 represents high
   * confidence in the algorithm's general capability, while individual position calculations may
   * have lower confidence based on signal quality.
   */
  private static final double BASE_CONFIDENCE = 0.75;

  /**
   * Minimum number of access points required for RSSI ratio calculations. Rationale: At least 2 APs
   * are needed to calculate signal strength ratios. With only 1 AP, no ratio comparison is
   * possible.
   */
   ...
   *
   * <p>Mathematical Foundation: In free space, signal strength decreases by 20 dB per decade of
   * distance. This constant is used in the formula: ratio = 10^((RSSI₁ - RSSI₂) / 20.0)
   *
   * <p>Rationale: 20.0 dB represents the theoretical free space path loss, providing a baseline for
   * signal-to-distance ratio calculations.
   *
   * <p>Reference: Friis transmission equation for free space propagation
   */
   ...
   *
   * <p>Mathematical Purpose: Converts signal strength differences (in dBm) to normalized weights
   * [0,1]. Formula: weight = |RSSI₁ - RSSI₂| / WEIGHT_NORMALIZATION_FACTOR
   *
   * <p>Rationale: 30.0 dB represents a significant signal difference that would receive maximum
   * weight (1.0). Smaller differences receive proportionally lower weights, reducing their
   * influence on the final position calculation.
   *
   * <p>Typical signal differences range from 2-30 dB in real environments.
   */
   ...
   *
   * <p>Rationale: -80.0 dBm represents a moderate signal strength typical in indoor environments.
   * Used as a safe fallback to prevent accuracy calculation failures.
   */
   ...
   *
   * <p>Rationale: 15.0 meters represents a reasonable baseline accuracy for RSSI-based positioning
   * systems in typical indoor environments.
   */
   ...
   *
   * <p>Mathematical Purpose: Used in formula: signalFactor = (-avgSignalStrength -
   * SIGNAL_THRESHOLD) / SCALING_DIVISOR
   *
   * <p>Rationale: -50.0 dBm represents a very strong signal. Signals weaker than this will result
   * in degraded accuracy through the scaling factor calculation.
   */
   ...
   *
   * <p>Mathematical Purpose: Converts signal strength differences to scaling multipliers. Formula:
   * signalFactor = (-avgSignalStrength - 50) / 10.0
   *
   * <p>Rationale: 10.0 dB increments provide reasonable granularity for accuracy scaling. Each 10
   * dB of signal degradation increases the accuracy scaling factor.
   */
   ...
   *
   * <p>Rationale: 1.0 ensures that even the strongest signals don't improve accuracy beyond the
   * base accuracy. Prevents unrealistic accuracy improvements.
   */
   ...
   *
   * <p>Rationale: 3.0 limits accuracy degradation to 3x the base accuracy even for very weak
   * signals. Prevents extremely pessimistic accuracy estimates.
   */
   ...
   *
   * <p>Mathematical Purpose: Used in formula: signalQuality = (signalStrength + SIGNAL_OFFSET) /
   * SIGNAL_RANGE
   *
   * <p>Rationale: 95.0 dBm offset maps typical WiFi signal range [-95, -50] dBm to a normalized
   * range starting from 0. Very weak signals (-95 dBm) become 0.
   */
   ...
   *
   * <p>Mathematical Purpose: Completes the normalization: signalQuality = (signalStrength + 95.0) /
   * 45.0
   *
   * <p>Rationale: 45.0 dB span covers typical WiFi range [-95, -50] dBm. Maps to [0,1] range where
   * 0 = very weak signal, 1 = very strong signal.
   */
   ...
   *
   * <p>Rationale: 0.5 represents moderate signal quality, providing a neutral baseline that neither
   * penalizes nor rewards the confidence calculation.
   */
   ...
   *
   * <p>Rationale: 0.85 represents high but not perfect confidence, acknowledging the inherent
   * limitations of RSSI-based positioning. Prevents overconfidence.
   */
   ...
   *
   * <p>Mathematical Purpose: Used in formula: confidence = baseConfidence + (signalQuality ×
   * CONFIDENCE_BOOST)
   *
   * <p>Rationale: 1.0 provides a direct linear relationship between signal quality and confidence
   * improvement, up to the maximum confidence limit.
   */
   ...
   *
   * <p>Rationale: -70.0 dBm represents a strong signal boundary. Signals stronger than this receive
   * a minimum confidence floor of HIGH_CONFIDENCE_FLOOR.
   */
   ...
   *
   * <p>Rationale: 0.7 ensures that strong signals always maintain high confidence, even if other
   * factors (like geometry) might reduce the calculated confidence.
   */
   ...
   * weaknesses of the RSSI Ratio algorithm: - Works optimally with 2 APs - Good with 3 APs but less
   * effective with more APs - Medium signal quality sensitivity - Moderate impact from geometric
   * quality - Good performance with uniform signals, worse with outliers
   */
  ...
  private static final double RSSI_RATIO_FOUR_PLUS_APS_WEIGHT =
      0.5; // Useful but not optimal for 4+ APs

  // Signal quality multipliers from framework document
  private static final double RSSI_RATIO_STRONG_SIGNAL_MULTIPLIER =
      1.0; // No change with strong signals
  private static final double RSSI_RATIO_MEDIUM_SIGNAL_MULTIPLIER =
      0.9; // Slight reduction with medium signals
  private static final double RSSI_RATIO_WEAK_SIGNAL_MULTIPLIER =
      0.6; // Significant reduction with weak signals
  private static final double RSSI_RATIO_VERY_WEAK_SIGNAL_MULTIPLIER =
      0.0; // ×0.0 for very weak signals

  // Geometric quality multipliers from framework document
  private static final double RSSI_RATIO_EXCELLENT_GDOP_MULTIPLIER =
  ...
  private static final double RSSI_RATIO_FAIR_GDOP_MULTIPLIER =
      0.9; // Slight reduction for fair geometry
  private static final double RSSI_RATIO_POOR_GDOP_MULTIPLIER =
      0.8; // More reduction for poor geometry

  // Signal distribution multipliers from framework document
  private static final double RSSI_RATIO_UNIFORM_SIGNALS_MULTIPLIER =
      1.2; // Significant improvement for uniform signals
  private static final double RSSI_RATIO_MIXED_SIGNALS_MULTIPLIER =
      0.9; // Slight reduction for mixed signals
  private static final double RSSI_RATIO_SIGNAL_OUTLIERS_MULTIPLIER =
      0.7; // Significant reduction for outliers
   ...
   *
   * <p>This class encapsulates the result of processing a single AP pair, including the weighted
   * coordinates and metadata about altitude availability.
   *
  ...
  private static final class WeightedPositionResult {
     ...
     */
    WeightedPositionResult(
        double weightedLat, double weightedLon, double weightedAlt, double weight) {
  ...
  private void validateInputs(List<WifiScanResult> wifiScan, List<WifiAccessPoint> knownAPs) {
    ...
    if (wifiScan.size() < MIN_REQUIRED_APS) {
      throw new IllegalArgumentException(
          "At least " + MIN_REQUIRED_APS + " APs are required for RSSI ratio calculation");
    }
  }
   ...
   *
   * <p>Mathematical Process: For each AP pair (APᵢ, APⱼ): 1. Calculate signal ratio: r = 10^((RSSIᵢ
   * - RSSIⱼ) / 20.0) 2. Calculate position weight: w = |RSSIᵢ - RSSIⱼ| / 30.0 3. Interpolate
   * position: P = (Pᵢ + r × Pⱼ) / (1 + r) 4. Apply weight: weightedP = P × w
   *
   * @param wifiScan list of WiFi scan results
   ...
   *
   * <p>This method processes all AP pairs where the first AP is at the given index and the second
   * AP is at any subsequent index. This ensures each pair is processed exactly once without
   * duplication.
   *
  ...
  private Stream<WeightedPositionResult> createAccessPointPairsForIndex(
    ...
    return IntStream.range(firstApIndex + 1, wifiScan.size())
        ...
        .mapToObj(
            secondApIndex ->
                processAccessPointPair(
                    wifiScan.get(firstApIndex), wifiScan.get(secondApIndex), apMap));
  }

  /**
   * Processes a single AP pair to calculate weighted position contribution.
   *
   * <p>Mathematical Implementation: - Signal Ratio: ratio = 10^((RSSI₁ - RSSI₂) /
   * PATH_LOSS_COEFFICIENT) - Weight: weight = |RSSI₁ - RSSI₂| / WEIGHT_NORMALIZATION_FACTOR -
  ...
  private WeightedPositionResult processAccessPointPair(
    ...
    return new WeightedPositionResult(
        lat * weight, lon * weight, alt * weight, weight, hasAltitudeData);
  }

  /**
   * Interpolates a single coordinate using the signal strength ratio.
   *
   * <p>Mathematical Formula: coordinate = (coord₁ + ratio × coord₂) / (1 + ratio)
   *
   * <p>This formula provides a weighted interpolation where stronger signals (higher ratio) bias
   * the result toward the second coordinate.
   *
   ...
   *
   * <p>Mathematical Model: 1. Calculate average signal strength 2. Calculate base accuracy from AP
   * horizontal accuracy values 3. Scale accuracy based on signal strength: signalFactor = max(1,
   * min(3, (-avgSignal - 50) / 10)) 4. finalAccuracy = baseAccuracy × signalFactor
   *
   * <p>Rationale: Weaker signals lead to less reliable distance estimates, degrading overall
   * position accuracy. The scaling is capped between 1x (no degradation) and 3x (maximum
   * degradation).
   *
   * @param wifiScan list of WiFi scan results
   * @param knownAPs list of known access points
  ...
  private double calculateAccuracy(List<WifiScanResult> wifiScan, List<WifiAccessPoint> knownAPs) {
    ...
    double signalFactor =
        Math.max(
            MIN_SCALING_FACTOR,
            Math.min(
                MAX_SCALING_FACTOR, (-avgSignalStrength + SIGNAL_THRESHOLD) / SCALING_DIVISOR));
   ...
   *
   * <p>Mathematical Model: 1. Signal Quality: normalized signal strength in range [0,1]
   * signalQuality = (signalStrength + SIGNAL_OFFSET) / SIGNAL_RANGE 2. Base Confidence: ratio of
   * actual weights to maximum possible weights baseConfidence = min(0.85, totalWeight /
   * maxPossibleWeight) 3. Enhanced Confidence: baseConfidence + signalQuality 4. Strong Signal
   * Floor: if avgSignal ≥ -70dBm, confidence ≥ 0.7
   *
   * <p>Rationale: Confidence increases with signal quality and weight coverage. Strong signals
   * receive a confidence floor to ensure reliable reporting.
   *
   * @param wifiScan list of WiFi scan results
  ...
  private double calculateConfidence(List<WifiScanResult> wifiScan, double totalWeight) {
    ...
    double signalQuality =
        wifiScan.stream()
            .mapToDouble(
                scan ->
                    Math.min(
                        1.0, Math.max(0.0, (scan.signalStrength() + SIGNAL_OFFSET) / SIGNAL_RANGE)))
            .average()
    ...
    double computedConfidence =
        Math.min(MAX_CONFIDENCE, baseConfidence + (signalQuality * CONFIDENCE_BOOST));
