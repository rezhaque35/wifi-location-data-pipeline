 ...
 *
 * <p>This algorithm implements a probabilistic approach that maximizes the likelihood function
 * P(position | measurements) using gradient descent optimization. It provides the most accurate
 * positioning when sufficient high-quality measurements are available.
 *
 * <p>ALGORITHM OVERVIEW: 1. Creates initial position estimate using weighted centroid method 2.
 * Builds measurement models incorporating signal strength probability distributions 3. Iteratively
 * refines position using gradient descent on log-likelihood function 4. Calculates confidence based
 * on convergence quality and signal characteristics 5. Applies GDOP analysis to refine accuracy
 * estimates
 *
 * <p>MATHEMATICAL MODEL: The algorithm maximizes P(position | measurements) using:
 *
 * <p>1. Likelihood Function: L(pos) = Π P(measurement_i | pos) where each measurement probability
 * follows a normal distribution: P(RSSI | pos) = N(RSSI; μ(d), σ²) - μ(d) = expected RSSI at
 * distance d using path loss model - σ² = signal variance (adaptive based on signal strength)
 *
 * <p>2. Log-Likelihood Maximization: LL(pos) = Σ log(P(measurement_i | pos)) Gradient: ∇LL(pos) = Σ
 * (RSSI_i - μ(d_i))/σ² * ∇d_i
 *
 * <p>3. Position Update using Gradient Descent: pos_new = pos + α * ∇LL(pos) where α is adaptive
 * learning rate that decreases if likelihood doesn't improve
 *
 * <p>4. Geometric Dilution of Precision (GDOP): GDOP = sqrt(trace((H^T * H)^-1)) where H is the
 * geometry matrix containing unit vectors from position to APs
 *
 * <p>USE CASES: - Best suited for environments with many APs (4+ APs optimal) - Effective in
 * complex indoor environments with multipath - Ideal when high accuracy and confidence estimates
 * are required - Good for scenarios with mixed signal quality (handles outliers well)
 *
 * <p>STRENGTHS: - Most accurate algorithm when sufficient data available - Handles noisy
 * measurements robustly through probabilistic modeling - Provides realistic confidence estimates
 * based on convergence - Accounts for AP geometry quality via GDOP analysis - Adaptive signal
 * variance estimation based on signal strength
 *
 * <p>WEAKNESSES: - Computationally intensive due to iterative optimization - Requires more
 * measurements than simpler methods (minimum 4 APs) - May converge slowly in challenging scenarios
...
public class MaximumLikelihoodAlgorithm implements PositioningAlgorithm {
  ...
  /**
   * Spatial resolution for grid-based optimization (meters). Determines the granularity of position
   * search space. Value based on typical indoor positioning requirements.
   */
  private static final double GRID_RESOLUTION_METERS = 1.0;

  /**
   * Maximum number of gradient descent iterations. Prevents infinite loops while allowing
   * sufficient convergence time. Value determined from empirical analysis of convergence patterns.
   */
  private static final int MAX_OPTIMIZATION_ITERATIONS = 100;

  /**
   * Convergence threshold for gradient descent (meters). Algorithm stops when learning rate falls
   * below this value. Balances convergence quality with computational efficiency.
   */
  private static final double CONVERGENCE_THRESHOLD_METERS = 0.1;

  /**
   * Initial learning rate for gradient descent optimization. Higher values enable faster
   * convergence but risk overshooting. Value chosen to balance speed and stability.
   */
  private static final double INITIAL_LEARNING_RATE = 1.0;

  /**
   * Learning rate reduction factor when likelihood doesn't improve. Multiplicative factor applied
   * to reduce step size. Value of 0.5 provides good balance between exploration and exploitation.
   */
  ...
  /**
   * Reference distance for path loss model (meters). Distance at which reference RSSI is measured.
   * Standard value used in WiFi signal propagation modeling.
   */
  private static final double REFERENCE_DISTANCE_METERS = 1.0;

  /**
   * Speed of light in meters per second. Used in free-space path loss calculations. Fundamental
   * physical constant for RF propagation.
   */
  private static final double SPEED_OF_LIGHT_MPS = 299792458.0;

  /**
   * Path loss exponent for signal propagation model. Describes how signal strength decreases with
   * distance. Value of 3.0 represents typical indoor environment with obstacles. - Free space: ~2.0
   * - Indoor with obstacles: 2.5-4.0 - Dense urban: 3.5-5.0
   */
  private static final double PATH_LOSS_EXPONENT = 3.0;

  /**
   * Earth radius in meters for geographic distance calculations. Used in Haversine formula for
   * calculating distances between coordinates. Standard value for Earth's mean radius.
   */
  private static final double EARTH_RADIUS_METERS = 6371000;

  /**
   * Default average signal strength when calculation fails (dBm). Fallback value representing
   * typical weak indoor WiFi signal. Used to prevent errors in edge cases.
   */
  ...
  /**
   * Signal strength threshold for "strong" signals (dBm). Signals above this level have high
   * reliability for distance estimation. Based on empirical analysis of WiFi signal quality vs.
   * positioning accuracy.
   */
  private static final double STRONG_SIGNAL_THRESHOLD_DBM = -60.0;

  /**
   * Signal strength threshold for "weak" signals (dBm). Signals below this level have reduced
   * reliability. Boundary between medium and weak signal categories.
   */
  private static final double WEAK_SIGNAL_THRESHOLD_DBM = -80.0;

  /**
   * Standard deviation for strong WiFi signals (dBm). Represents measurement uncertainty for
   * high-quality signals. Value derived from empirical studies of WiFi signal variation.
   */
  private static final double STRONG_SIGNAL_STD_DEV_DBM = 2.5;

  /**
   * Standard deviation for medium strength WiFi signals (dBm). Default measurement uncertainty for
   * typical indoor environments. Balances between strong and weak signal uncertainties.
   */
  private static final double MEDIUM_SIGNAL_STD_DEV_DBM = 4.0;

  /**
   * Standard deviation for weak WiFi signals (dBm). Higher uncertainty reflects increased noise in
   * weak signals. Accounts for multipath and interference effects.
   */
  ...
  /**
   * Minimum accuracy value for strong signals (meters). Lower bound on position accuracy estimates
   * for high-quality measurements. Represents best-case positioning accuracy achievable.
   */
  private static final double MIN_ACCURACY_METERS = 1.0;

  /**
   * Maximum accuracy value for strong signals (meters). Upper bound on position accuracy for strong
   * signal scenarios. Prevents overly optimistic accuracy claims.
   */
  private static final double MAX_ACCURACY_STRONG_SIGNALS_METERS = 5.0;

  /**
   * Maximum accuracy value for any scenario (meters). Global upper bound on accuracy estimates to
   * prevent unrealistic values. Represents reasonable worst-case accuracy expectation.
   */
  private static final double MAX_ACCURACY_ANY_SCENARIO_METERS = 25.0;

  /**
   * Base accuracy for strong signals in initial estimate (meters). Starting point for accuracy
   * calculation with high-quality signals. Represents middle of expected accuracy range (1-5m) for
   * strong signals.
   */
  private static final double BASE_ACCURACY_STRONG_SIGNALS_METERS = 3.0;

  /**
   * Base accuracy offset for signal-strength based calculation (meters). Used in accuracy formula
   * for medium/weak signals. Provides baseline accuracy before signal-dependent adjustments.
   */
  private static final double BASE_ACCURACY_OFFSET_METERS = 6.0;

  /**
   * Signal strength reference point for accuracy calculation (dBm). Used in formula: baseAccuracy +
   * |signal - reference| * multiplier Represents transition point between good and poor signal
   * quality.
   */
  private static final double ACCURACY_SIGNAL_REFERENCE_DBM = -70.0;

  /**
   * Multiplier for signal strength in accuracy calculation. Determines how much signal quality
   * affects accuracy estimates. Higher values increase sensitivity to signal strength variations.
   */
  ...
  /**
   * Minimum confidence value for any positioning result. Lower bound prevents unrealistically low
   * confidence estimates. Represents minimum useful confidence for positioning applications.
   */
  private static final double MIN_CONFIDENCE = 0.6;

  /**
   * Maximum confidence value for any positioning result. Upper bound prevents overconfident
   * estimates. Leaves room for uncertainty even in ideal conditions.
   */
  private static final double MAX_CONFIDENCE = 0.95;

  /**
   * Minimum confidence threshold for strong signals. Ensures high-quality measurements maintain
   * high confidence. Represents expected confidence for good positioning scenarios.
   */
  private static final double HIGH_CONFIDENCE_THRESHOLD = 0.8;

  /**
   * Maximum confidence cap for weak signals. Prevents weak signals from claiming high confidence.
   * Reflects increased uncertainty with poor signal quality.
   */
  private static final double WEAK_SIGNAL_CONFIDENCE_CAP = 0.65;

  /**
   * Default likelihood convergence factor. Used when likelihood calculation fails or produces
   * invalid results. Represents neutral convergence quality.
   */
  private static final double DEFAULT_LIKELIHOOD_FACTOR = 0.7;

  /**
   * Very weak signal threshold for confidence calculation (dBm). Signals below this level receive
   * minimal confidence. Represents boundary of useful signal information.
   */
  private static final double VERY_WEAK_SIGNAL_THRESHOLD_DBM = -100.0;

  /**
   * Minimum AP count for meaningful AP count factor calculation. Below this count, AP count factor
   * is zero. Represents minimum APs needed for reliable positioning.
   */
  private static final int MIN_AP_COUNT_FOR_FACTOR = 2;

  /**
   * Maximum AP count for AP count factor scaling. AP count factor scales from 0 (at MIN_AP_COUNT)
   * to 1 (at this value). Represents point where additional APs provide diminishing returns.
   */
  private static final int MAX_AP_COUNT_FOR_FACTOR = 8;

  /**
   * Weight for signal quality in confidence calculation. Determines relative importance of signal
   * strength vs. other factors. Higher values prioritize signal quality in confidence estimates.
   */
  private static final double CONFIDENCE_SIGNAL_WEIGHT = 0.7;

  /**
   * Weight for AP count in confidence calculation (strong signals). Balances signal quality with
   * geometric diversity. Lower weight reflects signal quality dominance.
   */
  private static final double CONFIDENCE_AP_COUNT_WEIGHT_STRONG = 0.3;

  /**
   * Weight for AP count in confidence calculation (weak signals). Higher weight for weak signals
   * emphasizes geometric diversity. Compensates for reduced signal quality information.
   */
  private static final double CONFIDENCE_AP_COUNT_WEIGHT_WEAK = 0.2;

  /**
   * Weight for AP count in confidence calculation (medium signals). Balanced weight between strong
   * and weak signal scenarios. Provides moderate emphasis on geometric diversity.
   */
  private static final double CONFIDENCE_AP_COUNT_WEIGHT_MEDIUM = 0.25;

  /**
   * Weight for likelihood convergence in confidence calculation (strong signals). Lower weight
   * reflects reduced importance when signals are strong. Strong signals provide good positioning
   * regardless of convergence.
   */
  private static final double CONFIDENCE_LIKELIHOOD_WEIGHT_STRONG = 0.1;

  /**
   * Weight for likelihood convergence in confidence calculation (weak signals). Higher weight
   * emphasizes convergence quality for poor signals. Good convergence can partially compensate for
   * weak signals.
   */
  private static final double CONFIDENCE_LIKELIHOOD_WEIGHT_WEAK = 0.1;

  /**
   * Weight for likelihood convergence in confidence calculation (medium signals). Balanced weight
   * providing moderate convergence emphasis. Reflects intermediate dependence on optimization
   * quality.
   */
  private static final double CONFIDENCE_LIKELIHOOD_WEIGHT_MEDIUM = 0.15;

  /**
   * Lower bound for likelihood factor normalization. Used in formula: (exp(likelihood/count) -
   * lower) / (upper - lower) Represents minimum meaningful likelihood value.
   */
  private static final double LIKELIHOOD_NORMALIZATION_LOWER_BOUND = 0.1;

  /**
   * Upper bound for likelihood factor normalization. Completes normalization range for likelihood
   * factor calculation. Represents maximum meaningful likelihood value.
   */
   ...
   * weaknesses of the Maximum Likelihood algorithm: - Works best with 4+ APs (optimal with many
   * APs) - Highly dependent on signal quality (works best with strong signals) - Moderately
   * sensitive to geometric quality - Extremely effective with mixed signals and outliers
   */

  // AP Count weights - Maximum Likelihood requires multiple APs for effectiveness
  private static final double MAXIMUM_LIKELIHOOD_SINGLE_AP_WEIGHT =
      0.0; // Not applicable for single AP

  private static final double MAXIMUM_LIKELIHOOD_TWO_APS_WEIGHT = 0.0; // Not applicable for two APs
  private static final double MAXIMUM_LIKELIHOOD_THREE_APS_WEIGHT =
      0.0; // Not applicable for three APs (needs more APs)
  private static final double MAXIMUM_LIKELIHOOD_FOUR_PLUS_APS_WEIGHT =
      1.0; // Optimal for four+ APs

  // Signal quality multipliers - Algorithm performance varies significantly with signal quality
  private static final double MAXIMUM_LIKELIHOOD_STRONG_SIGNAL_MULTIPLIER =
      1.2; // Significant improvement with strong signals
  private static final double MAXIMUM_LIKELIHOOD_MEDIUM_SIGNAL_MULTIPLIER =
      0.9; // Slight reduction with medium signals
  private static final double MAXIMUM_LIKELIHOOD_WEAK_SIGNAL_MULTIPLIER =
      0.5; // Major reduction for weak signals
  private static final double MAXIMUM_LIKELIHOOD_VERY_WEAK_SIGNAL_MULTIPLIER =
      0.0; // ×0.0 for very weak signals

  // Geometric quality multipliers - Moderate sensitivity to AP geometric distribution
  private static final double MAXIMUM_LIKELIHOOD_EXCELLENT_GDOP_MULTIPLIER =
      1.2; // Significant boost for excellent geometry
  private static final double MAXIMUM_LIKELIHOOD_GOOD_GDOP_MULTIPLIER =
      1.1; // Good boost for good geometry
  private static final double MAXIMUM_LIKELIHOOD_FAIR_GDOP_MULTIPLIER =
      0.9; // Some reduction for fair geometry
  private static final double MAXIMUM_LIKELIHOOD_POOR_GDOP_MULTIPLIER =
      0.7; // Significant reduction for poor geometry

  // Signal distribution multipliers - Algorithm handles mixed signals and outliers well
  private static final double MAXIMUM_LIKELIHOOD_UNIFORM_SIGNALS_MULTIPLIER =
      0.9; // Slightly reduced with uniform signals
  private static final double MAXIMUM_LIKELIHOOD_MIXED_SIGNALS_MULTIPLIER =
      1.1; // Some improvement with mixed signals
  private static final double MAXIMUM_LIKELIHOOD_SIGNAL_OUTLIERS_MULTIPLIER =
      1.2; // Significant improvement with outliers

  /**
   * Calculates position using Maximum Likelihood Estimation.
   *
   * <p>This method orchestrates the complete maximum likelihood positioning process: 1. Input
   * validation and preprocessing 2. Initial position estimation using weighted centroid 3.
   * Measurement model creation for likelihood calculations 4. Iterative position refinement using
   * gradient descent 5. Final accuracy and confidence calculation with GDOP analysis
   *
  ...
  public Position calculatePosition(List<WifiScanResult> wifiScan, List<WifiAccessPoint> knownAPs) {
    ...
    OptimizationResult optimizationResult =
        optimizePositionUsingGradientDescent(initialEstimate, measurements);
  ...
  /**
   * Validates input parameters for position calculation. Ensures that both WiFi scan results and
   * known AP lists are valid and non-empty.
   *
  ...
  /**
   * Creates a lookup map from known access points for efficient access during calculations. Uses
   * concurrent map for thread-safe operations during parallel processing. In case of duplicate MAC
   * addresses, keeps the first occurrence.
   *
   ...
   *
   * <p>The optimization process iteratively improves the position estimate by: 1. Computing the
   * gradient of the log-likelihood function 2. Updating position in the direction of steepest
   * ascent 3. Adapting learning rate based on likelihood improvement 4. Converging when learning
   * rate falls below threshold
   *
   * <p>Mathematical Foundation: - Gradient: ∇LL(pos) = Σ (RSSI_i - μ(d_i))/σ² * ∇d_i - Update:
   * pos_new = pos + α * ∇LL(pos) - Learning rate adaptation: α_new = α *
   * LEARNING_RATE_REDUCTION_FACTOR (if no improvement)
   *
   ...
   */
  private OptimizationResult optimizePositionUsingGradientDescent(
      Position initialPosition, List<MeasurementModel> measurements) {
   ...
   *
   * <p>Applies gradient ascent update rule: pos_new = pos_old + learningRate * gradient
   *
   ...
   */
  private Position updatePositionWithGradient(
      Position currentPosition, double[] gradient, double learningRate) {
    return new Position(
   ...
   *
   * <p>This method performs the final steps of position calculation: 1. Prepares coordinate arrays
   * for GDOP calculation 2. Calculates Geometric Dilution of Precision (GDOP) 3. Computes average
   * signal strength for accuracy/confidence 4. Refines accuracy estimate using GDOP factor 5.
   ...
   */
  private Position finalizePositionWithGDOPAnalysis(
      OptimizationResult optimizationResult,
    ...
    double gdop =
        GDOPCalculator.calculateGDOP(
            gdopCoordinates.coordinates, gdopCoordinates.position, gdopCoordinates.is3D);
    ...
    double refinedAccuracy =
        calculateAccuracy(bestPosition.accuracy(), gdopFactor, avgSignalStrength);
  ...
  /**
   * Prepares coordinate arrays and position data for GDOP calculation. Handles both 2D and 3D
   * scenarios based on altitude data availability.
   *
   ...
   */
  private GDOPCoordinates prepareGDOPCoordinates(
      Position position, List<MeasurementModel> measurements) {
    ...
    if (is3D) {
      positionArray = new double[] {position.latitude(), position.longitude(), position.altitude()};
    } else {
      positionArray = new double[] {position.latitude(), position.longitude()};
    }
  ...
  /**
   * Calculates average signal strength from WiFi scan results. Uses stream processing for efficient
   * calculation with fallback to default value.
   *
  ...
  /**
   * Data class to hold GDOP calculation coordinates and metadata. Encapsulates coordinate arrays,
   * position data, and dimensionality information.
   */
  ...
  /**
   * Data class to hold optimization results from gradient descent. Encapsulates the best position
   * found and its associated likelihood value.
   */
   ...
   *
   * <p>This method creates a starting point for gradient descent optimization by: 1. Computing
   * weighted centroid of AP locations using signal strength as weights 2. Calculating initial
   * accuracy based on signal quality and GDOP 3. Setting preliminary confidence for further
   * refinement
   *
   * <p>The weighted centroid approach gives stronger signals more influence on the initial
   * position, providing a reasonable starting point that is typically close to the optimal
   * solution.
   *
   * <p>Mathematical Foundation: - Weight calculation: w(i) = 10^(RSSI(i)/10) (converts dBm to
   * linear scale) - Position: lat = Σ(lat(i) * w(i)) / Σ(w(i)), similar for lon and alt - GDOP
   * analysis for geometry quality assessment
   *
   ...
   * @return Initial position estimate with preliminary accuracy and confidence, or null if
   *     calculation fails
   */
  private Position calculateInitialEstimate(
    ...
    Position initialPosition =
        new Position(
            weightedResult.latitude, weightedResult.longitude, weightedResult.altitude, 0.0, 0.0);
   ...
   *
   * <p>Uses exponential weighting based on signal strength to give stronger signals more influence
   * on position estimation. Handles both 2D and 3D positioning based on altitude data availability.
   *
   * @param wifiScan List of WiFi scan results
   * @param apMap Map of known AP locations
   * @return WeightedPositionResult containing calculated position and metadata, or null if no valid
   *     APs
   */
  private WeightedPositionResult calculateWeightedPosition(
      List<WifiScanResult> wifiScan, Map<String, WifiAccessPoint> apMap) {
    ...
    return new WeightedPositionResult(
        latitude, longitude, altitude, totalSignal.doubleValue(), has3DData);
  }

  /**
   * Calculates initial accuracy estimate based on signal strength and geometric quality.
   *
   * <p>Uses different calculation strategies for strong vs. weak signals: - Strong signals: Fixed
   * base accuracy with controlled GDOP adjustment - Weak signals: Signal-dependent base accuracy
   * with full GDOP adjustment
   *
   * <p>Mathematical Foundation: - Strong signals: accuracy = BASE_ACCURACY * (1 + (gdopFactor - 1)
   * * GDOP_MULTIPLIER) - Weak signals: accuracy = (BASE_OFFSET + |signal - reference| * multiplier)
   * * gdopFactor
   *
  ...
  private double calculateInitialAccuracy(double avgSignalStrength, double gdopFactor) {
    ...
    if (avgSignalStrength >= STRONG_SIGNAL_THRESHOLD_DBM) {
      ...
      baseAccuracy =
          BASE_ACCURACY_OFFSET_METERS
              + Math.abs(avgSignalStrength - ACCURACY_SIGNAL_REFERENCE_DBM)
                  * ACCURACY_SIGNAL_MULTIPLIER;
    }
    ...
    if (avgSignalStrength >= STRONG_SIGNAL_THRESHOLD_DBM) {
      ...
      initialAccuracy =
          baseAccuracy * (1.0 + (gdopFactor - 1.0) * GDOPCalculator.GDOP_ACCURACY_MULTIPLIER);
    } else {
    ...
    return Math.max(
        MIN_ACCURACY_METERS, Math.min(MAX_ACCURACY_ANY_SCENARIO_METERS, initialAccuracy));
  }

  /**
   * Data class to hold weighted position calculation results. Encapsulates position coordinates,
   * signal data, and 3D capability flag.
   */
  private static class WeightedPositionResult {
    ...
    WeightedPositionResult(
        double latitude, double longitude, double altitude, double totalSignal, boolean has3DData) {
  ...
  /**
   * Creates measurement models for each AP observation. Models incorporate: 1. Historical signal
   * strength patterns 2. AP-specific standard deviations 3. Location confidence from AP database 4.
   * Operating frequency for accurate path loss calculation
   *
   * @param wifiScan List of WiFi scan results
   * @param apMap Map of known AP locations
   * @return List of measurement models for gradient descent
   */
  private List<MeasurementModel> createMeasurementModels(
      List<WifiScanResult> wifiScan, Map<String, WifiAccessPoint> apMap) {
  ...
  /**
   * Calculates the gradient of the log-likelihood function. This drives the gradient descent
   * optimization by: 1. Computing partial derivatives for each coordinate 2. Weighting
   * contributions by measurement confidence 3. Combining gradients from all measurements 4. Using
   * frequency-dependent expected RSSI calculation
   *
   * @param position Current position estimate
   * @param measurements List of measurement models
   * @return Gradient vector [dLat, dLon, dAlt]
   */
  private double[] calculateLogLikelihoodGradient(
      Position position, List<MeasurementModel> measurements) {
    // These arrays will be updated by multiple threads, so we need thread-safe handling
    double[] gradient = new double[3];
    final Object lock = new Object();

    measurements.parallelStream()
        .forEach(
            m -> {
              double distance =
                  calculateDistance(
  ...
  /**
   * Calculates the log-likelihood of a position given the measurements. Higher values indicate
   * better fit to the measurements. Incorporates: 1. Frequency-dependent signal strength error
   * terms 2. Measurement standard deviations 3. AP location confidence weights
   *
  ...
  /**
   * Calculates confidence based on multiple factors: 1. Signal strength quality 2. Number of access
   * points 3. Likelihood convergence quality 4. Geometric distribution of APs (GDOP)
   *
   * <p>The confidence calculation uses a multi-factor approach that adapts to signal quality: -
   * Strong signals: Emphasizes signal quality with minor GDOP adjustment - Medium signals: Balanced
   * consideration of all factors - Weak signals: Emphasizes AP count and convergence quality
   *
   * <p>Mathematical Foundation: Base confidence = MIN_CONF + (MAX_CONF - MIN_CONF) * (signalWeight
   * * signalFactor + apWeight * apCountFactor + likelihoodWeight * likelihoodFactor)
   *
   * <p>Final confidence = baseConfidence * (1.0 - GDOP_WEIGHT * (1.0 - 1.0/gdopFactor))
   *
  ...
  private double calculateConfidence(
    ...
    ConfidenceFactors factors =
        calculateConfidenceFactors(maxLikelihood, measurements.size(), avgSignalStrength, apCount);
   ...
   *
   * <p>Each factor represents a different aspect of positioning quality: - Signal factor: Quality
   * based on average signal strength - AP count factor: Diversity based on number of access points
   ...
   */
  private ConfidenceFactors calculateConfidenceFactors(
      double maxLikelihood, int measurementCount, double avgSignalStrength, int apCount) {
    ...
    double apCountFactor =
        Math.min(
            1.0,
            Math.max(
                0.0,
                (apCount - MIN_AP_COUNT_FOR_FACTOR)
                    / (double) (MAX_AP_COUNT_FOR_FACTOR - MIN_AP_COUNT_FOR_FACTOR)));
   ...
   *
   * <p>Uses different scaling approaches for strong vs. weak signals: - Strong signals: Scale
   * relative to weak signal threshold - Weak signals: Scale relative to very weak signal threshold
   *
   ...
   *
   * <p>Evaluates how well the optimization converged by examining the final likelihood value.
   ...
   *
   * <p>Different signal categories use different weighting strategies: - Strong signals: Emphasize
   * signal quality, minimal likelihood weight - Medium signals: Balanced weights across all factors
  ...
  private double calculateBaseConfidence(ConfidenceFactors factors, SignalCategory signalCategory) {
    ...
    switch (signalCategory) {
      case STRONG:
        ...
        weightedScore =
            CONFIDENCE_SIGNAL_WEIGHT * factors.signalFactor
   ...
   *
   * <p>The adjustment strength varies by signal category: - Strong signals: Minor adjustment to
   * maintain high confidence - Weak signals: Stronger adjustment reflecting geometric sensitivity -
   * Medium signals: Balanced adjustment
   *
   * <p>Formula: confidence * (1.0 - GDOP_WEIGHT * (1.0 - 1.0/gdopFactor))
   *
   ...
   */
  private double applyGDOPAdjustment(
      double baseConfidence, double gdopFactor, SignalCategory signalCategory) {
    // Apply GDOP adjustment
    double adjustedConfidence =
        baseConfidence
            * (1.0
                - GDOPCalculator.GDOP_CONFIDENCE_WEIGHT * (1.0 - 1.0 / Math.max(1.0, gdopFactor)));
  ...
  /**
   * Data class to hold calculated confidence factors. Encapsulates the three main factors that
   * contribute to confidence estimation.
   */
  ...
  /**
   * Calculates expected RSSI at a given distance using the Close-In (CI) free-space reference
   * distance model.
   *
   * <p>This implementation follows the CI path loss model established by Rappaport et al. (2015)
   * and Sun et al. (2016) for wireless communication systems. The model uses a 1-meter reference
   * distance with 0 dBm transmit power assumption for the following academically-supported reasons:
   *
   * <p>1. MATHEMATICAL INVARIANCE: Maximum likelihood estimators are invariant to constant additive
   * terms (Van Trees, 2001). The choice of reference transmit power only affects the absolute RSSI
   * scale, not the relative positioning accuracy or likelihood function optimization.
   *
   * <p>2. STANDARDIZATION: The CI model with 1m reference distance and 0 dBm provides a
   * standardized approach widely adopted in 5G channel modeling (Sun et al., 2016; 3GPP TR 38.901).
   * Formula: RSSI = FSPL(f,1m) + 10×n×log₁₀(d) + shadow_fading where FSPL(f,1m) = 32.4 +
   * 20×log₁₀(f_GHz) dB
   *
   * <p>3. PARAMETER STABILITY: Studies show CI models exhibit superior parameter stability across
   * frequencies and distances compared to floating-intercept models (Sun et al., 2016).
   *
   * <p>4. COMPUTATIONAL SIMPLICITY: Using 0 dBm reference allows direct computation without needing
   * actual AP transmit power knowledge, which is often unavailable or varies.
   *
   * <p>5. POSITIONING ACCURACY: Indoor positioning studies (Bruno et al., 2014; Wang et al., 2021)
   * demonstrate that relative RSSI differences (fingerprinting basis) are unaffected by absolute
   * transmit power assumptions.
   *
   * <p>References: - Rappaport et al., "Wideband millimeter-wave propagation measurements...", IEEE
   * Trans. Comm., 2015 - Sun et al., "Investigation of Prediction Accuracy...", IEEE Trans. Veh.
   * Tech., 2016 - Bruno et al., "Indoor Positioning in Wireless Local Area Networks...", Sci. World
   * J., 2014 - Van Trees, "Detection, Estimation, and Modulation Theory", Wiley, 2001
   *
  ...
  /**
   * Calculates expected RSSI using default frequency for backward compatibility. Uses 2.4 GHz as
   * default frequency when frequency information is not available.
   *
  ...
  /**
   * Calculates 3D distance between two points using Haversine formula. Accounts for Earth's
   * curvature in horizontal distance. When altitude data is missing, falls back to 2D distance
   * calculation.
   *
  ...
  private double calculateDistance(
      double lat1, double lon1, Double alt1, double lat2, double lon2, Double alt2) {
  ...
  /**
   * Measurement model for a single access point. Encapsulates all information needed for likelihood
   * calculation: - AP location (lat, lon, alt) - Signal strength (RSSI) - Measurement uncertainty
   * (stdDev) - Location confidence from database - Operating frequency for accurate path loss
   * modeling
   */
  ...
  /**
   * Calculates accuracy based on base accuracy value, GDOP factor, and signal strength. The
   * calculation differs for strong vs. weak signals:
   *
   * <p>For strong signals: accuracy = baseAccuracy * (1.0 + (gdopFactor - 1.0) *
   * GDOP_ACCURACY_MULTIPLIER)
   *
   * <p>For weaker signals: accuracy = baseAccuracy * gdopFactor
   *
   * <p>This ensures that GDOP has a controlled effect on strong signals (maintaining high accuracy)
  ...
  private double calculateAccuracy(
      double baseAccuracy, double gdopFactor, double avgSignalStrength) {
    double refinedAccuracy;
    ...
    if (avgSignalStrength >= STRONG_SIGNAL_THRESHOLD_DBM) {
      ...
      refinedAccuracy =
          Math.max(
              MIN_ACCURACY_METERS, Math.min(MAX_ACCURACY_STRONG_SIGNALS_METERS, refinedAccuracy));
    } else {
