 ...
 *
 * <p>This controller handles HTTP requests for position calculation based on WiFi scan results. It
 * follows a response-based error handling approach where all errors are returned as structured
 * WifiPositioningResponse objects rather than throwing exceptions.
 *
 * <p>HTTP Status Code Mapping: - 200 OK: For all responses, with error details in response body
 * when applicable - 400 Bad Request: For validation errors and malformed requests - 500 Internal
...
public class PositioningController {
   ...
   *
   * <p>This method maps application-level success/error states to HTTP status codes: - SUCCESS
   * responses → 200 OK - ERROR responses → 200 OK (error details are in response body)
   *
   * <p>Rationale: Using 200 OK for all structured responses allows clients to consistently parse
   * the response body for error details. HTTP error codes are reserved for protocol-level issues
  ...
  private HttpStatus determineHttpStatus(WifiPositioningResponse response) {
    ...
    // This approach treats positioning errors as application-level responses rather than HTTP
    // errors
