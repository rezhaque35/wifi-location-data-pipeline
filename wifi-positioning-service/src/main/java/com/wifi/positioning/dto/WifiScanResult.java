 ...
 *
 * <p>Contains information about a WiFi access point detected during scanning:
 * - MAC address (required): Unique identifier in XX:XX:XX:XX:XX:XX format
 * - Signal strength (required): Measured in dBm, typically -100 to 0
 * - Frequency (optional): Operating frequency in MHz, defaults to 2400 MHz if not provided
...
public record WifiScanResult(
    ...
    /**
     * Compact constructor for default value assignment.
     * Sets default frequency if not provided. Field validation is handled by Jakarta validation annotations.
     */
    public WifiScanResult {
        // Set default frequency to 2400 MHz if not provided
        if (frequency == null) {
            frequency = 2400;
        }
    }
