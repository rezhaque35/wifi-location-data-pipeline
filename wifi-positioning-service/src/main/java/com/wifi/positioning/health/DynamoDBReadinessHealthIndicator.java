 ...
 * 
 * This health indicator checks if the service is ready to handle requests
 * by verifying that the DynamoDB dependency is accessible and functional
 * through the WifiAccessPointRepository layer.
 * 
 * Benefits of Repository-Based Approach:
 * - Separation of concerns: Health checking logic is separated from data access
 * - Testability: Easier to mock and test without actual DynamoDB dependencies
 ...
 * - DynamoDB connection status through repository validation
 * - Table accessibility and read permissions
 * - Response time measurements from repository operations
 * - Item count validation for data availability
 * - Last check timestamp for monitoring
 * - Error details when connection fails
 * 
 * The readiness check follows Kubernetes readiness probe best practices:
 * - Returns UP when repository reports healthy status and good performance
 * - Returns DOWN when repository reports unhealthy status or performance issues
 * - Returns OUT_OF_SERVICE for unexpected errors or configuration issues
 * 
 * Mathematical Formula for Response Time Measurement:
 * The response time is measured by the repository layer using:
 * response_time_ms = (end_time_nanos - start_time_nanos) / 1,000,000
 * 
 * Where:
 * - start_time_nanos: System.nanoTime() before repository operation
 * - end_time_nanos: System.nanoTime() after repository operation
 * - response_time_ms: Difference converted to milliseconds for readability
 * 
 * The response time helps monitor DynamoDB performance and can indicate
 * network latency, DynamoDB throttling, or other performance issues.
 */
@Component("dynamoDBReadiness")
public class DynamoDBReadinessHealthIndicator implements HealthIndicator {

    private static final Logger logger = LoggerFactory.getLogger(DynamoDBReadinessHealthIndicator.class);

    // Database Information Constants
    /**
     * Database type identifier for DynamoDB.
     * Used in health check responses to identify the database type.
     */
    private static final String DATABASE_TYPE = "DynamoDB";

    // Health Status Messages
    /**
     * Status message indicating DynamoDB is accessible.
     * Used when the DynamoDB readiness health check passes.
     */
    private static final String DYNAMODB_ACCESSIBLE_MESSAGE = "DynamoDB is accessible";
    
    /**
     * Status message indicating DynamoDB is not accessible.
     * Used when the DynamoDB readiness health check fails due to connection issues.
     */
    private static final String DYNAMODB_NOT_ACCESSIBLE_MESSAGE = "DynamoDB is not accessible";
    
    /**
     * Status message indicating DynamoDB table was not found.
     * Used when the DynamoDB readiness health check fails due to missing table.
     */
    private static final String DYNAMODB_TABLE_NOT_FOUND_MESSAGE = "DynamoDB table not found";
    
    /**
     * Status message indicating repository is not configured.
     * Used when the DynamoDB readiness health check fails due to missing repository configuration.
     */
    private static final String REPOSITORY_NOT_CONFIGURED_MESSAGE = "Repository not configured";
    
    /**
     * Status message indicating an unexpected error occurred during health check.
     * Used when the health check encounters an unexpected exception.
     */
    ...
    /**
     * Repository for accessing WiFi access point data and performing health checks.
     * This repository abstracts the DynamoDB access and provides health validation methods.
     */
     ...
     * 
     * This method delegates health checking to the repository layer, which provides:
     * 1. Table accessibility validation
     * 2. Response time measurement
     * 3. Item count verification for read permissions
     * 4. Comprehensive error handling
     * 
     * The health check process:
     * 1. Validate repository configuration
     * 2. Delegate to repository.validateTableHealth()
     * 3. Evaluate repository health result
     * 4. Return appropriate Spring Boot Actuator Health status
     * 
     * Repository Health Evaluation Logic:
     * - UP: Repository reports healthy status (good performance and accessibility)
     * - DOWN: Repository reports unhealthy status (poor performance or accessibility issues)
     * - DOWN: Repository throws ResourceNotFoundException (table not found)
     * - DOWN: Repository throws DynamoDbException (connection/permission issues)
     * - OUT_OF_SERVICE: Repository throws unexpected exceptions
     * 
     * Response Time Measurement:
     * The repository layer handles response time measurement using high-precision timing.
     * This health indicator adds minimal overhead by measuring the total time including
     * repository method call overhead.
     * 
    ...
    public Health health() {
        ...
        try {
            ...
            if (result.isHealthy()) {
                logger.debug("Repository health check successful - Table: {}, Response time: {}ms, Item count: {}", 
                        result.tableName(), result.responseTimeMs(), result.itemCount());
                ...
                logger.warn("Repository health check indicates poor performance - Table: {}, Response time: {}ms, Status: {}", 
                        result.tableName(), result.responseTimeMs(), result.statusMessage());
            ...
            logger.warn("DynamoDB table not found during repository health check (response time: {}ms)", responseTimeMs);
            ...
            logger.error("DynamoDB connection error during repository health check (response time: {}ms)", responseTimeMs, e);
            ...
            logger.error("Unexpected error during repository health check (response time: {}ms)", responseTimeMs, e);
