...
/**
 * Validates WiFi signal strengths against physical laws and constraints. Used to detect physically
 * impossible scenarios in signal strength relationships.
 *
 * <p>This validator is especially important for scenarios like Test Case 39, which tests the
 * system's ability to detect physically impossible signal combinations: - A very strong signal (-40
 * dBm) from one AP - Combined with very weak signals (-90 dBm, -95 dBm) from other APs on the same
 * frequency
 *
 * <p>In real-world situations, this combination would violate RF propagation physics because: 1.
 * WiFi signals at the same frequency attenuate consistently with distance 2. Nearby receivers
 * should observe relatively similar signal strengths 3. A strong signal mixed with very weak
 * signals on the same frequency suggests either signal reflection/interference or data corruption
 *
 * <p>When these physics violations are detected, the system returns an ERROR response rather than
 * calculating an incorrect position.
 */
@Component
public class SignalPhysicsValidator {

  private static final double SIGNAL_STRENGTH_RANGE_THRESHOLD =
      45.0; // Maximum reasonable dBm difference for same frequency
  ...
  private static final double MIN_SIGNAL_STRENGTH =
      -100.0; // Minimum detectable WiFi signal strength

  /**
   * Validates if the given set of WiFi scan results violates physical laws.
   *
   * <p>Checks for: 1. Signals outside realistic bounds 2. Physically impossible signal strength
   * relationships 3. Unrealistic signal strength variations within same frequency
   *
  ...
  public boolean isPhysicallyPossible(List<WifiScanResult> scanResults) {
    ...
    Map<Integer, List<WifiScanResult>> apsByFrequency =
        scanResults.stream().collect(Collectors.groupingBy(WifiScanResult::frequency));
