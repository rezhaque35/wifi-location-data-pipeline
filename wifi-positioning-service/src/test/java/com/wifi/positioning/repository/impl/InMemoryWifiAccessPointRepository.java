...
/**
 * In-memory implementation of the WifiAccessPointRepository interface for testing. This
 * implementation does not require any external dependencies like DynamoDB. Since the real DynamoDB
 * table only has a partition key (mac_addr), each MAC address can only have one entry, which is
 * reflected in this implementation.
 */
@Profile("test")
public class InMemoryWifiAccessPointRepository
    implements WifiAccessPointRepository, TestWifiAccessPointRepository {
  ...
  /**
   * Find an access point by MAC address and version. Since the table only has a partition key, this
   * method checks if the stored access point matches the requested version.
   *
  ...
  /**
   * Save an access point to the repository. Since the table only has a partition key, this replaces
   * any existing entry for the same MAC address.
   *
  ...
  /**
   * Delete an access point by MAC address and version. Since the table only has a partition key,
   * this deletes the entry only if the version matches.
   *
   * @param macAddress MAC address of the access point
   * @param version Version identifier
   */
   ...
   */
  @Override
  public List<WifiAccessPoint> findNearestByCoordinates(
    ...
    return dataStore.values().stream()
        ...
        .sorted(
            Comparator.comparingDouble(
                ap -> calculateDistance(latitude, longitude, ap.getLatitude(), ap.getLongitude())))
        .limit(limit)
        .collect(Collectors.toList());
  }
  ...
  /**
   * Helper method for tests to add access points to the in-memory store. Since table only has
   * partition key, this replaces any existing entry for the MAC address.
   */
  ...
  /**
   * Validates table accessibility and measures response time for health checks. In-memory
   * implementation always returns healthy status with minimal latency.
   *
