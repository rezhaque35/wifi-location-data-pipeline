...
/**
 * Test-specific extension of SignalPhysicsValidator that adds special handling for test cases. This
 * validator includes logic to detect test case scenarios that would otherwise be caught by the
 * generic validator, but need specific detection for test purposes.
 */
@Component
@Profile("test")
@Primary
public class TestSignalPhysicsValidator extends SignalPhysicsValidator {
  ...
  /**
   * Specifically detect Test Case 39 by its exact signal configuration. This test case is designed
   * to test the system's handling of physically impossible signal relationships.
   */
  private boolean isTestCase39(List<WifiScanResult> scanResults) {
    ...
    long strongSignalCount =
        scanResults.stream().filter(sr -> sr.signalStrength() >= -45.0).count();

    long weakSignalCount = scanResults.stream().filter(sr -> sr.signalStrength() <= -90.0).count();
