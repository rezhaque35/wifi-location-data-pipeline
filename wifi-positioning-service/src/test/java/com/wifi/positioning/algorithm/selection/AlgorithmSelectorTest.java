 ...
 *
 * <p>Tests the three-phase algorithm selection process: 1. Hard Constraints (Disqualification
 * Phase) 2. Algorithm Weighting (Ranking Phase) 3. Finalist Selection (Combination Phase)
 */
...
class AlgorithmSelectorTest {
  ...
  class HardConstraintsPhaseTests {
    ...
    void singleAPDisqualification() {
      ...
      assertTrue(
          weights.containsKey(proximityAlgorithm),
          "Proximity algorithm should be selected for single AP");
      ...
      assertFalse(
          weights.containsKey(rssiRatioAlgorithm),
          "RSSI Ratio should be disqualified for single AP");
      assertFalse(
          weights.containsKey(weightedCentroidAlgorithm),
          "Weighted Centroid should be disqualified for single AP");
      assertFalse(
          weights.containsKey(trilaterationAlgorithm),
          "Trilateration should be disqualified for single AP");
      assertFalse(
          weights.containsKey(maximumLikelihoodAlgorithm),
          "Maximum Likelihood should be disqualified for single AP");

      // Verify disqualification reasons
      for (PositioningAlgorithm algorithm :
          Arrays.asList(
              rssiRatioAlgorithm,
        ...
        assertTrue(
            reasons.get(algorithm).stream().anyMatch(reason -> reason.contains(DISQUALIFIED)),
      ...
      assertTrue(proximityWeight >= 0.4, "Proximity should have weight >= 0.4 for single AP");
    }
    ...
    void twoAPDisqualification() {
      ...
      assertFalse(
          weights.containsKey(trilaterationAlgorithm),
          "Trilateration should be disqualified for two APs");
      assertFalse(
          weights.containsKey(maximumLikelihoodAlgorithm),
          "Maximum Likelihood should be disqualified for two APs");

      // Verify disqualification reasons
      assertTrue(
          reasons.get(trilaterationAlgorithm).stream()
              .anyMatch(reason -> reason.contains(DISQUALIFIED)),
          "Trilateration should have DISQUALIFIED reason");
      assertTrue(
          reasons.get(maximumLikelihoodAlgorithm).stream()
              .anyMatch(reason -> reason.contains(DISQUALIFIED)),
    ...
    void collinearAPDisqualification() {
      ...
      assertFalse(
          weights.containsKey(trilaterationAlgorithm),
          "Trilateration should be disqualified for collinear APs");
      ...
      assertTrue(
          weights.containsKey(rssiRatioAlgorithm),
          "RSSI Ratio should be allowed for collinear APs");

      // Weighted Centroid should always be included for poor geometry
      assertTrue(
          weights.containsKey(weightedCentroidAlgorithm), "Weighted Centroid should be allowed");
    ...
    void extremelyWeakSignalDisqualification() {
      ...
      assertTrue(
          reasons.containsKey(proximityAlgorithm),
          "Proximity algorithm should have reasons for extremely weak signals");
      ...
      if (!weights.containsKey(proximityAlgorithm)) {
        ...
        assertEquals(
            1, weights.size(), "Only Proximity should be selected for extremely weak signals");
      }
    }
    ...
    void extremelyWeakSignalProximityPrioritization() {
      ...
      boolean hasVeryWeakSignalReason =
          reasons.get(proximityAlgorithm).stream()
              .anyMatch(reason -> reason.contains("weak") || reason.contains("prioritized"));

      assertTrue(
          hasVeryWeakSignalReason || proximityWeight >= 0.4,
  ...
  class AlgorithmWeightingPhaseTests {
    ...
    void fourAPsStrongSignalWeighting() {
      ...
      assertTrue(
          weights.containsKey(maximumLikelihoodAlgorithm), "Maximum Likelihood should be selected");
    ...
    void fourAPsWeakSignalWeighting() {
      ...
      assertTrue(
          weights.get(weightedCentroidAlgorithm)
              > weights.getOrDefault(trilaterationAlgorithm, 0.0),
          "Weighted Centroid should have higher weight than Trilateration for weak signals");

      assertTrue(
          weights.get(weightedCentroidAlgorithm)
              > weights.getOrDefault(maximumLikelihoodAlgorithm, 0.0),
    ...
    void poorGeometryAdjustmentTest() {
      ...
      // Trilateration Base weight 1.0, Signal Quality (Medium) = 0.8, Geometric Quality (Poor) =
      // 0.3
      // Expected Trilateration weight = 1.0 * 0.8 * 0.3 = 0.24
      // Weighted Centroid Base weight 0.8, Signal Quality (Medium) = 1.0, Geometric Quality (Poor)
      // = 1.3
      ...
      assertTrue(
          weightedCentroidWeight > trilaterationWeight,
      ...
      assertTrue(
          trilaterationWeight < 0.4, "Trilateration should have low weight for poor geometry");

      // Weighted Centroid weight should be high for poor geometry
      assertTrue(
          weightedCentroidWeight > 0.8,
    ...
    void extremelyWeakSignalWithContextBuilder() {
      ...
      assertEquals(
          APCountFactor.THREE_APS,
          context.getApCountFactor(),
      ...
      for (PositioningAlgorithm algorithm :
        ...
        if (weights.containsKey(algorithm)) {
          logger.info(
              "Algorithm {} was not disqualified despite very weak signals", algorithm.getName());
        }

        assertTrue(
            reasons.get(algorithm).stream()
                .anyMatch(reason -> reason.contains(DISQUALIFIED) || reason.contains("weak")),
            algorithm.getName()
                + " should have a disqualification reason or weight below threshold");
      }
    }
  }
  ...
  class FinalistSelectionPhaseTests {
    ...
    void highWeightSelection() {
      ...
      assertTrue(
          weights.size() <= 3, "For high weight algorithms, should select at most 3 algorithms");
    ...
    void weightThresholdTest() {
      ...
      assertTrue(
          hasWeightedCentroid,
          "Weighted Centroid should be selected for weak signals with poor geometry");
