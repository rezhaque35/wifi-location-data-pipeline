...
/**
 * Test suite for the Weighted Centroid positioning algorithm.
 * These tests verify the algorithm's ability to calculate positions using weighted averages
 * of AP positions based on signal strengths. The test suite covers core functionality,
 * edge cases, and real-world scenarios.
 * 
 * Key Test Areas:
 * 1. Basic Functionality - Core algorithm behavior
 * 2. Input Validation - Error handling
 * 3. Position Calculation - Accuracy and weighting
 * 4. Signal Strength Weighting - Weight calculation verification
 * 5. Scaling - Performance with many APs
 * 6. Confidence - Coverage-based confidence calculation
 */
...
class WeightedCentroidAlgorithmTest {
    ...
    class BasicFunctionalityTests {
         ...
         * The weighted centroid algorithm uses a base confidence of 0.7,
         * which is then adjusted based on AP coverage.
         */
        @Test
     ...
     * 1. Calculate weighted positions correctly
     * 2. Handle unknown APs gracefully
     * 3. Process signal strength variations appropriately
     */
    @Nested
    @DisplayName("Position Calculation Tests")
    class PositionCalculationTests {
        /**
         * Tests the core weighted centroid calculation.
         * Verifies that:
         * 1. Position lies between APs
         * 2. Weighting follows signal strength normalization
         * 3. Position is biased toward weaker signals due to formula
         * Expected: Position between APs, weighted by normalized signal strengths
         */
        @Test
        ...
        void shouldCalculateWeightedCentroidCorrectly() {
            ...
            assertTrue(position.longitude() > 1.5, 
                "Position should be weighted towards AP2 due to weaker signal getting more weight");
        }

        /**
         * Tests handling of unknown APs in scan results.
         * Verifies that:
         * 1. Unknown APs are properly filtered out
         * 2. Position is calculated using only known APs
         * 3. Weighting remains correct for known APs
         * Expected: Valid position using only known AP data
         */
        @Test
        ...
        /**
         * Tests behavior when no matching APs are found.
         * Expected: Return null as position cannot be calculated
         */
        @Test
    ...
    /**
     * Tests for signal strength weighting calculations.
     * These tests verify the algorithm's signal normalization
     * and weighting behavior under various signal conditions.
     */
    @Nested
    @DisplayName("Signal Strength Weighting Tests")
    class SignalStrengthWeightingTests {
        /**
         * Tests the signal strength normalization formula.
         * Verifies that:
         * 1. Weaker signals get more weight due to normalization
         * 2. Position is biased toward APs with weaker signals
         * 3. Formula behaves correctly at signal strength extremes
         * Expected: Position biased toward weaker signals due to normalization formula
         */
        @Test
        ...
        void shouldGiveMoreWeightToStrongerSignals() {
            List<WifiAccessPoint> knownAPs = Arrays.asList(
                createAP("AP1", 1.0, 1.0, 10.0, 5.0),
                createAP("AP2", 10.0, 10.0, 20.0, 5.0)
            );
            ...
            assertTrue(position.latitude() > 5.0,
                "Position should be weighted towards AP2 due to weaker signal getting more weight");
            assertTrue(position.longitude() > 5.0,
                "Position should be weighted towards AP2 due to weaker signal getting more weight");
        }
    ...
    class ScalingTests {
        ...
        void shouldHandleLargeNumberOfAccessPoints() {
            ...
            List<WifiAccessPoint> knownAPs = IntStream.range(0, 100)
                .mapToObj(i -> createAP(
                    "AP" + i, 
            ...
            List<WifiScanResult> scans = IntStream.range(0, 100)
                .mapToObj(i -> createScan(
                    "AP" + i, 
                    -50.0 - (i / 5.0))) // Signal strength decreases as i increases
                .collect(Collectors.toList());
            ...
            assertTrue(position.latitude() >= 0.0 && position.latitude() <= 0.9,
                "Position latitude should be within the AP grid");
            assertTrue(position.longitude() >= 0.0 && position.longitude() <= 0.9,
            ...
            assertTrue(position.latitude() > 0.4, 
