 ...
 *
 * <p>This algorithm uses signal strength to estimate distances from APs and then performs
 * trilateration to determine position. The test suite verifies: 1. Basic functionality and input
 * validation 2. Trilateration calculations with various AP configurations 3. Signal strength to
 * distance conversion accuracy 4. Handling of geometric edge cases (collinear APs) 5. Performance
 * with many APs
 *
 * <p>Key Test Areas: - Basic Functionality: Core algorithm behavior and validation - Input
 * Validation: Error handling and edge cases - Trilateration Calculation: Position estimation
 * accuracy - Signal Strength Handling: Distance estimation from RSSI - Geometric Scenarios: Various
 * AP arrangements
 */
...
class TrilaterationAlgorithmTest {
  ...
  class BasicFunctionalityTests {
     ...
     * the hybrid positioning system. Expected: Returns "trilateration" as the algorithm name.
     */
    @Test
    ...
    /**
     * Validates that confidence values are within valid range (0-1). The trilateration algorithm
     * uses a base confidence that is then adjusted based on geometric dilution of precision (GDOP)
     * and signal strength quality. Expected: Confidence between 0 and 1, typically around 0.85 for
     * ideal conditions.
     */
    @Test
  ...
  /**
   * Tests for the core trilateration calculation functionality. These tests verify the algorithm's
   * ability to: 1. Calculate positions using least squares trilateration 2. Handle various AP
   * geometric configurations 3. Provide accurate position estimates with good AP coverage
   */
  @Nested
  @DisplayName("Trilateration Calculation Tests")
  class TrilaterationCalculationTests {
    /**
     * Tests trilateration with three APs in an ideal triangular arrangement. This is the optimal
     * case for trilateration, with: 1. Good geometric distribution (triangle) 2. Clear signal
     * strength differences 3. Reasonable distances between APs
     *
     * <p>Expected: - Position should be within the triangle formed by the APs - Confidence should
     * be high due to good geometry - Accuracy should reflect signal strength quality
     */
    @Test
    ...
    void shouldCalculatePositionWith3APsInTriangle() {
      ...
      List<WifiScanResult> scans =
          Arrays.asList(
              createScan("AP1", -60.0), createScan("AP2", -70.0), createScan("AP3", -65.0));
      ...
      assertTrue(
          Math.abs(position.altitude() - 10.0) < 5.0, "Altitude should be approximately 10.0");
    ...
    /**
     * Tests position calculation with many APs for improved accuracy. Verifies that: 1. Algorithm
     * can handle redundant measurements 2. More APs generally improve accuracy 3. Position estimate
     * favors stronger signals 4. Confidence increases with more measurements
     *
     * <p>Expected: - Position should be near the center AP (strongest signal) - Confidence should
     * be higher than with just 3 APs - Accuracy should be better than minimum case
     */
    @Test
    ...
    /**
     * Tests handling of collinear APs (geometric singularity). This is a challenging case for
     * trilateration because: 1. Solution is not unique (symmetric about AP line) 2. Matrix
     * operations may be unstable 3. Position ambiguity increases error
     *
     * <p>Expected: - Algorithm should either return null or - Return a position with increased
     * uncertainty - Confidence should be lower than normal
     */
    @Test
  ...
  /**
   * Tests for signal strength to distance conversion accuracy. These tests verify that the
   * algorithm correctly: 1. Converts RSSI to distance estimates 2. Handles signal strength
   * variations 3. Provides reasonable distance estimates
   */
  @Nested
  @DisplayName("Signal Strength to Distance Tests")
  class SignalStrengthToDistanceTests {
    /**
     * Tests distance estimation from signal strength. Verifies that: 1. Stronger signals indicate
     * shorter distances 2. Distance estimates are reasonable 3. Path loss model is applied
     * correctly
     *
     * <p>Expected: - Stronger signals should result in smaller distance estimates - Distance
     * estimates should be within reasonable bounds - Equal signals should give similar distances
     */
    @Test
    ...
    void shouldEstimateReasonablePositionBasedOnSignalStrength() {
      ...
      List<WifiScanResult> equalScans =
          Arrays.asList(
              createScan("AP1", -65.0), createScan("AP2", -65.0), createScan("AP3", -65.0));
      ...
      List<WifiScanResult> strongAP1Scans =
          Arrays.asList(
      ...
      assertTrue(
          strongAP1Position.latitude() != equalPosition.latitude()
  ...
  class AccuracyAndConfidenceRangeTests {
    ...
    void shouldReturnLowerAccuracyAndConfidenceForWeakSignals() {
      ...
      List<WifiScanResult> scans =
          Arrays.asList(
              createScan("AP1", -85.0), createScan("AP2", -88.0), createScan("AP3", -90.0));
      Position result = algorithm.calculatePosition(scans, aps);
      assertNotNull(result);
  ...
  class IncompleteDataHandlingTests {
    /**
     * Tests algorithm behavior with missing altitude data. This tests the 2D positioning fallback
     * capability of the algorithm.
     *
     * <p>Expected outcome: - Algorithm should still calculate position using 2D coordinates -
     * Position should have reasonable accuracy and confidence - Altitude should be 0.0 when all
     * altitude data is missing
     */
    @Test
    ...
    void shouldCalculate2DPositionWhenAltitudeDataIsMissing() {
      ...
      assertEquals(
          0.0, result.altitude(), "Altitude should be 0.0 when all APs have null altitude");
    ...
    void shouldCalculatePositionWithMixed2DAnd3DData() {
      ...
      assertTrue(
          result.altitude() >= 10.0 && result.altitude() <= 15.0,
