...
class WiFiMeasurementsPublisherTest {
  ...
  @Mock private Consumer<List<byte[]>> batchConsumer; // Now consumes JSON byte arrays
  ...
  void setUp() throws Exception {
    ...
    publisher = new WiFiMeasurementsPublisher(firehoseProperties, batchConsumer, objectMapper);
  }
  ...
  void shouldAccumulateRecordsUntilBatchSizeBytesLimit() throws Exception {
    ...
    when(objectMapper.writeValueAsBytes(any(WifiMeasurement.class)))
        .thenReturn(
            ("{\"bssid\":\"00:11:22:33:44:55\",\"data\":\"" + "x".repeat(300) + "\"}")
                .getBytes()); // ~340 byte JSON string
