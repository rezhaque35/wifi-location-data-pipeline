 ...
 * 
 * Tests both success and failure scenarios, verifies metrics and core functionality.
 */
@Testcontainers
class SqsMessageReceiverIntegrationTest {

    private static final Logger logger = LoggerFactory.getLogger(SqsMessageReceiverIntegrationTest.class);
    ...
    void shouldProcessValidS3EventSuccessfully() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(10))
                .until(() -> {
                    ...
                    logger.info("Test metrics - Received: {}, Processed: {}, Deleted: {}, Queue count: {}", 
                            receivedCounter.count(), processedCounter.count(), deletedCounter.count(), getMessageCount());
    ...
    void shouldHandleInvalidJsonMessage() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(10))
                .until(() -> {
                    ...
                    logger.info("Test metrics - Failed: {}, Deleted: {}, Queue count: {}", 
                            failedCounter.count(), deletedCounter.count(), getMessageCount());
    ...
    void shouldHandleMultipleMessagesInBatch() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(15))
                .until(() -> {
                    ...
                    logger.info("Test metrics - Processed: {}, Failed: {}, Deleted: {}, Queue count: {}", 
                            processedCounter.count(), failedCounter.count(), deletedCounter.count(), getMessageCount());
    ...
    void shouldHandleEmptyQueueGracefully() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(5))
                .until(() -> messageReceiver.isRunning());
    ...
    void shouldProcessLargeBatchOfMessages() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(30))
                .until(() -> {
                    ...
                    logger.info("Test metrics - Processed: {}, Failed: {}, Deleted: {}, Queue count: {}, Expected: P={}, F={}, D={}", 
                            processedCounter.count(), failedCounter.count(), deletedCounter.count(), getMessageCount(),
                            expectedProcessed, expectedFailed, messageCount);
        ...
        assertEquals(expectedProcessed, processedCounter.count(), "Should have processed correct number of valid messages");
        assertEquals(expectedFailed, failedCounter.count(), "Should have failed correct number of invalid messages");
    ...
    private SqsMessageReceiver createTestMessageReceiver() {
        ...
        SqsConfigurationProperties testConfig = new SqsConfigurationProperties(
                queueUrl,           // queueUrl
                10,                 // maxMessages
        ...
        String sampleWifiScanJson = "{\"wifiConnectedEvents\":[{\"timestamp\":1731091615562,\"eventId\":\"test-id\",\"eventType\":\"CONNECTED\",\"wifiConnectedInfo\":{\"bssid\":\"b8:f8:53:c0:1e:ff\",\"ssid\":\"TestNetwork\",\"rssi\":-58},\"location\":{\"latitude\":40.6768816,\"longitude\":-74.416391,\"accuracy\":100.0}}]}";
        ...
        String base64EncodedCompressedData = java.util.Base64.getEncoder().encodeToString(byteStream.toByteArray());
        
        InputStream inputStream = new java.io.ByteArrayInputStream((base64EncodedCompressedData + "\n").getBytes());
        software.amazon.awssdk.services.s3.model.GetObjectResponse getObjectResponse = software.amazon.awssdk.services.s3.model.GetObjectResponse.builder().build();
        software.amazon.awssdk.core.ResponseInputStream<software.amazon.awssdk.services.s3.model.GetObjectResponse> responseInputStream = 
            new software.amazon.awssdk.core.ResponseInputStream<>(getObjectResponse, inputStream);
        
        when(mockS3Client.getObject(any(software.amazon.awssdk.services.s3.model.GetObjectRequest.class))).thenReturn(responseInputStream);
        ...
        com.wifi.measurements.transformer.config.properties.S3ConfigurationProperties s3Config = 
            new com.wifi.measurements.transformer.config.properties.S3ConfigurationProperties("test-region", "test-bucket", 1048576L, 30, 60);
        
        com.wifi.measurements.transformer.config.properties.DataFilteringConfigurationProperties.MobileHotspotConfiguration mobileHotspotConfig = 
            new com.wifi.measurements.transformer.config.properties.DataFilteringConfigurationProperties.MobileHotspotConfiguration(
                false, java.util.Set.of("00:00:00"), 
                com.wifi.measurements.transformer.config.properties.DataFilteringConfigurationProperties.MobileHotspotAction.LOG_ONLY
            );
        
        com.wifi.measurements.transformer.config.properties.DataFilteringConfigurationProperties filteringConfig = 
            new com.wifi.measurements.transformer.config.properties.DataFilteringConfigurationProperties(
        ...
        com.wifi.measurements.transformer.service.S3FileProcessorService s3FileProcessorService = 
            new com.wifi.measurements.transformer.service.S3FileProcessorService(mockS3Client, s3Config);
        ...
        com.wifi.measurements.transformer.service.DataValidationService dataValidationService = 
            new com.wifi.measurements.transformer.service.DataValidationService(filteringConfig, new io.micrometer.core.instrument.simple.SimpleMeterRegistry());
        com.wifi.measurements.transformer.service.WifiDataTransformationService wifiDataTransformationService = 
            new com.wifi.measurements.transformer.service.WifiDataTransformationService(dataValidationService, filteringConfig);
        
        // Create Firehose dependencies for DefaultFeedProcessor
        com.wifi.measurements.transformer.service.WiFiMeasurementsPublisher mockWiFiMeasurementsPublisher = 
            mock(com.wifi.measurements.transformer.service.WiFiMeasurementsPublisher.class);
        
        com.wifi.measurements.transformer.config.properties.FirehoseConfigurationProperties firehoseConfig = 
        ...
        DefaultFeedProcessor realFeedProcessor =
            new DefaultFeedProcessor(
                s3FileProcessorService, dataDecodingService, wifiDataTransformationService,
                mockWiFiMeasurementsPublisher, objectMapper
            );
        ...
        MessageProcessor messageProcessor = new MessageProcessor(s3EventExtractor, feedProcessorFactory);
        ...
        return new SqsMessageReceiver(sqsAsyncClient, testConfig, messageProcessor, mockSqsMonitoringService, meterRegistry);
    }
    ...
    private void sendMessageToQueue(String messageBody) {
        ...
        logger.info("Sent message to queue: {}", messageBody.substring(0, Math.min(100, messageBody.length())));
    }
