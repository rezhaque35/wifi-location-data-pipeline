 ...
 * 
 * <p>This health indicator provides comprehensive readiness checks for the SQS queue,
 * ensuring the service is ready to receive traffic and process messages. It performs
 * detailed connectivity checks, queue attribute validation, and processing readiness
 * assessment based on the pattern from the wifi-scan-queue-consumer implementation.</p>
 * 
 * <p><strong>Readiness Health Checks:</strong></p>
 * <ul>
 *   <li><strong>Queue Connectivity:</strong> Basic SQS queue accessibility and responsiveness</li>
 *   <li><strong>Queue Attributes:</strong> Comprehensive queue configuration validation</li>
 *   <li><strong>Message Visibility:</strong> Queue message counts and visibility timeout status</li>
 *   <li><strong>Processing Readiness:</strong> Service readiness for message processing</li>
 *   <li><strong>Connection Stability:</strong> Recent connection success tracking</li>
 * </ul>
 * 
 * <p><strong>Health Status Criteria:</strong></p>
 * <ul>
 *   <li><strong>UP:</strong> Queue is accessible, responsive, and ready for processing</li>
 *   <li><strong>DOWN:</strong> Queue is unreachable, misconfigured, or processing not ready</li>
 * </ul>
 * 
 * <p><strong>Readiness Validation:</strong></p>
 * <ul>
 *   <li>Queue URL accessibility and valid configuration</li>
 *   <li>AWS credentials and permissions validation</li>
 *   <li>Message processing pipeline readiness</li>
 *   <li>Recent connection success within acceptable timeframe</li>
 * </ul>
 */
@Component("sqs")
public class SqsHealthIndicator implements HealthIndicator {
    ...
    public SqsHealthIndicator(SqsClient sqsClient, SqsConfigurationProperties sqsConfig, 
    ...
    public Health health() {
        try {
            ...
            String approximateMessages = response.attributes().get(QueueAttributeName.APPROXIMATE_NUMBER_OF_MESSAGES);
            String approximateNotVisible = response.attributes().get(QueueAttributeName.APPROXIMATE_NUMBER_OF_MESSAGES_NOT_VISIBLE);
            String visibilityTimeout = response.attributes().get(QueueAttributeName.VISIBILITY_TIMEOUT);
            String messageRetention = response.attributes().get(QueueAttributeName.MESSAGE_RETENTION_PERIOD);
            ...
            boolean isReady = queueConnected && queueAccessible && 
                            (metrics.getConsecutiveConnectionFailures() < 3);
