...
/**
 * Configuration for AWS Kinesis Data Firehose client.
 * Provides async clients for both AWS and LocalStack environments.
 */
@Configuration
...
public class FirehoseConfiguration {
    ...
    /**
     * Creates a single Firehose async client that works for all environments.
     * Uses region from application.yml and automatically detects LocalStack endpoint.
     */
    @Bean
    public FirehoseAsyncClient firehoseAsyncClient(
        ...
        var builder = FirehoseAsyncClient.builder()
    ...
    /**
     * Creates a synchronous Firehose client for health checks and monitoring.
    ...
    /**
     * Creates a Consumer function that writes pre-serialized JSON byte arrays to Firehose.
     * This is used by WiFiMeasurementsPublisher to handle batch writing.
     */
    @Bean
    public Consumer<List<byte[]>> firehoseBatchConsumer(FirehoseIntegrationService firehoseIntegrationService) {
        ...
        return jsonRecords -> {
            if (jsonRecords != null && !jsonRecords.isEmpty()) {
                ...
                firehoseIntegrationService.writeBatch(jsonRecords)
