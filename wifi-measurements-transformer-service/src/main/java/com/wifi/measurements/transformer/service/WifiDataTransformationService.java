 ...
 * 
 * <p>This service implements the primary data transformation logic for the WiFi location data pipeline.
 * It processes both WiFi connected events and scan results, applying comprehensive validation,
 * filtering, and quality assessment rules to ensure data integrity and relevance.</p>
 * 
 * <p><strong>Transformation Process:</strong></p>
 * <ol>
 *   <li><strong>Data Validation:</strong> Validates BSSID format, RSSI values, and location data</li>
 *   <li><strong>Mobile Hotspot Detection:</strong> Identifies and optionally excludes mobile hotspot data</li>
 *   <li><strong>Quality Assessment:</strong> Calculates quality weights based on signal strength and link speed</li>
 *   <li><strong>Data Normalization:</strong> Standardizes BSSID format and cleans SSID values</li>
 *   <li><strong>Measurement Creation:</strong> Builds comprehensive measurement records with metadata</li>
 * </ol>
 * 
 * <p><strong>Business Rules:</strong></p>
 * <ul>
 *   <li>Connected events receive higher quality weights than scan results</li>
 *   <li>Low link speeds trigger quality weight adjustments</li>
 *   <li>Mobile hotspots can be excluded based on configuration</li>
 *   <li>Invalid or missing data is filtered out</li>
 *   <li>Device IDs are generated consistently for tracking</li>
 * </ul>
 * 
 * <p><strong>Data Sources:</strong></p>
 * <ul>
 *   <li><strong>WiFi Connected Events:</strong> Active connection data with detailed network information</li>
 *   <li><strong>Scan Results:</strong> Passive network discovery data from WiFi scans</li>
 * </ul>
 * 
 * <p>This service is designed to be stateless and thread-safe, allowing for concurrent
 * processing of multiple data streams while maintaining data consistency.</p>
 * 
...
public class WifiDataTransformationService {
     ...
     * 
     * <p>This constructor initializes the service with the validation service for data quality
     * checks and filtering configuration for business rule application. The service uses
     * dependency injection to ensure loose coupling and testability.</p>
     * 
     * @param validationService Service responsible for validating WiFi data quality and detecting mobile hotspots
    ...
    public WifiDataTransformationService(
        ...
        logger.info("WiFi Data Transformation Service initialized with validation service and filtering configuration");
    }
    
    /**
     * Transforms WiFi scan data into a stream of validated and normalized WiFi measurements.
     * 
     * <p>This method is the main entry point for data transformation. It processes both
     * WiFi connected events and scan results, applying comprehensive validation and
     * transformation rules to create high-quality measurement records.</p>
     * 
     * <p><strong>Processing Steps:</strong></p>
     * <ol>
     *   <li><strong>Input Validation:</strong> Ensures input data is not null</li>
     *   <li><strong>Device ID Generation:</strong> Creates consistent device identifier</li>
     *   <li><strong>Event Processing:</strong> Transforms connected events into measurements</li>
     *   <li><strong>Scan Processing:</strong> Transforms scan results into measurements</li>
     *   <li><strong>Stream Combination:</strong> Merges both data streams into single output</li>
     * </ol>
     * 
     * <p><strong>Data Flow:</strong></p>
     * <ul>
     *   <li>Connected events are processed individually with detailed network information</li>
     *   <li>Scan results are processed as collections of network entries</li>
     *   <li>Both streams undergo the same validation and quality assessment</li>
     *   <li>Invalid or filtered data is excluded from the output stream</li>
     * </ul>
     * 
     * <p><strong>Performance Characteristics:</strong></p>
     * <ul>
     *   <li>Uses lazy evaluation with Java Streams for memory efficiency</li>
     *   <li>Processes data incrementally without loading entire datasets into memory</li>
     *   <li>Supports parallel processing for improved throughput</li>
     * </ul>
     * 
     ...
     */
    public Stream<WifiMeasurement> transformToMeasurements(WifiScanData wifiScanData, String processingBatchId) {
        ...
        logger.debug("Starting transformation: deviceId={}, batchId={}, timestamp={}", 
                    deviceId, processingBatchId, ingestionTimestamp);
        ...
        Stream<WifiMeasurement> connectedEventStream = Optional.ofNullable(wifiScanData.wifiConnectedEvents())
            .map(List::stream)
            .orElse(Stream.empty())
            .flatMap(event -> transformConnectedEvent(event, wifiScanData, deviceId, processingBatchId, ingestionTimestamp).stream());
        ...
        Stream<WifiMeasurement> scanResultStream = Optional.ofNullable(wifiScanData.scanResults())
            .map(List::stream)
            .orElse(Stream.empty())
            .flatMap(scanResult -> transformScanResult(scanResult, wifiScanData, deviceId, processingBatchId, ingestionTimestamp));
     ...
     * 
     * <p>This method processes WiFi connected events, which represent active WiFi connections
     * with detailed network information. Connected events are considered higher quality data
     * sources compared to scan results because they represent actual network connections.</p>
     * 
     * <p><strong>Processing Steps:</strong></p>
     * <ol>
     *   <li><strong>Data Validation:</strong> Ensures WiFi connected info is present and valid</li>
     *   <li><strong>Field Validation:</strong> Validates BSSID, RSSI, and location data</li>
     *   <li><strong>Mobile Hotspot Detection:</strong> Checks if the network is a mobile hotspot</li>
     *   <li><strong>Quality Assessment:</strong> Calculates quality weight based on signal strength and link speed</li>
     *   <li><strong>Measurement Creation:</strong> Builds comprehensive measurement record</li>
     * </ol>
     * 
     * <p><strong>Quality Characteristics:</strong></p>
     * <ul>
     *   <li>Connected events receive higher base quality weights than scan results</li>
     *   <li>Link speed information is used to adjust quality weights</li>
     *   <li>Signal strength (RSSI) contributes to overall quality assessment</li>
     * </ul>
     * 
     * <p><strong>Filtering Rules:</strong></p>
     * <ul>
     *   <li>Events without WiFi connected info are excluded</li>
     *   <li>Invalid BSSID, RSSI, or location data results in exclusion</li>
     *   <li>Mobile hotspots may be excluded based on configuration</li>
     * </ul>
     * 
    ...
    private Optional<WifiMeasurement> transformConnectedEvent(
        ...
        if (!isValidForTransformation(event.wifiConnectedInfo().bssid(), 
     ...
     * 
     * <p>This method processes WiFi scan results, which represent discovered networks from
     * passive WiFi scans. Scan results provide broader network coverage but with less
     * detailed information compared to connected events.</p>
     * 
     * <p><strong>Processing Approach:</strong></p>
     * <ul>
     *   <li>Uses a declarative stream-based approach for efficient processing</li>
     *   <li>Handles null scan results gracefully by returning empty streams</li>
     *   <li>Processes each scan result entry individually for detailed validation</li>
     *   <li>Applies the same validation and quality assessment rules as connected events</li>
     * </ul>
     * 
     * <p><strong>Data Characteristics:</strong></p>
     * <ul>
     *   <li>Scan results contain multiple network entries per scan</li>
     *   <li>Each entry represents a discovered network with basic information</li>
     *   <li>Location data is shared across all entries in a scan result</li>
     *   <li>Quality weights are typically lower than connected events</li>
     * </ul>
     * 
    ...
    private Stream<WifiMeasurement> transformScanResult(
        ...
        return Optional.ofNullable(scanResult.results()).stream()
            .flatMap(List::stream)
            .flatMap(entry -> transformScanResultEntrySafely(entry, scanResult, wifiScanData, deviceId, processingBatchId, ingestionTimestamp).stream());
    }
    
    /**
     * Safely transforms a single scan result entry into a WiFi measurement.
     * 
     * <p>This method processes individual network entries from WiFi scan results.
     * Each entry represents a discovered network with basic connectivity information.
     * The method applies comprehensive validation and transformation rules to ensure
     ...
     *   <li><strong>Field Validation:</strong> Validates BSSID, RSSI, and location data</li>
     *   <li><strong>Mobile Hotspot Detection:</strong> Checks if the network is a mobile hotspot</li>
     *   <li><strong>Event ID Generation:</strong> Creates unique identifier for the measurement</li>
     *   <li><strong>Measurement Building:</strong> Constructs comprehensive measurement record</li>
     *   <li><strong>Quality Assessment:</strong> Calculates quality score and weight</li>
     * </ol>
     * 
     * <p><strong>Data Mapping:</strong></p>
     * <ul>
     *   <li>Network information comes from the scan result entry</li>
     *   <li>Location data is shared from the parent scan result</li>
     *   <li>Device information is inherited from the WiFi scan data</li>
     *   <li>Processing metadata is added for tracking and debugging</li>
     * </ul>
     * 
     * <p><strong>Error Handling:</strong></p>
     * <ul>
     *   <li>Invalid entries are filtered out without affecting other entries</li>
     *   <li>Exceptions are caught and logged to prevent processing failures</li>
     *   <li>Returns empty Optional for any validation failures</li>
     * </ul>
     * 
    ...
    private Optional<WifiMeasurement> transformScanResultEntrySafely(
        ...
        try {
            ...
            if (shouldExcludeForMobileHotspot(hotspotResult)) {
                logger.debug("Excluding scan result for BSSID {} due to mobile hotspot detection", entry.bssid());
                return Optional.empty();
            }
            ...
            WifiMeasurement measurement = WifiMeasurement.builder()
                ...
                .deviceManufacturer(wifiScanData.manufacturer())  // Device manufacturer (e.g., "samsung")
                .osVersion(wifiScanData.osVersion())
                .appVersion(wifiScanData.appNameVersion())
                ...
                .locationAccuracy(scanResult.location() != null ? scanResult.location().accuracy() : null)
                .locationTimestamp(scanResult.location() != null ? scanResult.location().time() : null)
                .locationProvider(scanResult.location() != null ? scanResult.location().provider() : null)
     ...
     * 
     * <p>This method performs comprehensive validation of all critical fields required
     * for creating a valid WiFi measurement. It ensures data quality by checking
     * BSSID format, RSSI values, and location data integrity.</p>
     * 
     * <p><strong>Validation Criteria:</strong></p>
     * <ul>
     *   <li><strong>BSSID:</strong> Must be a valid MAC address format</li>
     *   <li><strong>RSSI:</strong> Must be within acceptable signal strength range</li>
     *   <li><strong>Location:</strong> Must have valid coordinates and accuracy</li>
     * </ul>
     * 
     * <p><strong>Validation Process:</strong></p>
     * <ol>
     *   <li>Validates BSSID format and structure using validation service</li>
     *   <li>Checks RSSI value against acceptable ranges</li>
     *   <li>Validates location data including coordinates and accuracy</li>
     *   <li>Returns false if any validation fails, preventing downstream errors</li>
     * </ol>
     * 
     ...
     * 
     * <p>This method implements the business rule for handling mobile hotspot networks.
     * Mobile hotspots may be excluded from analysis to improve data quality, as they
     * represent temporary networks rather than fixed infrastructure.</p>
     * 
     * <p><strong>Exclusion Logic:</strong></p>
     * <ul>
     *   <li>Mobile hotspot must be detected by the validation service</li>
     *   <li>Configuration must specify EXCLUDE action for mobile hotspots</li>
     *   <li>Both conditions must be true for exclusion to occur</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method applies business rules to adjust quality weights based on network
     * performance characteristics. It specifically handles cases where link speed
     * indicates poor network performance despite good signal strength.</p>
     * 
     * <p><strong>Quality Adjustment Rules:</strong></p>
     * <ul>
     *   <li>Low link speed (< 50 Mbps) with good RSSI (> -70 dBm) triggers weight reduction</li>
     *   <li>This indicates network congestion or interference despite strong signal</li>
     *   <li>Reduced weight reflects lower data quality for location analysis</li>
     * </ul>
     * 
     * <p><strong>Weight Calculation:</strong></p>
     * <ul>
     *   <li>Normal conditions: Returns the base weight (higher for connected events)</li>
     *   <li>Poor performance: Returns configured low link speed weight</li>
     *   <li>Missing data: Returns base weight (graceful degradation)</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method computes a composite quality score that reflects the overall
     * reliability of the measurement for location analysis. The score combines
     * location accuracy and signal strength to provide a normalized quality metric.</p>
     * 
     * <p><strong>Score Components:</strong></p>
     * <ul>
     *   <li><strong>Base Score (50%):</strong> Default quality level for all measurements</li>
     *   <li><strong>Location Accuracy (30%):</strong> Higher accuracy improves score</li>
     *   <li><strong>Signal Strength (20%):</strong> Stronger signals improve score</li>
     * </ul>
     * 
     * <p><strong>Scoring Algorithm:</strong></p>
     * <ol>
     *   <li>Start with base score of 0.5 (50%)</li>
     *   <li>Add location accuracy contribution (0-30% based on accuracy in meters)</li>
     *   <li>Add RSSI contribution (0-20% based on signal strength)</li>
     *   <li>Cap final score at 1.0 (100%)</li>
     * </ol>
     * 
     * <p><strong>Quality Ranges:</strong></p>
     * <ul>
     *   <li>0.5-0.7: Low quality (poor accuracy or weak signal)</li>
     *   <li>0.7-0.9: Medium quality (moderate accuracy and signal)</li>
     *   <li>0.9-1.0: High quality (excellent accuracy and strong signal)</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method creates a unique, deterministic device identifier by combining
     * multiple device characteristics and applying cryptographic hashing. This approach
     * ensures privacy by not storing raw device information while maintaining
     * consistency for tracking and analytics purposes.</p>
     * 
     * <p><strong>Device Characteristics Used:</strong></p>
     * <ul>
     *   <li><strong>Manufacturer:</strong> Device manufacturer (e.g., "samsung")</li>
     *   <li><strong>Model:</strong> Device model (e.g., "SM-A536V")</li>
     *   <li><strong>Device:</strong> Device identifier (e.g., "a53x")</li>
     *   <li><strong>OS Version:</strong> Operating system version string</li>
     * </ul>
     * 
     * <p><strong>Privacy Features:</strong></p>
     * <ul>
     *   <li>Uses SHA-256 hashing to prevent reverse engineering</li>
     *   <li>Consistent identifier for same device characteristics</li>
     *   <li>No storage of raw device information</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method creates a deterministic event ID by combining the scan timestamp
     * and BSSID. This ensures unique identification of each measurement while
     * maintaining consistency for deduplication and tracking purposes.</p>
     * 
     * <p><strong>Event ID Components:</strong></p>
     * <ul>
     *   <li><strong>Timestamp:</strong> When the scan occurred (milliseconds since epoch)</li>
     *   <li><strong>BSSID:</strong> The network identifier (MAC address)</li>
     * </ul>
     * 
     * <p><strong>Use Cases:</strong></p>
     * <ul>
     *   <li>Deduplication of duplicate measurements</li>
     *   <li>Tracking individual measurements through the pipeline</li>
     *   <li>Correlation with other data sources</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method standardizes BSSID format to ensure consistent representation
     * across the system. It handles various input formats and converts them to
     * the standard MAC address format with lowercase letters and colons.</p>
     * 
     * <p><strong>Normalization Process:</strong></p>
     * <ol>
     *   <li>Convert to lowercase for consistency</li>
     *   <li>Replace hyphens with colons for standard format</li>
     *   <li>Ensure proper MAC address format</li>
     * </ol>
     * 
     * <p><strong>Input Formats Handled:</strong></p>
     * <ul>
     *   <li>"B8:F8:53:C0:1E:FF" → "b8:f8:53:c0:1e:ff"</li>
     *   <li>"B8-F8-53-C0-1E-FF" → "b8:f8:53:c0:1e:ff"</li>
     *   <li>"b8f853c01eff" → "b8:f8:53:c0:1e:ff"</li>
     * </ul>
     * 
     ...
     * 
     * <p>This method sanitizes SSID (Service Set Identifier) values by removing
     * null bytes and trimming whitespace. This ensures clean, consistent SSID
     * representation for storage and analysis.</p>
     * 
     * <p><strong>Cleaning Process:</strong></p>
     * <ul>
     *   <li>Remove null bytes (0x00) that may be present in raw data</li>
     *   <li>Trim leading and trailing whitespace</li>
     *   <li>Preserve internal whitespace and special characters</li>
     *   <li>Return null for empty strings after cleaning</li>
     * </ul>
     * 
     * <p><strong>Examples:</strong></p>
     * <ul>
     *   <li>"MyWiFi\u0000" → "MyWiFi"</li>
     *   <li>"  Home Network  " → "Home Network"</li>
     *   <li>"Office-5G\u0000\u0000" → "Office-5G"</li>
     *   <li>"\u0000\u0000" → null</li>
     * </ul>
     * 
