 ...
 * 
 * <p>This service provides detailed monitoring capabilities for SQS message processing,
 * including connectivity tracking, message processing activity, queue status monitoring,
 * and performance metrics collection. It serves as the foundation for SQS-based health
 * indicators in both readiness and liveness probes.</p>
 * 
 * <p><strong>Monitoring Capabilities:</strong></p>
 * <ul>
 *   <li><strong>Connectivity Monitoring:</strong> SQS queue accessibility and connection health</li>
 *   <li><strong>Processing Activity:</strong> Message processing rate, success/failure tracking</li>
 *   <li><strong>Queue Status:</strong> Queue attributes, message counts, visibility metrics</li>
 *   <li><strong>Performance Metrics:</strong> Processing times, throughput, error rates</li>
 * </ul>
 * 
 * <p><strong>Health Check Support:</strong></p>
 * <ul>
 *   <li><strong>Readiness:</strong> Queue connectivity, accessibility, configuration validation</li>
 *   <li><strong>Liveness:</strong> Processing activity, message consumption tracking, stuck detection</li>
 * </ul>
 * 
 * <p><strong>Activity Tracking:</strong></p>
 * <ul>
 *   <li>Last message received timestamp for idle period detection</li>
 *   <li>Message processing rate calculation for performance monitoring</li>
 *   <li>Success/failure ratio tracking for reliability assessment</li>
 *   <li>Processing pipeline health status monitoring</li>
 * </ul>
 * 
...
public class SqsMonitoringService {
    ...
    // Activity tracking
    private final AtomicReference<Instant> lastMessageReceivedTime = new AtomicReference<>(Instant.now());
    private final AtomicReference<Instant> lastProcessingActivityTime = new AtomicReference<>(Instant.now());
    ...
    public void recordMessageProcessed(boolean success) {
        ...
        logger.debug("Message processing recorded - success: {}, total: {}, succeeded: {}", 
                success, totalMessagesProcessed.get(), totalMessagesSucceeded.get());
    }
    ...
    public static class SqsMetrics {
        ...
        public SqsMetrics(long totalMessagesReceived, long totalMessagesProcessed,
                         long totalMessagesSucceeded, long totalMessagesFailed,
                         double successRate, double processingRate,
                         long timeSinceLastMessageReceived, long timeSinceLastProcessingActivity,
                         Instant lastSuccessfulConnection, long consecutiveConnectionFailures) {
        ...
        public long getTotalMessagesSucceeded() { return totalMessagesSucceeded; }
        ...
        public long getTimeSinceLastMessageReceived() { return timeSinceLastMessageReceived; }
        public long getTimeSinceLastProcessingActivity() { return timeSinceLastProcessingActivity; }
        public Instant getLastSuccessfulConnection() { return lastSuccessfulConnection; }
        public long getConsecutiveConnectionFailures() { return consecutiveConnectionFailures; }
    }
}