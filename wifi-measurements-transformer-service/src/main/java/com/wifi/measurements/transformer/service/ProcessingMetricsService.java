 ...
 * processing pipeline, including file processing, record transformation, Firehose operations, and
 * overall system performance.
 *
 * <p><strong>Metric Categories:</strong>
 *
 * <ul>
 *   <li><strong>File Processing:</strong> Files processed, sizes, processing times
 *   <li><strong>Record Processing:</strong> Records transformed, validation outcomes, errors
 *   <li><strong>Firehose Operations:</strong> Batch delivery, success/failure rates, latency
 *   <li><strong>System Performance:</strong> Memory usage, processing throughput, error rates
 * </ul>
 *
 * <p><strong>Performance Tracking:</strong>
 *
 * <ul>
 *   <li>End-to-end processing duration from S3 download to Firehose delivery
 *   <li>Component-level timing for optimization opportunities
 *   <li>Throughput metrics for capacity planning
 *   <li>Error rate tracking for reliability monitoring
 * </ul>
 */
@Service
public class ProcessingMetricsService {
  ...
  public ProcessingMetricsService(MeterRegistry meterRegistry) {
    ...
    this.filesProcessedCounter =
        Counter.builder("processing.files.processed.total")
            .description("Total number of files processed successfully")
            .register(meterRegistry);

    this.filesFailedCounter =
        Counter.builder("processing.files.failed.total")
            .description("Total number of files that failed processing")
            .register(meterRegistry);

    this.fileProcessingTimer =
        Timer.builder("processing.files.duration")
            .description("Time taken to process individual files")
            .register(meterRegistry);

    this.fileSizeDistribution =
        DistributionSummary.builder("processing.files.size.bytes")
            .description("Distribution of file sizes being processed")
            .baseUnit("bytes")
            .register(meterRegistry);

    this.fileDownloadErrors =
        Counter.builder("processing.files.download.errors.total")
            .description("Total number of file download errors")
            .register(meterRegistry);

    // Initialize Record Processing Metrics
    this.recordsProcessedCounter =
        Counter.builder("processing.records.processed.total")
            .description("Total number of records processed")
            .register(meterRegistry);

    this.recordsValidatedCounter =
        Counter.builder("processing.records.validated.total")
            .description("Total number of records that passed validation")
            .register(meterRegistry);

    this.recordsFilteredCounter =
        Counter.builder("processing.records.filtered.total")
            .description("Total number of records filtered out")
            .register(meterRegistry);

    this.recordsRejectedCounter =
        Counter.builder("processing.records.rejected.total")
            .description("Total number of records rejected due to validation failures")
            .register(meterRegistry);

    this.recordTransformationTimer =
        Timer.builder("processing.records.transformation.duration")
            .description("Time taken to transform individual records")
            .register(meterRegistry);

    this.recordsPerFileDistribution =
        DistributionSummary.builder("processing.records.per.file")
            .description("Distribution of number of records per file")
            .register(meterRegistry);

    // Initialize Firehose Metrics
    this.firehoseBatchesDeliveredCounter =
        Counter.builder("firehose.batches.delivered.total")
            .description("Total number of batches successfully delivered to Firehose")
            .register(meterRegistry);

    this.firehoseBatchesFailedCounter =
        Counter.builder("firehose.batches.failed.total")
            .description("Total number of batches that failed to deliver to Firehose")
            .register(meterRegistry);

    this.firehoseRecordsDeliveredCounter =
        Counter.builder("firehose.records.delivered.total")
            .description("Total number of records successfully delivered to Firehose")
            .register(meterRegistry);

    this.firehoseRecordsFailedCounter =
        Counter.builder("firehose.records.failed.total")
            .description("Total number of records that failed to deliver to Firehose")
            .register(meterRegistry);

    this.firehoseBatchDeliveryTimer =
        Timer.builder("firehose.batch.delivery.duration")
            .description("Time taken to deliver batches to Firehose")
            .register(meterRegistry);

    this.firehoseBatchSizeDistribution =
        DistributionSummary.builder("firehose.batch.size.records")
            .description("Distribution of batch sizes in number of records")
            .register(meterRegistry);

    this.firehoseBatchSizeBytesDistribution =
        DistributionSummary.builder("firehose.batch.size.bytes")
            .description("Distribution of batch sizes in bytes")
            .baseUnit("bytes")
            .register(meterRegistry);

    this.firehoseRetryCounter =
        Counter.builder("firehose.retries.total")
            .description("Total number of Firehose delivery retries")
            .register(meterRegistry);

    // Initialize System Performance Metrics
    this.endToEndProcessingTimer =
        Timer.builder("processing.end.to.end.duration")
            .description("Total time from file receipt to Firehose delivery")
            .register(meterRegistry);

    this.processingErrorsCounter =
        Counter.builder("processing.errors.total")
            .description("Total number of processing errors")
            .register(meterRegistry);

    this.processingThroughputDistribution =
        DistributionSummary.builder("processing.throughput.records.per.second")
  ...
  private void registerGauges() {
    ...
    Gauge.builder(
            "processing.bytes.processed.total", this, service -> service.totalBytesProcessed.get())
        .description("Total bytes processed since startup")
        .baseUnit("bytes")
        .register(meterRegistry);

    Gauge.builder(
            "processing.records.processed.total.counter",
            this,
    ...
    Gauge.builder(
            "processing.last.processing.time.seconds",
            this,
            service -> {
  ...
  public void recordFirehoseBatchDelivered(
      Timer.Sample sample, int recordCount, long batchSizeBytes) {
    if (sample != null) {
  ...
  public void recordEndToEndProcessing(
      Timer.Sample sample, int recordCount, Duration processingDuration) {
    if (sample != null) {
    ...
    lastProcessingTime.set(Instant.now());
    logger.info(
        "End-to-end processing completed: {} records in {}ms",
        recordCount,
