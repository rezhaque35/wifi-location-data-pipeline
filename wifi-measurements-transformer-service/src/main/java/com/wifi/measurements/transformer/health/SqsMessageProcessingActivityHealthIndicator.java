 ...
 * are available but not being processed. It provides comprehensive monitoring for SQS consumer
 * liveness and processing pipeline health.
 *
 * <p><strong>Key Functionality:</strong>
 *
 * <ul>
 *   <li><strong>Processing Activity Tracking:</strong> Monitor message processing count and trends
 *   <li><strong>Rate Monitoring:</strong> Track messages processed per time window with
 *       configurable thresholds
 *   <li><strong>Activity Comparison:</strong> Compare current processing rate with historical
 *       baseline
 *   <li><strong>Stuck Detection:</strong> Detect sustained periods of zero message processing when
 *       messages are available
 *   <li><strong>Idle Period Handling:</strong> Properly handle "no messages available" vs "messages
 ...
 *
 * <ol>
 *   <li><strong>Connectivity Check:</strong> Verify SQS queue connectivity and accessibility
 *   <li><strong>Processing Activity:</strong> Monitor time since last processing activity
 *   <li><strong>Message Reception:</strong> Track time since last message received
 *   <li><strong>Stuck Detection:</strong> Detect if processing appears stuck or inactive
 *   <li><strong>Rate Analysis:</strong> Analyze message processing rates and success rates
 *   <li><strong>Threshold Comparison:</strong> Compare processing patterns against configured
 *       thresholds
 ...
 *
 * <ul>
 *   <li>Track message processing count and trends for processing pipeline health
 *   <li>Monitor messages processed per time window with configurable rate thresholds
 *   <li>Compare current processing rate with historical baseline for anomaly detection
 *   <li>Detect sustained periods of zero message processing when messages are available
 *   <li>Fail if processing is stuck but messages are available for processing
 *   <li>Distinguish between "no messages available" vs "messages available but not processed"
 * </ul>
 *
 * <p><strong>Health Check Categories:</strong>
 *
 * <ul>
 *   <li><strong>Connectivity:</strong> SQS queue connection and accessibility
 *   <li><strong>Processing Activity:</strong> Time since last processing activity
 *   <li><strong>Message Reception:</strong> Time since last message received
 *   <li><strong>Processing Rate:</strong> Messages processed per time window
 *   <li><strong>Success Rate:</strong> Successful vs failed message processing ratio
 *   <li><strong>Pipeline Health:</strong> Overall message processing pipeline status
 * </ul>
 *
 * <p><strong>Configuration Properties:</strong>
 *
 * <ul>
 *   <li><strong>message-timeout-threshold:</strong> Timeout for message activity (default: 5
 *       minutes)
 *   <li><strong>processing-rate-threshold:</strong> Minimum processing rate (default: 0.1
 *       messages/minute)
 * </ul>
 *
 ...
 *   <li>SQS consumer health monitoring and alerting
 *   <li>Processing pipeline health assessment
...
public class SqsMessageProcessingActivityHealthIndicator implements HealthIndicator {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(SqsMessageProcessingActivityHealthIndicator.class);
  ...
  @Value("${management.health.sqs-message-processing.message-timeout-threshold:5}") // 5 minutes
  // default
  ...
  /**
   * Creates a new SqsMessageProcessingActivityHealthIndicator for testing with explicit
   * configuration.
   *
   ...
   *
   * <p>This method implements the core health check logic, evaluating multiple aspects of SQS
   * consumer health including connectivity, processing activity, message reception, processing
   ...
   * <ol>
   *   <li><strong>Connectivity Check:</strong> Verify SQS queue connectivity and accessibility
   *   <li><strong>Activity Monitoring:</strong> Check time since last processing activity
   *   <li><strong>Reception Tracking:</strong> Monitor time since last message received
   *   <li><strong>Stuck Detection:</strong> Detect if processing appears stuck or inactive
   *   <li><strong>Rate Analysis:</strong> Analyze message processing rates and success rates
   *   <li><strong>Threshold Validation:</strong> Compare processing patterns against thresholds
   ...
   *
   * <ul>
   *   <li><strong>Queue Connectivity:</strong> Must be able to connect to SQS queue
   *   <li><strong>Queue Accessibility:</strong> Must be able to access queue attributes
   *   <li><strong>Processing Activity:</strong> Must have recent processing activity within timeout
   *   <li><strong>Processing Status:</strong> Must not be stuck (receiving but not processing)
   *   <li><strong>Processing Rate:</strong> Must meet minimum processing rate threshold when
   *       applicable
   *   <li><strong>Success Rate:</strong> Must maintain acceptable processing success rate
   * </ul>
   *
  ...
  public Health health() {
    try {
      ...
      long timeSinceLastProcessingActivity =
          sqsMonitoringService.getTimeSinceLastProcessingActivity();
      ...
      // Step 3: Check if processing hasn't occurred recently (may indicate inactive processing or
      ...
      if (timeSinceLastProcessingActivity > timeoutThresholdMs) {
        return buildDownHealth(
            String.format(
                "SQS processing hasn't been active in %d ms (threshold: %d ms) - may indicate inactive processing or no available messages",
      ...
      if (processingStuck) {
        return buildDownHealth(
            "SQS processing is stuck - receiving messages but not processing them effectively",
            checkTimestamp);
      }
      ...
      if (totalMessagesReceived >= 10
          && timeSinceLastProcessingActivity <= (timeoutThresholdMs / 2)) {
        ...
        if (processingRate > 0 && processingRate < processingRateThreshold) {
          logger.warn(
              "Low processing rate detected: {} msgs/min (threshold: {} msgs/min)",
              processingRate,
              processingRateThreshold);
