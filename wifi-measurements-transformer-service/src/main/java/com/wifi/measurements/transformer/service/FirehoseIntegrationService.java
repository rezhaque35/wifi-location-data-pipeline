...
/**
 * Service for writing WiFi measurement batches to Kinesis Data Firehose. Implements comprehensive
 * error handling, retry logic, and monitoring based on AWS best practices.
 */
@Service
public class FirehoseIntegrationService {
     ...
     */
    CompletableFuture<Void> writeBatchWithAttempt(
            List<byte[]> jsonRecords, int attempt, String correlationId) {
        ...
        try {
            ...
            PutRecordBatchRequest request = buildPutRequest(jsonRecords, attempt, correlationId);
            ...
            return firehoseClient
                    ...
                    .exceptionally(
                            throwable -> handleBatchError(throwable, jsonRecords, attempt, correlationId));
    ...
    private PutRecordBatchRequest buildPutRequest(List<byte[]> jsonRecords, int attempt, String correlationId) {
        var records =
                createFirehoseRecords(jsonRecords);
        ...
        logger.debug(
                "Writing batch {} to Firehose: {} records, attempt {}",
                correlationId,
                jsonRecords.size(),
                attempt + 1);
        return request;
     ...
     */
    private List<software.amazon.awssdk.services.firehose.model.Record> createFirehoseRecords(
            List<byte[]> jsonRecords) {

        return jsonRecords.stream().map(bytes -> software.amazon.awssdk.services.firehose.model.Record.builder()
    ...
    private void handleSuccessfulResponse(
            PutRecordBatchResponse response, List<byte[]> originalRecords, String correlationId) {
        ...
        logger.info(
                "Batch {} written to Firehose: {} records, {} failed",
                correlationId,
                totalRecords,
                failedRecords);
    ...
    private void handlePartialFailures(
        ...
        if (!failedRecords.isEmpty()) {
            logger.warn(
                    "Batch {} had {} partial failures, retrying failed records",
                    correlationId,
                    failedRecords.size());
    ...
    private Void handleBatchError(
            Throwable throwable, List<byte[]> records, int attempt, String correlationId) {
    ...
    private Void handleRetriableError(
            Throwable cause, List<byte[]> records, int attempt, String correlationId) {
        String errorType = getRetriableErrorType(cause);

        logger.warn(
                "Retriable {} error for batch {} (attempt {}): {}",
                errorType,
                correlationId,
                attempt + 1,
                cause.getMessage());
        ...
        if (attempt >= firehoseProperties.maxRetries()) {
            logger.error(
                    "Max retries ({}) exceeded for batch {} due to {}",
                    firehoseProperties.maxRetries(),
                    correlationId,
                    errorType);
        ...
        logger.info(
                "Retrying batch {} in {}ms due to {} (attempt {}/{})",
                correlationId,
                delay,
                errorType,
                attempt + 1,
    ...
    private void logPermanentError(Throwable cause, List<byte[]> records, String correlationId) {
        ...
        logger.error(
                "Permanent {} error for batch {} - NOT RETRYING: {}",
                errorType,
                correlationId,
                cause.getMessage());
    ...
    private void logUnknownError(Throwable cause, List<byte[]> records, String correlationId) {
        logger.error(
                "Unknown error type for batch {} - NOT RETRYING (being conservative): {} - {}",
                correlationId,
        ...
        meterRegistry
                .counter("firehose.unknown.errors", "class", cause.getClass().getSimpleName())
    ...
    private boolean isRetriableError(Throwable cause) {
        return cause instanceof ServiceUnavailableException
                || // Throttling/service issues
    ...
    private void retryAfterDelay(
            List<byte[]> records, int attempt, String correlationId, long delayMs) {
                ...
                () -> {
                    writeBatchWithAttempt(records, attempt, correlationId)
                            .exceptionally(
                                    ex -> {
                                        logger.error(
                                                "Scheduled retry failed for batch {}: {}", correlationId, ex.getMessage());
                                        return null;
                                    });
                },
                delayMs,
    ...
    private long calculateRetryDelayWithJitter(int attempt) {
        ...
        long exponentialDelay =
                firehoseProperties.retryBackoffMs() * (1L << attempt); // 1s, 2s, 4s, 8s...
