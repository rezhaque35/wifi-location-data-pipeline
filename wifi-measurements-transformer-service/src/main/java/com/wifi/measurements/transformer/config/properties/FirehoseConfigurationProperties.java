...
/**
 * Configuration properties for AWS Kinesis Data Firehose integration.
 * Handles batch size limits, timeouts, retry configuration, and delivery stream settings.
 */
...
public record FirehoseConfigurationProperties(
     ...
     */
    @NotBlank(message = "Delivery stream name is required")
     ...
     */
    @Positive(message = "Batch timeout must be positive")
    long batchTimeoutMs,
     ...
     */
    @Positive(message = "Retry backoff must be positive")
    long retryBackoffMs
) {
} 