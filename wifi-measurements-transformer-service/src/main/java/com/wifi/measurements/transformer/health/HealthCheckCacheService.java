// wifi-measurements-transformer-service/src/main/java/com/wifi/measurements/transformer/health/HealthCheckCacheService.java
...
/**
 * Service for caching health check results to avoid excessive AWS API calls.
 * 
 * <p>This service provides cached access to expensive health check operations like
 * SQS and Firehose connectivity checks. It implements a simple time-based cache
...
public class HealthCheckCacheService {
    ...
    public <T> T getCachedOrRefresh(String cacheKey, Supplier<T> supplier) {
        ...
        try {
            ...
            logger.warn("Error refreshing cache for key: {}", cacheKey, e);
            
            // If we have a stale cached result, return it rather than failing
            if (cachedResult != null) {
                logger.debug("Returning stale cached result for key: {} due to refresh error", cacheKey);
                @SuppressWarnings("unchecked")
                T result = (T) cachedResult.value;
                return result;
            }
