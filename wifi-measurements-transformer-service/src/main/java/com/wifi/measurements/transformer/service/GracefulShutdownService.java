 ...
 * 
 * <p>This service ensures that during application shutdown, all pending Firehose batches
 * are properly flushed and delivered before the application terminates. It coordinates
 * with SQS message processing to stop accepting new work while allowing in-flight
 * operations to complete.</p>
 * 
 * <p><strong>Shutdown Sequence:</strong></p>
 * <ol>
 *   <li>Stop accepting new SQS messages</li>
 *   <li>Wait for in-flight processing to complete</li>
 *   <li>Flush all pending Firehose batches</li>
 *   <li>Wait for Firehose delivery confirmation</li>
 *   <li>Complete shutdown</li>
 * </ol>
 * 
 * <p><strong>Timeout Handling:</strong></p>
 * <ul>
 *   <li>Maximum shutdown timeout: 30 seconds</li>
 *   <li>Firehose flush timeout: 15 seconds</li>
 *   <li>Processing completion timeout: 10 seconds</li>
 * </ul>
 */
@Service
public class GracefulShutdownService {
    ...
    public GracefulShutdownService(SqsMessageReceiver sqsMessageReceiver,
    ...
    private void waitForProcessingCompletion() {
        ...
        try {
            ...
            CompletableFuture<Void> processingCompletion = CompletableFuture.runAsync(() -> {
                while (processingMetricsService.getCurrentlyProcessingFiles() > 0) {
                    try {
                        Thread.sleep(500); // Check every 500ms
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt();
                        throw new RuntimeException("Interrupted while waiting for processing completion", e);
                    }
                }
            });
            ...
            Duration waitDuration = Duration.between(waitStart, Instant.now());
            logger.warn("Timeout waiting for processing completion after {}ms, proceeding with shutdown", 
                       waitDuration.toMillis(), e);
        }
    }
    ...
    private void flushFirehoseBatches() {
        ...
        try {
            ...
            CompletableFuture<Void> flushCompletion = CompletableFuture.runAsync(() -> {
                try {
