 ...
 * including connectivity tracking, delivery activity monitoring, stream status validation, and
 * performance metrics collection. It serves as the foundation for Firehose-based health indicators
 * in both readiness and liveness probes.
 *
 * <p><strong>Monitoring Capabilities:</strong>
 *
 * <ul>
 *   <li><strong>Connectivity Monitoring:</strong> Firehose delivery stream accessibility and health
 *   <li><strong>Delivery Activity:</strong> Batch delivery tracking, success/failure metrics
 *   <li><strong>Stream Status:</strong> Delivery stream state monitoring and validation
 *   <li><strong>Performance Metrics:</strong> Delivery times, throughput, error rates
 * </ul>
 *
 * <p><strong>Health Check Support:</strong>
 *
 * <ul>
 *   <li><strong>Readiness:</strong> Stream connectivity, status validation, configuration checks
 *   <li><strong>Liveness:</strong> Delivery activity monitoring, batch processing tracking, stuck
 *       detection
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Last delivery attempt timestamp for idle period detection
 *   <li>Delivery success/failure ratio tracking for reliability assessment
 *   <li>Batch processing rate calculation for performance monitoring
 *   <li>Delivery pipeline health status monitoring
 * </ul>
 *
...
public class FirehoseMonitoringService {
  ...
  // Activity tracking
  private final AtomicReference<Instant> lastDeliveryAttemptTime =
      new AtomicReference<>(Instant.now());
  private final AtomicReference<Instant> lastSuccessfulDeliveryTime =
      new AtomicReference<>(Instant.now());
  ...
  private final AtomicReference<DeliveryStreamStatus> lastKnownStreamStatus =
   ...
   */
  public FirehoseMonitoringService(
      FirehoseClient firehoseClient, FirehoseConfigurationProperties firehoseConfig) {
  ...
  public boolean isDeliveryStuck() {
    ...
    if (timeSinceLastAttempt < 300000
        && timeSinceLastSuccess > 600000) { // Attempting within 5 min but no success in 10 min
      return true;
    }
  ...
  public void recordDeliveryAttempt(int batchSize) {
    ...
    logger.debug(
        "Delivery attempt recorded - batch size: {}, total batches: {}",
        batchSize,
  ...
  public void recordDeliveryResult(boolean success, int successfulRecords, int failedRecords) {
    ...
    logger.debug(
        "Delivery result recorded - success: {}, successful records: {}, failed records: {}",
        success,
        successfulRecords,
        failedRecords);
  }
  ...
  public static class FirehoseMetrics {
    ...
    public FirehoseMetrics(
        long totalBatchesDelivered,
        long totalBatchesSucceeded,
        long totalBatchesFailed,
        long totalRecordsDelivered,
        long totalRecordsSucceeded,
        long totalRecordsFailed,
        double deliverySuccessRate,
        double deliveryRate,
        long timeSinceLastDeliveryAttempt,
        long timeSinceLastSuccessfulDelivery,
        Instant lastSuccessfulStreamCheck,
        DeliveryStreamStatus lastKnownStreamStatus,
        long consecutiveStreamCheckFailures,
        long consecutiveDeliveryFailures) {
    ...
    // Getters
    public long getTotalBatchesDelivered() {
      return totalBatchesDelivered;
    }

    public long getTotalBatchesSucceeded() {
      return totalBatchesSucceeded;
    }

    public long getTotalBatchesFailed() {
      return totalBatchesFailed;
    }

    public long getTotalRecordsDelivered() {
      return totalRecordsDelivered;
    }

    public long getTotalRecordsSucceeded() {
      return totalRecordsSucceeded;
    }
    ...
    public long getTimeSinceLastDeliveryAttempt() {
      return timeSinceLastDeliveryAttempt;
    }

    public long getTimeSinceLastSuccessfulDelivery() {
      return timeSinceLastSuccessfulDelivery;
    }

    public Instant getLastSuccessfulStreamCheck() {
      return lastSuccessfulStreamCheck;
    }

    public DeliveryStreamStatus getLastKnownStreamStatus() {
      return lastKnownStreamStatus;
    }

    public long getConsecutiveStreamCheckFailures() {
      return consecutiveStreamCheckFailures;
    }

    public long getConsecutiveDeliveryFailures() {
      return consecutiveDeliveryFailures;
    }
  }
}
