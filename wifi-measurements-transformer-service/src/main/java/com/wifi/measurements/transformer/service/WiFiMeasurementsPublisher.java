...
public class WiFiMeasurementsPublisher {
    ...
    public void publishMeasurement(WifiMeasurement measurement) {
        ...
        byte[] jsonBytes = getBytes(measurement);
        if (isThereIsReasonToSkip(jsonBytes)) return;
        addRecordToCurrentBatch(jsonBytes);
    }

    private void addRecordToCurrentBatch(byte[] jsonBytes) {
        lock.lock();
        try {
    ...
    private boolean isThereIsReasonToSkip(byte[] jsonBytes) {
        long recordSize = 0;
        boolean skip = (jsonBytes == null) || ((recordSize = jsonBytes.length) > firehoseProperties.maxRecordSizeBytes()); // Skip this record

        // Validate record size against Firehose limits
        if (skip && jsonBytes != null) {
            logger.error(
    ...
    private byte[] getBytes(WifiMeasurement measurement) {
        byte[] jsonBytes;
        try {
    ...
    private void emitCurrentBatch() {
        ...
        List<byte[]> batchToEmit = getBatchToEmit();
    ...
    private  Function<Throwable, Void> handleError(List<byte[]> batchToEmit) {
        return throwable -> {
    ...
    private List<byte[]> getBatchToEmit() {
