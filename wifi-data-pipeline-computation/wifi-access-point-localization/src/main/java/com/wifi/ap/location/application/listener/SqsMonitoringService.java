...
public class SqsMonitoringService {
    ...
    public void recordFailedBatch(List<MessageProcessingResult> results) {
        lastProcessingActivityTime.set(Instant.now());
        totalMessagesProcessed.addAndGet(results.size());
        long success = results.stream().filter(MessageProcessingResult::status).count();
        totalMessagesSucceeded.addAndGet(success);
        totalMessagesFailed.addAndGet(results.size() - success);

    }


    public void recordFailedBatch(long totalMessages) {
        lastProcessingActivityTime.set(Instant.now());
        totalMessagesProcessed.addAndGet(totalMessages);
        totalMessagesFailed.addAndGet(totalMessages);
