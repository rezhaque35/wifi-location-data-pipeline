...
/** Integration tests for health endpoints. Tests the actual HTTP endpoints and their responses. */
@SpringBootTest
...
class HealthEndpointIntegrationTest {
  ...
  void should_ReturnHealthStatus_When_HealthEndpointCalled() throws Exception {
    ...
    assertTrue(
        response.get("status").asText().equals("UP")
  ...
  void should_ReturnLivenessHealth_When_LivenessEndpointCalled() throws Exception {
    // Act & Assert
    MvcResult result =
        mockMvc.perform(get("/health/liveness")).andExpect(status().isOk()).andReturn();
  ...
  void should_ReturnReadinessHealth_When_ReadinessEndpointCalled() throws Exception {
    // Act & Assert
    MvcResult result = mockMvc.perform(get("/health/readiness")).andReturn();
    ...
    // Verify readiness status (can be UP, DOWN, or OUT_OF_SERVICE depending on DynamoDB
    // availability)
    assertNotNull(response.get("status"));
    assertTrue(
        response.get("status").asText().equals("UP")
  ...
  void should_ReturnServiceLivenessDetails_When_ServiceLivenessEndpointCalled() throws Exception {
    // Act & Assert
    MvcResult result =
        mockMvc.perform(get("/health/serviceLiveness")).andExpect(status().isOk()).andReturn();
  ...
  void should_ReturnDynamoDBReadinessDetails_When_DynamoDBReadinessEndpointCalled()
      throws Exception {
    // Act & Assert
    MvcResult result = mockMvc.perform(get("/health/dynamoDBReadiness")).andReturn();
  ...
  void should_ReturnProperContentType_When_HealthEndpointsCalled() throws Exception {
    ...
    assertTrue(
        ...
        "Expected status 200 or 503, but got: " + healthStatus);
    ...
    assertTrue(
        readinessStatus == 200 || readinessStatus == 503,
        "Expected status 200 or 503, but got: " + readinessStatus);
  }
  ...
  void should_ReturnValidJsonStructure_When_HealthEndpointsCalled() throws Exception {
    ...
    MvcResult healthResult = mockMvc.perform(get("/health")).andReturn();
    ...
    JsonNode healthResponse =
        objectMapper.readTree(healthResult.getResponse().getContentAsString());
    assertNotNull(healthResponse.get("status"));

    // Test individual health indicator structure
    MvcResult livenessResult =
        mockMvc.perform(get("/health/serviceLiveness")).andExpect(status().isOk()).andReturn();

    JsonNode livenessResponse =
        objectMapper.readTree(livenessResult.getResponse().getContentAsString());
