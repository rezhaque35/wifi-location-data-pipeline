...
/**
 * Represents a WiFi scan result with signal information. This record serves as both a Data Transfer
 * Object (DTO) for API communication and as a domain model for internal processing.
 *
 * <p>Contains information about a WiFi access point detected during scanning: - MAC address
 * (required): Unique identifier in XX:XX:XX:XX:XX:XX format - Signal strength (required): Measured
 * in dBm, typically -30 to -100 - Frequency (required): Operating frequency in MHz - SSID
 * (optional): Network name - Link speed (optional): Connection speed in Mbps - Channel width
 * (optional): WiFi channel width in MHz
 */
public record WifiScanResult(
    ...
    @Min(value = 2400, message = "Frequency must be at least 2400 MHz")
        ...
        Integer frequency,
    String ssid,
    @Min(value = 0, message = "Link speed must be non-negative") Integer linkSpeed,
  ...
  /**
   * Compact constructor for validation. Ensures required fields are not null even if validation
   * annotations are bypassed.
   */
  ...
  /**
   * Creates a new WifiScanResult with only the required fields. This factory method is provided for
   ...
   */
  public static WifiScanResult of(
      String macAddress, double signalStrength, int frequency, String ssid) {
    return new WifiScanResult(macAddress, signalStrength, frequency, ssid, null, null);
  }

  /**
   * Creates a new WifiScanResult with primitive values automatically boxed. This factory method is
