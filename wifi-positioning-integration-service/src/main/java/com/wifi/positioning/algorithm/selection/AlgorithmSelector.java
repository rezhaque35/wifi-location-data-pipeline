 ...
 * 1. Hard Constraints (Disqualification Phase) - Eliminate algorithms that are
 * mathematically or practically invalid
 * 2. Algorithm Weighting (Ranking Phase) - Assign and adjust weights based on
 * various factors
 * 3. Finalist Selection (Combination Phase) - Select the final set of
 * algorithms based on weights
 */
@Component
public class AlgorithmSelector {
    ...
    private static final String VALID_FOR_SINGLE_AP_WITH_MODEL = "Valid for single AP with path loss model";
    ...
    private static final String ONLY_VIABLE_FOR_WEAK = "Only viable algorithm for extremely weak signals";
    ...
    private static SelectedAlgorithms initializeSingleApAlgorithms() {
        Set<PositioningAlgorithmType> eligibleTypes = Set.of(
    ...
    /**
     * Initialize algorithms for two APs scenario
     * (proximity, RSSI ratio, weighted centroid, log distance)
     */
    ...
    private static SelectedAlgorithms initializeThreeApsAlgorithms() {
        Set<PositioningAlgorithmType> eligibleTypes = new HashSet<>(Arrays.asList(PositioningAlgorithmType.values()));
    ...
    /**
     * Record that holds algorithm selection information including weights and
     * reasoning
     */
    ...
    /**
     * Record to hold the result of algorithm selection after applying hard constraints
     * Contains both eligible algorithm types and the reasons for inclusion/exclusion
     */
    record SelectedAlgorithms(
        ...
        public SelectedAlgorithms copyWithFilter(
                java.util.function.Predicate<PositioningAlgorithmType> filter,
                String reasonForExclusion) {
            
            // Create a copy of the eligible algorithm types that pass the filter
            Set<PositioningAlgorithmType> filteredTypes = eligibleAlgorithmTypes.stream()
                    .filter(filter)
                    .collect(Collectors.toSet());
            
            // Create a deep copy of reasons
            Map<PositioningAlgorithmType, List<String>> newReasons = new HashMap<>();
            reasons.forEach((type, reasonList) -> 
                newReasons.put(type, new ArrayList<>(reasonList)));
    ...
    public AlgorithmSelector() {
        ...
        this.maximumLikelihoodAlgorithm = PositioningAlgorithmType.MAXIMUM_LIKELIHOOD.getImplementation();
        this.logDistanceAlgorithm = PositioningAlgorithmType.LOG_DISTANCE.getImplementation();
    }

    /**
     * Record to hold the result of algorithm weight calculation including
     * the algorithm, its calculated weight and the reason formula
     */
    private record WeightedAlgorithm(
    ...
    /**
     * Apply algorithm weighting based on various factors
     * Returns a list of weight results using a functional approach
     * 
    ...
    private Set<WeightedAlgorithm> applyAlgorithmWeighting(
            SelectedAlgorithms hardConstraintsResult,
    ...
    private WeightedAlgorithm calculateWeight(
        ...
        double geometricMultiplier = algorithm.getGeometricQualityMultiplier(context.getGeometricQuality());
        double distributionMultiplier = algorithm.getSignalDistributionMultiplier(context.getSignalDistribution());
    ...
    /**
     * Apply the algorithm selection framework to select and weight appropriate
     * algorithms.
     * This method implements the three-phase selection process:
     * 1. Hard Constraints - Eliminate algorithms that are mathematically or
     * practically invalid
     * 2. Algorithm Weighting - Assign and adjust weights based on various factors
     * 3. Finalist Selection - Select the final set of algorithms based on weights
     * 
     ...
     * @return AlgorithmSelectionInfo containing weights and detailed selection
     *         reasons
     */
    public AlgorithmSelectionInfo selectAlgorithmsWithReasons(
        ...
        logger.debug("After hard constraints, eligible algorithms: {}",
        ...
        Set<WeightedAlgorithm> weightedAlgorithms = applyAlgorithmWeighting(eligibleAlgorithms, context);


        // -------------- PHASE THREE: FINALIST SELECTION ----------------
        Set<WeightedAlgorithm>  finalSelectedAlgorithms = applyFinalistSelection(weightedAlgorithms, context);

        logger.debug("Final algorithm selection: {}",
                weightedAlgorithms.stream()
                        .map(e -> e.algorithm().getName() + "=" + e.weight())
                        .collect(Collectors.joining(", ")));

        return createAlgorithmSelectionInfo(finalSelectedAlgorithms, mergeReasons(eligibleAlgorithms, weightedAlgorithms));
    }

    /**
     * Applies the finalist selection phase of the algorithm selection framework.
     * This phase filters the algorithms based on signal quality and confidence thresholds.
    ...
    private Set<WeightedAlgorithm> applyFinalistSelection(
        ...
        logger.debug("After threshold filter (min weight {}): {} algorithms remain", 
            minimumWeightThreshold, weightedAlgorithms.size());
        
        // ---------- STEP 2: ADAPTIVE SELECTION ----------
        // Find the highest weight
        Optional<WeightedAlgorithm> highestWeighted = weightedAlgorithms.stream()
            .max(Comparator.comparingDouble(WeightedAlgorithm::weight));
        ...
        if (highestWeighted.isPresent() && highestWeighted.get().weight() > HIGH_CONFIDENCE_THRESHOLD) {
            ...
            logger.debug("High confidence algorithm detected with weight > {}: {}", 
                HIGH_CONFIDENCE_THRESHOLD, highestWeighted.get().algorithm().getName());
    ...
    private AlgorithmSelectionInfo createAlgorithmSelectionInfo(
        ...
        Map<PositioningAlgorithm, Double> algorithmWeights = weightedAlgorithms.stream()
                .collect(Collectors.toMap(
                        WeightedAlgorithm::algorithm,
                        WeightedAlgorithm::weight));
     ...
     */
    private SelectedAlgorithms selectAlgorithmsBasedOnHardConstraints(
            SelectionContext context) {
    ...
    /**
     * Handles geometric constraints (collinearity or poor geometry) by filtering out unsuitable algorithms.
     * 
    ...
    private SelectedAlgorithms handleGeometricConstraints(
        ...
        SelectedAlgorithms filteredSelection = baseSelection.copyWithFilter(
            type -> type != PositioningAlgorithmType.TRILATERATION,
            reason
        );
    ...
    private Map<PositioningAlgorithm, List<String>> mergeReasons(
            SelectedAlgorithms eligibleAlgorithms, 
            Set<WeightedAlgorithm> weightedAlgorithms) {
        
        // Create a map from algorithm to its weight reason
        Map<PositioningAlgorithm, String> weightedAlgorithmsMap = weightedAlgorithms.stream()
                .collect(Collectors.toMap(
                        WeightedAlgorithm::algorithm, 
                        WeightedAlgorithm::reason));
        
        // Process each algorithm type
        Map<PositioningAlgorithm, List<String>> collect = Arrays.stream(PositioningAlgorithmType.values())
                .map(algorithm -> {
