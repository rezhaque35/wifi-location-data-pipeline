 ...
 * 
 * Thread Safety: This class is thread-safe. The ExecutorService is shared across requests
 * but individual algorithm executions are isolated. The class uses immutable data structures
 * where possible and defensive copying for mutable collections.
 */
@Component
public class WifiPositioningCalculator {
    ...
    /**
     * Timeout for individual algorithm execution in seconds.
     * Rationale: 5 seconds allows sufficient time for complex algorithms (trilateration, 
     * maximum likelihood) while preventing indefinite blocking. Based on empirical testing,
     * most algorithms complete within 100-500ms, so 5s provides ample safety margin.
     */
    private static final long ALGORITHM_EXECUTION_TIMEOUT_SECONDS = 5L;
    
    /**
     * Timeout for executor service shutdown in seconds.
     * Rationale: Same as algorithm timeout - allows running algorithms to complete gracefully
     * before forcing shutdown during application termination.
     */
    private static final long EXECUTOR_SHUTDOWN_TIMEOUT_SECONDS = 5L;
    
    /**
     * Minimum number of threads in the executor pool.
     * Rationale: Ensures at least 2 threads are available for parallel execution even on
     * single-core systems, allowing for basic parallelism in algorithm execution.
     */
    private static final int MIN_THREAD_POOL_SIZE = 2;
    
    /**
     * Divisor for calculating thread pool size based on available processors.
     * Rationale: Uses half of available processors to balance parallelism with system resource
     * usage. WiFi positioning is CPU-intensive but not the only system workload. This leaves
     * resources for other application components and system processes.
     */
    ...
    public WifiPositioningCalculator(
        ...
        // Calculate optimal thread pool size: max(MIN_THREAD_POOL_SIZE, availableProcessors / PROCESSOR_DIVISOR)
        // This ensures adequate parallelism while preserving system resources
        int threadPoolSize = Math.max(MIN_THREAD_POOL_SIZE, 
                                    Runtime.getRuntime().availableProcessors() / PROCESSOR_DIVISOR);
    ...
    public PositioningResult calculatePosition(List<WifiScanResult> wifiScan, List<WifiAccessPoint> knownAPs) {
        ...
        List<PositionCombiner.WeightedPosition> positions = calculatePositionsInParallel(weightedAlgorithms, validScans, knownAPs);
     ...
     * - ExecutionException: Algorithm threw an exception, logged as error
     * - Other exceptions: Unexpected errors, logged as error
     *
     * Mathematical Foundation:
     * Each algorithm produces a position estimate P_i with confidence C_i.
     * The final weight W_i = baseWeight_i * C_i where:
     * - baseWeight_i: Algorithm's suitability for the scenario (from selector)
     * - C_i: Algorithm's confidence in its result (self-assessed)
     * 
     * This weighting scheme ensures that both algorithmic appropriateness and 
    ...
    /**
     * Submits positioning algorithm tasks to the executor service for parallel execution.
     * Each algorithm is wrapped in a CompletableFuture with proper exception handling.
     *
        ...
        for (Map.Entry<PositioningAlgorithm, Double> entry : weightedAlgorithms.entrySet()) {
            ...
            CompletableFuture<PositionCombiner.WeightedPosition> future = 
                CompletableFuture.supplyAsync(() -> 
                    executeAlgorithmWithExceptionHandling(algorithm, baseWeight, validScans, knownAPs), 
                    executorService);
    ...
    /**
     * Executes a single positioning algorithm with comprehensive exception handling.
     * Calculates the final weight by combining base weight with algorithm confidence.
     *
    ...
    /**
     * Collects results from all algorithm futures with proper timeout and exception handling.
     * Each future is processed with individual timeout and error recovery strategies.
     *
    ...
    private PositionCombiner.WeightedPosition retrievePositionWithTimeout(
        ...
        try {
            ...
            logger.warn("Algorithm execution timed out after {} seconds", ALGORITHM_EXECUTION_TIMEOUT_SECONDS);
            ...
            logger.error("Algorithm execution failed with exception: {}", 
    ...
    /**
     * Clean up executor service on application shutdown with proper timeout handling.
     * Follows the recommended shutdown pattern from Java Concurrency in Practice.
     */
    @PreDestroy
    public void cleanup() {
